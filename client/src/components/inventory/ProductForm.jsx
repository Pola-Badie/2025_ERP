...
const ProductForm = ({ onSuccess, productId, initialData }) => {
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className={`space-y-4 ${isRTL ? 'rtl' : 'ltr'}`} dir={isRTL ? 'rtl' : 'ltr'}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <FormField control={form.control} name="name" render={({ field }) => (<FormItem>
                ...
                <FormControl>
                  <Input placeholder="Enter product name" {...field} className={isRTL ? 'text-right' : 'text-left'}/>
                </FormControl>
                <FormMessage />
              </FormItem>)}/>
          
          <FormField control={form.control} name="drugName" render={({ field }) => (<FormItem>
                <FormLabel>{t('chemicalName')}</FormLabel>
                <FormControl>
                  <Input placeholder="Enter chemical name" {...field} className={isRTL ? 'text-right' : 'text-left'}/>
                </FormControl>
                <FormMessage />
              </FormItem>)}/>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <FormField control={form.control} name="categoryId" render={({ field }) => (<FormItem>
                <FormLabel>{t('category')}</FormLabel>
                <Select onValueChange={(value) => field.onChange(Number(value))} value={field.value ? field.value.toString() : "0"}>
                  <FormControl>
                  ...
                  <SelectContent>
                    {isLoadingCategories ? (<SelectItem value="loading" disabled>Loading categories...</SelectItem>) : (categories?.map((category) => (<SelectItem key={category.id} value={category.id.toString()}>
                          {category.name}
                        </SelectItem>)))}
                  </SelectContent>
                </Select>
          ...
          <FormField control={form.control} name="grade" render={({ field }) => (<FormItem>
                ...
                { value: 'T', label: 'Technical (T)' }
                        ...
                        <Checkbox id={`grade-${grade.value}`} checked={field.value?.includes(grade.value) || false} onCheckedChange={(checked) => {
                    const currentGrades = field.value || [];
                    if (checked) {
                        field.onChange([...currentGrades, grade.value]);
                    }
                    else {
                        field.onChange(currentGrades.filter((g) => g !== grade.value));
                    }
                }}/>
                        <label htmlFor={`grade-${grade.value}`} className={`text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70 ${isRTL ? 'text-right' : 'text-left'}`}>
                          {grade.label}
                        </label>
                      </div>))}
                  </div>
          ...
          <FormField control={form.control} name="sku" render={({ field }) => (<FormItem>
                ...
                <FormControl>
                  <Input placeholder="Enter SKU (optional)" {...field} className={isRTL ? 'text-right' : 'text-left'}/>
                </FormControl>
                <FormMessage />
              </FormItem>)}/>
        </div>
        
        <FormField control={form.control} name="gs1Code" render={({ field }) => (<FormItem>
              ...
              <FormControl>
                <Input placeholder="Enter GS1 Code for ETA compliance (optional)" {...field} value={field.value || ''} className={`font-mono ${isRTL ? 'text-right' : 'text-left'}`}/>
              </FormControl>
              <FormMessage />
            </FormItem>)}/>
        ...
        <FormField control={form.control} name="description" render={({ field }) => (<FormItem>
              ...
              <FormControl>
                <Textarea placeholder="Product description..." {...field} value={field.value || ''} className={isRTL ? 'text-right' : 'text-left'}/>
              </FormControl>
              <FormMessage />
            </FormItem>)}/>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <FormField control={form.control} name="quantity" render={({ field }) => (<FormItem>
                ...
                <FormControl>
                  <Input type="number" placeholder="0" min="0" step="1" {...field} className={isRTL ? 'text-right' : 'text-left'}/>
                </FormControl>
                <FormMessage />
              </FormItem>)}/>
          
          <FormField control={form.control} name="unitOfMeasure" render={({ field }) => (<FormItem>
          ...
          <FormField control={form.control} name="lowStockThreshold" render={({ field }) => (<FormItem>
                ...
                <FormControl>
                  <Input type="number" placeholder="Enter threshold (e.g. 10)" min="1" step="1" {...field} className={isRTL ? 'text-right' : 'text-left'}/>
                </FormControl>
                <FormMessage />
              </FormItem>)}/>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <FormField control={form.control} name="costPrice" render={({ field }) => (<FormItem>
                <FormLabel>{t('costPrice')} ($)</FormLabel>
                <FormControl>
                  <Input type="number" placeholder="0.00" step="0.01" min="0" {...field} className={isRTL ? 'text-right' : 'text-left'}/>
                </FormControl>
                <FormMessage />
              </FormItem>)}/>
          
          <FormField control={form.control} name="sellingPrice" render={({ field }) => (<FormItem>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <FormField control={form.control} name="location" render={({ field }) => (<FormItem>
                ...
                <Select onValueChange={field.onChange} value={field.value || ''}>
                  ...
                  <SelectContent>
                    {isLoadingWarehouses ? (<SelectItem value="loading" disabled>Loading warehouses...</SelectItem>) : (warehouses?.map((warehouse) => (<SelectItem key={warehouse.id} value={warehouse.name}>
                          {warehouse.name}
                        </SelectItem>)))}
                  </SelectContent>
                </Select>
          ...
          <FormField control={form.control} name="shelf" render={({ field }) => (<FormItem>
                <FormLabel>{t('shelfNumber')}</FormLabel>
                <FormControl>
                  <Input placeholder="S-101, Rack-3, etc." {...field} value={field.value || ''} className={isRTL ? 'text-right' : 'text-left'}/>
                </FormControl>
                <FormMessage />
              </FormItem>)}/>
        </div>
            ...
            return (<FormItem className="flex flex-col">
                ...
                <div className="space-y-3">
                  <div className="flex items-center space-x-2">
                    <input type="checkbox" id="no-expiry" checked={noExpiry} onChange={(e) => {
                      ...
                      <PopoverTrigger asChild>
                        <FormControl>
                          <Button variant="outline" className={cn("w-full pl-3 text-left font-normal", !field.value && "text-muted-foreground")}>
                            {field.value && field.value !== 'no-expiry' ? (format(new Date(field.value), "dd/MM/yy")) : (<span>Pick a date</span>)}
                            <CalendarIcon className="ml-auto h-4 w-4 opacity-50"/>
                          </Button>
                        </FormControl>
                      </PopoverTrigger>
                      <PopoverContent className="w-auto p-0" align="start">
                        <Calendar mode="single" selected={field.value && field.value !== 'no-expiry' ? new Date(field.value) : undefined} onSelect={(date) => field.onChange(date ? format(date, "yyyy-MM-dd") : '')} initialFocus/>
                      </PopoverContent>
                    </Popover>)}
                </div>
                <FormMessage />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <FormField control={form.control} name="productType" render={({ field }) => (<FormItem>
