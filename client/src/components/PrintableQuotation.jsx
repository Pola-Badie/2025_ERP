...
export const PrintableQuotation = ({ quotationNumber, date, validUntil, customer, items, packagingItems = [], subtotal, transportationFees, packagingFees, vatPercentage, vatAmount, grandTotal, notes, transportationType, transportationNotes, packagingType, packagingNotes, quotationType, termsAndConditions, }) => {
    ...
    return (<div className="printable-quotation bg-white p-6 max-w-4xl mx-auto text-black text-sm">
      {/* Header */}
      <div className="flex justify-between items-start mb-4 border-b pb-3">
        <div className="company-info flex items-start gap-4">
          <img src={logoPath} alt="Morgan ERP Logo" className="w-16 h-16 object-contain"/>
          <div>
      ...
      <div className="customer-info mb-4">
        ...
        <div className="bg-gray-50 p-4 rounded border">
          <div className="flex justify-between items-start">
            <div className="flex-1">
              {customer.company ? (<div>
                ...
                </div>) : (<h3 className="font-medium text-lg">{customer.company || customer.name}</h3>)}
              
              {/* Customer Code and Mobile prominently displayed */}
              <div className="flex flex-wrap gap-2 mt-2">
                {customer.id && (<span className="inline-flex items-center px-2 py-1 rounded-md text-xs font-medium bg-blue-100 text-blue-800">
                    Code: CUST-{String(customer.id).padStart(4, '0')}
                  </span>)}
                {customer.phone && (<span className="inline-flex items-center px-2 py-1 rounded-md text-xs font-medium bg-green-100 text-green-800">
          ...
          <div className="space-y-1 pt-2 border-t mt-3">
            {customer.taxNumber && (<p className="text-sm text-gray-600">ETA Number: {customer.taxNumber}</p>)}
            {customer.address && (<p className="text-sm text-gray-600">Address: {customer.address}</p>)}
          </div>
        </div>
      </div>
      ...
      <div className="items-table mb-4">
        ...
        <table className="w-full border-collapse border border-gray-300">
          ...
          <tbody>
            {items.map((item, index) => (<tr key={index} className="hover:bg-gray-50">
                ...
                <td className="border border-gray-300 px-2 py-1 text-gray-600 text-xs">
                  {item.description}
                  {item.type === 'manufacturing' && item.processingTime && (<div className="text-xs mt-1 text-blue-600">
                      Processing Time: {item.processingTime} days
                      {item.qualityGrade && ` | Quality: ${item.qualityGrade}`}
                    </div>)}
                  {item.type === 'refining' && item.specifications && (<div className="text-xs mt-1 text-blue-600">
      ...
      {transportationFees > 0 && (<div className="transportation-info mb-3">
          ...
          <div className="bg-blue-50 p-4 rounded border">
            <div className="flex justify-between items-center">
              <div>
                ...
                {transportationNotes && (<p className="text-sm text-blue-700 mt-1">{transportationNotes}</p>)}
              </div>
      ...
      {packagingItems && packagingItems.length > 0 && (<div className="packaging-info mb-3">
          <h3 className="text-sm font-semibold text-gray-800 mb-1">Packaging Items</h3>
          <div className="space-y-3">
            {packagingItems.map((item, index) => (<div key={item.id} className="bg-green-50 p-4 rounded border">
                <div className="flex justify-between items-start">
                  <div className="flex-1">
                    <p className="font-medium text-green-900">{item.type}</p>
                    {item.description && (<p className="text-sm text-green-700 mt-1">{item.description}</p>)}
                    {item.notes && (<p className="text-xs text-green-600 mt-1 italic">{item.notes}</p>)}
                  </div>
      ...
      <div className="flex justify-end mb-4">
        <div className="w-80">
          <div className="border border-gray-300 bg-gray-50">
            ...
            {transportationFees > 0 && (<div className="flex justify-between px-2 py-1 border-b border-gray-300 text-xs">
                <span className="font-medium">Transportation:</span>
                <span>EGP {transportationFees.toFixed(2)}</span>
              </div>)}
            
            {packagingFees > 0 && (<div className="flex justify-between px-2 py-1 border-b border-gray-300 text-xs">
      ...
      {/* Notes */}
      {notes && (<div className="notes mb-3">
