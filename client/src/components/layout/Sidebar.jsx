...
import { Home, Package, ShoppingCart, FileText, PieChart, Briefcase, Settings, DollarSign, Sliders, FilePlus, Receipt, BookOpen, Users, UserPlus, ClipboardList, Calculator, Landmark, Truck, ShoppingBag, Factory, History, Quote } from 'lucide-react';
...
const Sidebar = ({ className, isMobile, onClose }) => {
    ...
    return (<div className={cn("flex flex-col h-screen bg-[#1C3149] text-white transition-all duration-300", isMobile ? "w-full" : (isCollapsed ? "w-16" : "w-64"), className)}>
      {/* Header */}
      <div className="p-4 border-b border-[#2A3F55] flex-shrink-0 relative">
        <div className="flex items-center cursor-pointer hover:bg-[#26405A] rounded-md p-2 -m-2 transition-colors duration-200" onClick={() => !isMobile && toggleCollapsed()} title={isCollapsed ? "Expand sidebar" : "Collapse sidebar"}>
          <svg width="32" height="32" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="text-[#3BCEAC]">
          ...
          {isMobile && (<button onClick={onClose} className="absolute top-2 right-2 p-2 text-gray-300 hover:text-white hover:bg-[#26405A] rounded-md transition-colors z-10" aria-label="Close menu">
      ...
      <div className="flex-1 overflow-hidden">
        <nav className="h-full py-4 overflow-y-scroll" style={{
            scrollbarWidth: 'thin',
            scrollbarColor: '#4B5563 #1F2937'
        }}>
          <ul className="space-y-0">
            {navItems.map((item) => (<li key={item.path}>
                <div className={cn("flex items-center border-l-4 border-transparent hover:bg-[#26405A] cursor-pointer", isCollapsed ? "justify-center px-2 py-4" :
                language === 'ar' ? "space-x-reverse space-x-4 px-4 py-3" : "space-x-4 px-4 py-3", location === item.path && "bg-[#26405A] border-l-4 border-[#3BCEAC]")} onClick={() => {
                setLocation(item.path);
                if (isMobile && onClose)
                    onClose();
            }}>
                  <span className={cn("text-gray-300", location === item.path && "text-[#3BCEAC]")}>
                    {renderIcon(item.icon)}
                  </span>
                  {!isCollapsed && (<span className={cn("text-sm font-medium", location === item.path && "text-[#3BCEAC]")}>
                      {t(item.key).toUpperCase()}
                    </span>)}
                  {!isCollapsed && location === item.path && (<span className={language === 'ar' ? 'mr-auto' : 'ml-auto'}>
      ...
      <div className="p-4 border-t border-[#2A3F55] flex-shrink-0 space-y-3">
        {/* Language Selector */}
        {!isCollapsed && (<div className={`flex items-center justify-center mb-2 ${language === 'ar' ? 'space-x-reverse space-x-2' : 'space-x-2'}`}>
            <button className={cn("text-sm px-3 py-1 border rounded-md transition-colors flex-1", language === 'en'
                ? "bg-[#3BCEAC] text-white border-[#3BCEAC]"
                : "text-[#3BCEAC] border-[#3BCEAC] hover:bg-[#26405A]")} onClick={() => setLanguage('en')}>
              English
            </button>
            <button className={cn("text-sm px-3 py-1 border rounded-md transition-colors flex-1", language === 'ar'
                ? "bg-[#3BCEAC] text-white border-[#3BCEAC]"
                : "text-[#3BCEAC] border-[#3BCEAC] hover:bg-[#26405A]")} onClick={() => setLanguage('ar')}>
              عربي
            </button>
          </div>)}
        ...
        <div className="flex items-center justify-center">
          <button onClick={() => setShowLogoutDialog(true)} className={cn("text-white bg-red-600 hover:bg-red-700 rounded-md py-2 flex items-center justify-center transition-colors", isCollapsed ? "w-10 h-10 p-0" : "px-4 w-full")}>
            <svg xmlns="http://www.w3.org/2000/svg" className={cn("h-5 w-5", !isCollapsed && (language === 'ar' ? 'ml-2' : 'mr-2'))} fill="none" viewBox="0 0 24 24" stroke="currentColor">
