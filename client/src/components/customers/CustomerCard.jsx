...
import { FileText, Pencil, Trash2, MoreHorizontal, Eye } from 'lucide-react';
import { DropdownMenu, DropdownMenuContent, DropdownMenuItem, DropdownMenuTrigger, } from "@/components/ui/dropdown-menu";
const CustomerCard = ({ customer, onEdit, onDelete, onViewProfile, onViewOrders }) => {
    return (<div className="border-b border-slate-200 py-4 overflow-hidden">
      <div className="grid grid-cols-4 md:grid-cols-8 gap-4 items-center text-sm min-h-[60px]">
        ...
        <div className="col-span-1 overflow-hidden">
          <a href={`mailto:${customer.email}`} className="text-blue-600 hover:underline truncate block" title={customer.email}>
            {customer.email}
          </a>
        </div>
        ...
        <div className="flex justify-end md:justify-center">
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" side="bottom" className="min-w-[180px]">
              <DropdownMenuItem onClick={() => onViewProfile && onViewProfile(customer)} className="cursor-pointer">
                <Eye className="h-4 w-4 mr-2"/>
                <span>View Profile</span>
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => onViewOrders && onViewOrders(customer)} className="cursor-pointer">
                <FileText className="h-4 w-4 mr-2"/>
                <span>View Orders</span>
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => onEdit && onEdit(customer)} className="cursor-pointer">
                <Pencil className="h-4 w-4 mr-2"/>
                <span>Edit</span>
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => onDelete && onDelete(customer)} className="cursor-pointer text-red-600 hover:text-red-700 focus:text-red-700">
