...
import { AlertTriangle, XCircle, Calendar, ArrowRight, ChevronDown, ChevronUp } from 'lucide-react';
...
const ExpiringProductsCard = () => {
    ...
    const { data: expiringProducts = [], isLoading, error } = useQuery({
    ...
    return (<Card className="h-full">
      ...
      <CardContent>
        {!expiringProducts || expiringProducts.length === 0 ? (<div className="text-center py-8 text-gray-500">
            <Calendar className="h-12 w-12 mx-auto mb-2 text-gray-300"/>
            <p>{t('noProductsExpiringSoon')}</p>
          </div>) : (<div className="space-y-3">
            {(showAll ? expiringProducts : Array.isArray(expiringProducts) ? expiringProducts.slice(0, 5) : []).map((product) => (<div key={product.id} className="flex items-center justify-between p-3 border rounded-lg hover:bg-gray-50 cursor-pointer transition-all duration-200 hover:shadow-md alert-card-item" onClick={() => navigateToProduct(product.id, product.name)}>
                ...
                <div className="text-right flex-shrink-0">
                  <Badge className={`${getExpiryStatusColor(product.expiryStatus)} text-xs mb-1`}>
            ...
            {expiringProducts.length > 5 && (<div className="flex gap-2">
                <Button variant="outline" size="sm" className="flex-1" onClick={() => setShowAll(!showAll)}>
                  {showAll ? (<>
                      {t('showLess')}
                ...
                <Button variant="ghost" size="sm" onClick={() => setLocation('/inventory?tab=expiring')} className="text-blue-600 hover:text-blue-700">
