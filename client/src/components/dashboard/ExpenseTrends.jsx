        ...
        expenses.forEach(expense => {
            ...
            switch (period) {
                ...
                case 'quarter':
                    ...
                    if (monthDiff > 2 || expenseDate.getFullYear() < currentYear)
                        return;
    ...
    return (<Card className="mb-6">
      <CardContent className="p-6">
        ...
        {isLoading ? (<div className="h-72 w-full flex items-center justify-center bg-slate-50 rounded-lg">
          ...
          </div>) : (chartData.length === 0 ? (<div className="h-72 w-full flex flex-col items-center justify-center bg-slate-50 rounded-lg">
            ...
            </div>) : (<div className="h-72 w-full">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={chartData} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
                  <defs>
                    {categories.map((category, index) => (<linearGradient key={category} id={`color-${category}`} x1="0" y1="0" x2="0" y2="1">
                        <stop offset="5%" stopColor={categoryColors[category] || fallbackColors[index % fallbackColors.length]} stopOpacity={0.8}/>
                        <stop offset="95%" stopColor={categoryColors[category] || fallbackColors[index % fallbackColors.length]} stopOpacity={0.1}/>
                      </linearGradient>))}
                  </defs>
                  ...
                  <Legend />
                  {categories.map((category, index) => (<Area key={category} type="monotone" dataKey={category} name={category} stroke={categoryColors[category] || fallbackColors[index % fallbackColors.length]} fillOpacity={1} fill={`url(#color-${category})`} stackId="1"/>))}
                </AreaChart>
              </ResponsiveContainer>
            </div>))}
      </CardContent>
