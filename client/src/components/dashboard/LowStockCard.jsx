...
const LowStockCard = () => {
    ...
    return (<Card className="h-full">
      ...
      <CardContent>
        {!lowStockProducts || lowStockProducts.length === 0 ? (<div className="text-center py-8 text-gray-500">
            <Package className="h-12 w-12 mx-auto mb-2 text-gray-300"/>
            <p>{t('allProductsWellStocked')}</p>
          </div>) : (<div className="space-y-3">
            {(showAll ? lowStockProducts : lowStockProducts.slice(0, 5)).map((product) => (<div key={product.id} className="flex items-center justify-between p-3 border rounded-lg hover:bg-gray-50 cursor-pointer transition-all duration-200 hover:shadow-md alert-card-item" onClick={() => navigateToProduct(product.id, product.name)}>
                ...
                <div className="text-right flex-shrink-0">
                  <Badge className={`${getStockStatusColor(product.stockStatus)} text-xs mb-1`}>
            ...
            {lowStockProducts.length > 5 && (<div className="flex gap-2">
                ...
                <Button variant="ghost" size="sm" onClick={() => setLocation('/inventory?tab=low-stock')} className="text-blue-600 hover:text-blue-700">
