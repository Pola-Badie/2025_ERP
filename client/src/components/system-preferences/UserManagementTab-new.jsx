...
import { Loader2, Pencil, UserX, Download, Settings, Trash2, ShieldCheck } from 'lucide-react';
...
const UserManagementTab = ({ preferences, refetch }) => {
    ...
    const [modulePermissionFeatures, setModulePermissionFeatures] = useState({});
    ...
    const hasPermission = (moduleName) => {
        if (!userPermissions || !Array.isArray(userPermissions))
            return false;
        const permission = userPermissions.find((p) => p.moduleName === moduleName);
        return permission?.accessGranted === true;
    };
    // Toggle permission mutation
    const togglePermissionMutation = useMutation({
        mutationFn: async ({ userId, moduleName, accessGranted }) => {
    ...
    const handleTogglePermission = async (moduleName, accessGranted) => {
    ...
    const handleConfigurePermissions = (permission) => {
    ...
    const managerCount = Array.isArray(users) ? users.filter((user) => user.role === 'manager').length : 0;
    const staffCount = Array.isArray(users) ? users.filter((user) => user.role === 'staff').length : 0;
    if (isLoading) {
    ...
    return (<div className="space-y-6">
      ...
      <div className="rounded-md border">
        <Table>
          ...
          <TableBody>
            ...
            }} className="h-6 w-6 p-0" title="Reset Password">
        ...
        <DialogContent>
          ...
          <Form {...editForm}>
            <form onSubmit={editForm.handleSubmit((data) => updateUserMutation.mutate(data))} className="space-y-4">
              <FormField control={editForm.control} name="username" render={({ field }) => (<FormItem>
                    <FormLabel>Username</FormLabel>
                    <FormControl>
                      <Input placeholder="Enter username" {...field}/>
                    </FormControl>
              ...
              <FormField control={editForm.control} name="email" render={({ field }) => (<FormItem>
                    <FormLabel>Email</FormLabel>
                    <FormControl>
                      <Input type="email" placeholder="Enter email" {...field}/>
                    </FormControl>
              ...
              <FormField control={editForm.control} name="role" render={({ field }) => (<FormItem>
                    <FormLabel>Role</FormLabel>
      ...
      <Dialog open={isPermissionsDialogOpen} onOpenChange={setIsPermissionsDialogOpen}>
        <DialogContent className="sm:max-w-[800px] max-h-[80vh]">
          ...
          <div className="space-y-4 overflow-y-auto max-h-[60vh] pr-2">
            ...
            const permissionRecord = userPermissions.find((p) => p.moduleName === module.key);
            ...
            return (<div key={module.key} className="grid grid-cols-3 gap-4 py-3 border-b items-center hover:bg-gray-50">
                    <div>
                    ...
                    <div className="flex items-center gap-2">
                      <Switch checked={hasPermission} disabled={isUpdatingPermission} onCheckedChange={(checked) => handleTogglePermission(module.key, checked)}/>
                      {isUpdatingPermission && (<Loader2 className="h-3 w-3 animate-spin text-muted-foreground"/>)}
                      {hasPermission ? (<Badge variant="secondary" className="bg-green-100 text-green-800 text-xs">
                          {t('grantedStatus')} (ID: {permissionRecord?.id})
                        </Badge>) : (<Badge variant="secondary" className="bg-gray-100 text-gray-800 text-xs">
                          {t('deniedStatus')}
                        </Badge>)}
                    </div>
                    <div className="flex gap-2">
                      <Button variant="outline" size="sm" onClick={() => handleConfigurePermissions({ moduleName: module.key, accessGranted: hasPermission })} disabled={!hasPermission || isUpdatingPermission}>
                        <Settings className="h-3 w-3 mr-1"/>
                        {t('configure')}
                      </Button>
                      {hasPermission && (<Button variant="outline" size="sm" className="text-red-600 hover:text-red-700" disabled={isUpdatingPermission} onClick={() => handleTogglePermission(module.key, false)}>
          ...
          <DialogFooter className="mt-6 pt-4 border-t">
            <Button variant="outline" onClick={() => setIsPermissionsDialogOpen(false)} className="flex-1">
              Close
            </Button>
            <Button onClick={() => {
            toast({
      ...
      <Dialog open={isConfigurePermissionsOpen} onOpenChange={setIsConfigurePermissionsOpen}>
        <DialogContent className="sm:max-w-[700px] max-h-[80vh] overflow-y-auto">
          ...
          <div className="space-y-6">
            {selectedPermission && moduleFeatures[selectedPermission.moduleName] && (<div className="space-y-4">
                ...
                {selectedPermission.accessGranted ? (<div className="space-y-6">
                    {/* Group features by category */}
                    {["tabs", "content", "actions"].map((category) => {
                    const categoryFeatures = moduleFeatures[selectedPermission.moduleName]
                    ...
                    return (<div key={category} className="space-y-3">
                          ...
                          <div className="grid gap-3">
                            {categoryFeatures.map((feature) => (<div key={feature.key} className="flex items-start justify-between p-3 bg-gray-50 rounded-lg border border-gray-200 hover:bg-gray-100 transition-colors">
                                ...
                                <div className="ml-3">
                                  <Switch checked={modulePermissionFeatures[feature.key] ?? true} onCheckedChange={(checked) => {
                          ...
                          <div className="text-xs text-muted-foreground bg-blue-50 p-2 rounded border border-blue-200">
                            <span className="font-medium text-blue-800">
                              {categoryFeatures.filter((f) => modulePermissionFeatures[f.key] ?? true).length} of {categoryFeatures.length} features enabled
                            </span>
          ...
          <div className="flex justify-end gap-2 pt-4">
            <Button variant="outline" onClick={() => setIsConfigurePermissionsOpen(false)}>
              Cancel
            </Button>
            <Button onClick={() => {
      ...
      <Dialog open={isPasswordResetDialogOpen} onOpenChange={setIsPasswordResetDialogOpen}>
        <DialogContent>
          ...
          <div className="space-y-4">
            <div>
              ...
              <Input id="new-password" type="password" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} placeholder="Enter new password (min 6 characters)" data-testid="input-new-password"/>
        ...
        }} disabled={resetPasswordMutation.isPending || newPassword.length < 6} data-testid="button-reset-password">
