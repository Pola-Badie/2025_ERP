...
const InventorySettingsTab = ({ preferences, refetch }) => {
    ...
    useEffect(() => {
        if (preferences) {
            const inventoryPrefs = preferences.filter((pref) => pref.category === 'inventory');
            if (inventoryPrefs.length) {
                const prefsObj = {};
                inventoryPrefs.forEach((pref) => {
    ...
    const handleSaveAll = async () => {
        ...
        try {
            ...
            const settingsToSave = Object.entries(settings).map(([key, value]) => ({
                key: `inventory_${key}`,
                value,
                existingPref: preferences?.find((pref) => pref.key === `inventory_${key}`),
            }));
    ...
    return (<div className="space-y-6">
      ...
      <Card>
        ...
        <div className="space-y-3">
          ...
          <Input id="lowStockThreshold" type="number" value={settings.lowStockThreshold} onChange={(e) => handleChangeSetting('lowStockThreshold', e.target.value)} min="0"/>
          <p className="text-sm text-muted-foreground">
        ...
        {/* Default Unit */}
            <div className="space-y-3">
              ...
              <Select value={settings.defaultUnit} onValueChange={(value) => handleChangeSetting('defaultUnit', value)}>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="flex items-center justify-between p-4 border rounded-lg">
            ...
            <Switch id="batchTracking" checked={settings.batchTracking} onCheckedChange={(checked) => handleChangeSetting('batchTracking', checked)}/>
          </div>
          ...
          <div className="flex items-center justify-between p-4 border rounded-lg">
            ...
            <Switch id="expiryAlerts" checked={settings.expiryAlerts} onCheckedChange={(checked) => handleChangeSetting('expiryAlerts', checked)}/>
          </div>
          ...
          <div className="flex items-center justify-between p-4 border rounded-lg">
            ...
            <Switch id="barcodeGeneration" checked={settings.barcodeGeneration} onCheckedChange={(checked) => handleChangeSetting('barcodeGeneration', checked)}/>
          </div>
        </CardContent>
      </Card>
    </div>);
