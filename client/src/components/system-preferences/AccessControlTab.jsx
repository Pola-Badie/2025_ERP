...
const AccessControlTab = ({ preferences, refetch }) => {
    ...
    useEffect(() => {
        if (preferences) {
            const accessPrefs = preferences.filter((pref) => pref.category === 'access_control');
            if (accessPrefs.length) {
                const prefsObj = {};
                accessPrefs.forEach((pref) => {
    ...
    const handleSaveAll = async () => {
        ...
        try {
            ...
            const settingsToSave = Object.entries(settings).map(([key, value]) => ({
                key: `access_${key}`,
                value,
                existingPref: preferences?.find((pref) => pref.key === `access_${key}`),
            }));
    ...
    return (<div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-3">
          ...
          <Input id="sessionTimeout" type="number" value={settings.sessionTimeout} onChange={(e) => handleChangeSetting('sessionTimeout', e.target.value)} min="5"/>
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between">
          ...
          <Switch id="maintenanceMode" checked={settings.maintenanceMode} onCheckedChange={(checked) => handleChangeSetting('maintenanceMode', checked)}/>
        </div>
        ...
        <div className="flex items-center justify-between">
          ...
          <Switch id="loginLogs" checked={settings.loginLogs} onCheckedChange={(checked) => handleChangeSetting('loginLogs', checked)}/>
        </div>
      </div>
