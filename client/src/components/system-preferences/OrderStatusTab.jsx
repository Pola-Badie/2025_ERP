...
const OrderStatusTab = ({ preferences, refetch }) => {
    ...
    return (<div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Button onClick={handleReset} variant="outline" size="sm" disabled={saveStatusMutation.isPending || initializeStatusMutation.isPending}>
            <RotateCcw className="h-4 w-4 mr-1"/>
            Reset to Default
          </Button>
          <Button onClick={handleSave} size="sm" disabled={saveStatusMutation.isPending || initializeStatusMutation.isPending}>
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          {orderStatuses.map((status, index) => (<div key={status.key} className="p-4 border rounded-lg space-y-4">
              <div className="flex items-center justify-between">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <Input id={`label-${status.key}`} value={status.label} onChange={(e) => handleStatusChange(index, 'label', e.target.value)} placeholder="Status label"/>
                </div>

                <div>
                  <Label>Color Preset</Label>
                  <div className="flex flex-wrap gap-2 mt-1">
                    {colorPresets.map((preset) => (<button key={preset.name} onClick={() => {
                ...
                }} className={`px-3 py-1 rounded text-xs font-medium border ${preset.bg} ${preset.text} ${preset.border} hover:opacity-80 transition-opacity`} title={`Apply ${preset.name} color scheme`}>
                        {preset.name}
                      </button>))}
                  </div>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  ...
                  <Input id={`bg-${status.key}`} value={status.color} onChange={(e) => handleStatusChange(index, 'color', e.target.value)} placeholder="bg-yellow-100"/>
                </div>
                <div>
                  <Label htmlFor={`text-${status.key}`}>Text Color Class</Label>
                  <Input id={`text-${status.key}`} value={status.textColor} onChange={(e) => handleStatusChange(index, 'textColor', e.target.value)} placeholder="text-yellow-800"/>
                </div>
                <div>
                  <Label htmlFor={`border-${status.key}`}>Border Class</Label>
                  <Input id={`border-${status.key}`} value={status.borderColor} onChange={(e) => handleStatusChange(index, 'borderColor', e.target.value)} placeholder="border-yellow-300"/>
                </div>
              </div>
            </div>))}
        </CardContent>
      ...
      <Card>
        ...
        <CardContent>
          <div className="flex flex-wrap gap-3">
            {orderStatuses.map((status) => (<Badge key={status.key} className={`${status.color} ${status.textColor} ${status.borderColor} border px-2 py-1 text-xs font-medium`}>
                {status.label}
              </Badge>))}
          </div>
        </CardContent>
