...
const ProductSearch = ({ value, onChange, quotationType }) => {
    ...
    return (<div className="flex flex-col space-y-2 w-full">
      <div className="flex items-center">
        ...
        {value && (<Button variant="ghost" size="sm" className="h-6 px-2 ml-2" onClick={clearSelection} data-testid="button-clear-product">
            <X className="h-3 w-3 mr-1"/>
            Clear
          </Button>)}
      </div>
      {value ? (<div className="flex items-start gap-2 border rounded-md p-3 relative">
          ...
          <div className="flex-1 space-y-1">
            ...
            <div className="flex items-center gap-2">
              ...
              {value.productType && (<Badge variant="secondary" className={`text-xs ${getProductTypeColor(value.productType)}`}>
                  {value.productType}
                </Badge>)}
            </div>
          </div>
          ...
          <PopoverTrigger asChild>
            <Button variant="outline" role="combobox" aria-expanded={open} className="w-full justify-between" data-testid="button-select-product">
          ...
          <PopoverContent className="w-[450px] p-0" align="start">
            <Command>
              <CommandInput placeholder="Search products by name, SKU, drug name..." value={searchTerm} onValueChange={setSearchTerm} className="h-9" data-testid="input-search-product"/>
              <CommandList>
                <CommandEmpty>
                  {isLoading ? "Loading..." : "No products found."}
                </CommandEmpty>
                <CommandGroup>
                  {filteredProducts.map((product) => (<CommandItem key={product.id} value={product.id.toString()} onSelect={() => handleSelect(product)} className="flex items-start gap-2 py-3 px-2" data-testid={`option-product-${product.id}`}>
                      <div className="h-8 w-8 rounded-full bg-primary/10 flex items-center justify-center">
                        <Package className="h-4 w-4 text-primary"/>
                      </div>
                      <div className="flex-1 space-y-1">
                        ...
                        <div className="flex items-center gap-2">
                          {product.productType && (<Badge variant="secondary" className={`text-xs ${getProductTypeColor(product.productType)}`}>
                              {product.productType}
                            </Badge>)}
                          {product.manufacturer && (<span className="text-xs text-muted-foreground">by {product.manufacturer}</span>)}
                        </div>
                      </div>
                    </CommandItem>))}
                </CommandGroup>
