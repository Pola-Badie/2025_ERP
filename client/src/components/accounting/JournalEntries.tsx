...
import { PlusCircle, Trash2, FileText, ChevronDown, ChevronUp, Check, X, Plus, DollarSign } from "lucide-react";
  ...
  lines: z.array(journalLineSchema)
    ...
    .refine(
      (lines) => {
        ...
        return Math.abs(totalDebits - totalCredits) < 0.001; // Allow for small rounding errors
      },
      {
...
const JournalEntryDetailsDialog: React.FC<JournalEntryDetailsDialogProps> = ({ entry, onClose }) => {
  ...
  // Calculate totals
  const totalDebits = entry.lines?.reduce((sum, line) => sum + parseFloat(line.debit || "0"), 0) || 0;
  const totalCredits = entry.lines?.reduce((sum, line) => sum + parseFloat(line.credit || "0"), 0) || 0;
    ...
    <Dialog open={true} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[600px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Viewing journal entry #{entry.id}
          </DialogDescription>
        </DialogHeader>
              ...
              <TableBody>
                {entry.lines?.map((line, index) => (
                  <TableRow key={index}>
                    ...
                    <TableCell className="text-right">{parseFloat(line.debit) ? '$' + parseFloat(line.debit).toFixed(2) : '-'}</TableCell>
                    <TableCell className="text-right">{parseFloat(line.credit) ? '$' + parseFloat(line.credit).toFixed(2) : '-'}</TableCell>
                  </TableRow>
                ))}
                <TableRow className="bg-muted/50">
                  <TableCell colSpan={2} className="font-medium">Total</TableCell>
                  <TableCell className="text-right font-medium">${totalDebits.toFixed(2)}</TableCell>
                  <TableCell className="text-right font-medium">${totalCredits.toFixed(2)}</TableCell>
                </TableRow>
              </TableBody>
            </Table>
          </div>
...
const JournalEntries: React.FC = () => {
  ...
  const [selectedEntry, setSelectedEntry] = useState<(JournalEntry & { lines?: any[] }) | null>(null);
  ...
  const { data: entries = [], isLoading } = useQuery({
    queryKey: ['/api/journal-entries', dateRange],
    queryFn: async () => {
      const res = await apiRequest('GET', `/api/journal-entries?startDate=${dateRange.startDate}&endDate=${dateRange.endDate}`);
      return await res.json();
    }
  });
  ...
  const createJournalMutation = useMutation({
    mutationFn: async (data: JournalEntryFormValues) => {
      // Format the data
      const formattedData = {
        entry: {
          date: data.date,
      ...
      const res = await apiRequest('POST', '/api/journal-entries', formattedData);
      return await res.json();
    },
    onSuccess: () => {
  ...
  // Helper function to view entry details
  const viewEntryDetails = async (entryId: number) => {
    try {
