...
import { PlusCircle, DollarSign, Search, AlertCircle, FileText, ChevronDown, ChevronUp } from "lucide-react";
...
const paymentFormSchema = z.object({
    ...
    allocations: z.array(z.object({
        ...
        const totalAllocations = allocations.reduce((sum, allocation) => sum + parseFloat(allocation.amount || "0"), 0);
...
const CustomerPayments = () => {
    ...
    const [localPayments, setPayments] = useState([]);
    ...
    const calculateAllocationStats = () => {
        ...
        const totalAllocated = watchAllocations.reduce((sum, allocation) => sum + parseFloat(allocation.amount || "0"), 0);
        return {
            paymentAmount,
            totalAllocated,
    ...
    return (<div className="space-y-6">
            ...
            <Form {...form}>
              <form onSubmit={form.handleSubmit((data) => createPaymentMutation.mutate(data))} className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <FormField control={form.control} name="customerId" render={({ field }) => (<FormItem>
                        <FormLabel>Customer</FormLabel>
                        <Select onValueChange={(value) => handleCustomerChange(parseInt(value))} defaultValue={field.value ? field.value.toString() : undefined}>
                          <FormControl>
                            <SelectTrigger>
                          ...
                          <SelectContent>
                            {customers.map((customer) => (<SelectItem key={customer.id} value={customer.id.toString()}>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <FormField control={form.control} name="paymentDate" render={({ field }) => (<FormItem>
                ...
                {watchCustomerId && fields.length > 0 ? (<div className="space-y-4">
                    <div className="flex justify-between items-center">
                      <h4 className="text-sm font-medium">Invoice Allocations</h4>
                      <Button type="button" variant="outline" size="sm" onClick={autoAllocatePayment} disabled={!watchAmount || parseFloat(watchAmount) <= 0}>
                        Auto-Allocate
                      </Button>
                    </div>
                    ...
                    <Card>
                      <CardContent className="p-4">
                        <div className="space-y-2">
                          ...
                          {allocationStats.remaining < 0 && (<Alert variant="destructive">
                          ...
                          {allocationStats.remaining > 0 && allocationStats.paymentAmount > 0 && (<Alert>
                              <AlertCircle className="h-4 w-4"/>
                    ...
                    <Table>
                      ...
                      <TableBody>
                        {fields.map((field, index) => (<TableRow key={field.id}>
                            <TableCell>
                              <FormField control={form.control} name={`allocations.${index}.amount`} render={({ field: { value, onChange } }) => (<Checkbox checked={parseFloat(value) > 0} onCheckedChange={(checked) => {
                        onChange(checked
                            ...
                            <TableCell className="text-right">
                              <FormField control={form.control} name={`allocations.${index}.amount`} render={({ field }) => (<FormItem className="m-0">
                                    <FormControl>
                                      <div className="relative">
                                        <DollarSign className="absolute left-2 top-2.5 h-4 w-4 text-muted-foreground"/>
      ...
      <div className="flex justify-between mb-4">
        <div className="flex items-center space-x-4">
          <div className="flex items-center space-x-2">
            ...
            <Select defaultValue="all" value={invoiceStatusFilter} onValueChange={(value) => setInvoiceStatusFilter(value)}>
        ...
        <div className="flex items-center space-x-4">
          <div className="w-[250px]">
            <Select onValueChange={(value) => setSelectedCustomer(value === "all" ? null : parseInt(value))} value={selectedCustomer?.toString() || "all"}>
              <SelectTrigger>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
        ...
        <TabsContent value="pending-invoices" className="space-y-4">
          <div className="rounded-md border">
            <Table>
              ...
              <TableBody>
                {isLoadingInvoices ? (<TableRow>
                  ...
                  </TableRow>) : pendingInvoices.length === 0 ? (<TableRow>
            ...
            return filteredInvoices.map((invoice, index) => (<TableRow key={invoice.id}>
                        ...
                        <TableCell>
                          <Badge variant={invoice.status === 'overdue' ? 'destructive' :
            ...
            <Table>
              ...
              <TableBody>
                {isLoadingPayments ? (<TableRow>
                  ...
                  </TableRow>) : (payments.map((payment, index) => (<React.Fragment key={payment.id}>
                      <TableRow>
                        <TableCell>
                          <Button variant="ghost" size="icon" onClick={() => togglePaymentExpand(payment.id)}>
                            {expandedPayments.includes(payment.id) ? (<ChevronDown className="h-4 w-4"/>) : (<ChevronUp className="h-4 w-4"/>)}
                          </Button>
                        </TableCell>
                ...
                payment.paymentMethod === 'creditCard' ? 'Credit Card' :
                    payment.paymentMethod.charAt(0).toUpperCase() + payment.paymentMethod.slice(1)}
                        ...
                        <TableCell>
                          <Badge variant={payment.status === 'completed' ? 'outline' :
                payment.status === 'pending' ? 'secondary' :
                    'destructive'}>
                      ...
                      {expandedPayments.includes(payment.id) && payment.allocations.length > 0 && (<TableRow className="bg-muted/30">
                          <TableCell colSpan={9}>
                            <div className="px-4 py-2">
                              ...
                              <Table>
                                ...
                                <TableBody>
                                  {payment.allocations.map((allocation) => (<TableRow key={allocation.id}>
      ...
      {selectedPayment && (<Dialog open={isPaymentDetailsDialogOpen} onOpenChange={setIsPaymentDetailsDialogOpen}>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Badge variant={selectedPayment.status === 'completed' ? 'outline' :
              ...
              <div>
                ...
                {selectedPayment.allocations.length === 0 ? (<p className="text-sm text-muted-foreground">No allocations for this payment.</p>) : (<Table>
                    <TableHeader>
                      <TableRow>
                        <TableHead>Invoice #</TableHead>
                        <TableHead className="text-right">Amount Applied</TableHead>
                    ...
                    <TableBody>
                      {selectedPayment.allocations.map((allocation) => (<TableRow key={allocation.id}>
                          <TableCell>{allocation.invoiceNumber}</TableCell>
                          <TableCell className="text-right">{formatCurrency(allocation.amount)}</TableCell>
                        </TableRow>))}
                      <TableRow className="bg-muted/50">
                        ...
                        <TableCell className="text-right font-medium">
                          {formatCurrency(selectedPayment.allocations.reduce((sum, a) => sum + a.amount, 0))}
                        </TableCell>
                      </TableRow>
                    </TableBody>
                  </Table>)}
