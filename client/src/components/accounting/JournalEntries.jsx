...
const JournalEntryDetailsDialog = ({ entry, onClose }) => {
    return (<Dialog open={!!entry} onOpenChange={onClose}>
...
const JournalEntries = () => {
    ...
    return (<div className="space-y-6 h-[calc(100vh-200px)] overflow-y-auto">
              ...
              <Form {...form}>
                <form onSubmit={form.handleSubmit((data) => createJournalMutation.mutate(data))} className="space-y-4">
                  ...
                  <FormField control={form.control} name="description" render={({ field }) => (<FormItem>
                        ...
                        <FormControl>
                          <Textarea placeholder="Description of the journal entry" className="resize-none" {...field}/>
                        </FormControl>
                        <FormMessage />
                      </FormItem>)}/>
                  ...
                  <div>
                    <div className="flex justify-between items-center mb-2">
                      ...
                      <Button type="button" variant="outline" size="sm" onClick={() => append({ accountId: 0, description: '', debit: '', credit: '' })}>
                      ...
                      <Table>
                        ...
                        <TableBody>
                          {fields.map((field, index) => (<TableRow key={field.id}>
                              <TableCell>
                                <FormField control={form.control} name={`lines.${index}.accountId`} render={({ field }) => (<FormItem className="m-0">
                                      <Select onValueChange={(value) => field.onChange(parseInt(value))} defaultValue={field.value ? field.value.toString() : undefined}>
                                        ...
                                        <SelectContent className="max-h-[200px]">
                                          {Object.entries(groupedAccounts).map(([type, accs]) => (<React.Fragment key={type}>
                                              <div className="px-2 py-1.5 text-xs font-medium text-muted-foreground">
                                                {type}
                                              </div>
                                              {accs.map((account) => (<SelectItem key={account.id} value={account.id.toString()}>
                              ...
                              <TableCell>
                                <FormField control={form.control} name={`lines.${index}.description`} render={({ field }) => (<FormItem className="m-0">
                                      <FormControl>
                              ...
                              <TableCell>
                                <FormField control={form.control} name={`lines.${index}.debit`} render={({ field }) => (<FormItem className="m-0">
                                      <FormControl>
                                        <div className="relative">
                                          <DollarSign className="absolute left-2 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground"/>
                                          <Input placeholder="0.00" className="pl-8 text-right" type="number" step="0.01" min="0" {...field} onChange={(e) => {
                    field.onChange(e);
                    if (e.target.value) {
                        form.setValue(`lines.${index}.credit`, '');
                              ...
                              <TableCell>
                                <FormField control={form.control} name={`lines.${index}.credit`} render={({ field }) => (<FormItem className="m-0">
                                      <FormControl>
                                        <div className="relative">
                                          <DollarSign className="absolute left-2 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground"/>
                                          <Input placeholder="0.00" className="pl-8 text-right" type="number" step="0.01" min="0" {...field} onChange={(e) => {
                    ...
                    if (e.target.value) {
                        form.setValue(`lines.${index}.debit`, '');
                              ...
                              <TableCell>
                                {fields.length > 2 && (<Button type="button" variant="ghost" size="icon" onClick={() => remove(index)}>
                    ...
                    <div className="flex justify-end mt-2">
                      {totals.isBalanced ? (<Badge variant="outline" className="bg-green-50 text-green-700 border-green-200 flex items-center">
                          <Check className="h-3.5 w-3.5 mr-1"/>
                          Balanced
                        </Badge>) : (<Badge variant="outline" className="bg-red-50 text-red-700 border-red-200 flex items-center">
                  ...
                  <DialogFooter>
                    ...
                    <Button type="submit" disabled={createJournalMutation.isPending || !totals.isBalanced}>
        ...
        <CardContent className="p-4 pt-0">
          <div className="flex space-x-4">
            <div className="grid w-full items-center gap-1.5">
              ...
              <Input id="startDate" type="date" value={dateRange.startDate} onChange={(e) => handleDateRangeChange('startDate', e.target.value)}/>
            </div>
            <div className="grid w-full items-center gap-1.5">
              <label htmlFor="endDate" className="text-sm font-medium">End Date</label>
              <Input id="endDate" type="date" value={dateRange.endDate} onChange={(e) => handleDateRangeChange('endDate', e.target.value)}/>
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      {isLoading ? (<div className="flex justify-center items-center h-64">
        ...
        </div>) : entries.length === 0 ? (<div className="bg-muted/30 p-8 rounded-lg border border-dashed flex flex-col items-center justify-center text-center">
          ...
          <Table>
            ...
            <TableBody>
              {entries.map((entry) => (<TableRow key={entry.id}>
                  ...
                  <TableCell>
                    <Button variant="ghost" size="icon" onClick={() => viewEntryDetails(entry.id)}>
      ...
      {selectedEntry && (<JournalEntryDetailsDialog entry={selectedEntry} onClose={() => setSelectedEntry(null)}/>)}
    </div>);
};
