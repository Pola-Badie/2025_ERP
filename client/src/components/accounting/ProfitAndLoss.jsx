...
const ProfitAndLoss = () => {
    ...
    return (<div className="space-y-6">
        ...
        <CardContent className="space-y-4">
          <div className="flex flex-wrap gap-4 items-end">
            ...
            <div className="flex-1 min-w-40">
              ...
              <Input type="date" value={dateRange.startDate} onChange={(e) => handleDateRangeChange('startDate', e.target.value)} className="w-full"/>
            </div>
            <div className="flex-1 min-w-40">
              <label className="text-sm font-medium">End Date</label>
              <Input type="date" value={dateRange.endDate} onChange={(e) => handleDateRangeChange('endDate', e.target.value)} className="w-full"/>
            </div>
            <Button onClick={handleGenerateReport} disabled={isLoading}>
              <RefreshCw className={`h-4 w-4 mr-2 ${isLoading ? 'animate-spin' : ''}`}/>
              {isLoading ? 'Generating...' : 'Generate Report'}
            </Button>
            {data && (<Button onClick={handlePrint} disabled={isPrinting} variant="outline">
      ...
      {isLoading ? (<div className="flex justify-center items-center h-64">
        ...
        </div>) : (<Card className="w-full" id="pnl-report">
          ...
          <CardContent className="space-y-6 p-6">
            <Table>
              ...
              <TableBody>
                ...
                {data.revenue.accounts.map((account, index) => (<TableRow key={index}>
                ...
                {data.expenses.accounts.map((expense, index) => (<TableRow key={index}>
