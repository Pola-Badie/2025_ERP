...
import { Upload, Package, CheckCircle, Eye, RefreshCw, Download, Users } from 'lucide-react';
import { parseCSV, readFileAsText, readDataFile, isExcelFile } from '@/lib/csv-utils';
import { useToast } from '@/hooks/use-toast';
export const CSVImport = ({ onImport, buttonText = 'Import Data', className = '', variant = 'outline', size = 'default', accept = '.csv,.xls,.xlsx,.xlsm', hasHeader = true, requiredColumns = [], validateRow, showWarehouseDialog = false, warehouseLocations = ['Warehouse 1', 'Warehouse 2', 'Warehouse 3', 'Warehouse 4', 'Warehouse 5', 'Warehouse 6'], dialogTitle, dialogDescription, dataType = 'inventory', hideWarehouseSelection = false }) => {
    ...
    const handleFileSelect = async (event) => {
        ...
        try {
            ...
            if (requiredColumns.length > 0 && jsonData.length > 0) {
                ...
                const missingColumns = requiredColumns.filter(col => !fileColumns.some(fileCol => fileCol.toLowerCase().includes(col.toLowerCase())));
    ...
    return (<>
      ...
      <Button onClick={handleButtonClick} variant={variant} size={size} className={className} disabled={isLoading}>
        {isLoading ? (<>
      ...
      <Dialog open={showDialog} onOpenChange={setShowDialog}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-gray-100">
          ...
          <div className="space-y-6">
            ...
            {dataType === 'customers' && (<div className="bg-green-50 p-4 rounded-lg border border-green-200">
            ...
            {!hideWarehouseSelection && dataType === 'inventory' && (<div className="bg-purple-50 p-4 rounded-lg border border-purple-200">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Select value={selectedWarehouse} onValueChange={setSelectedWarehouse}>
                      ...
                      <SelectContent>
                        {warehouseLocations.map((location) => (<SelectItem key={location} value={location}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex gap-2">
                    <button onClick={downloadTemplate} className="flex-1 text-xs bg-blue-600 text-white px-3 py-2 rounded hover:bg-blue-700 flex items-center justify-center gap-1">
            ...
            {pendingData.length > 0 && (<div className="bg-green-50 p-4 rounded-lg border border-green-200">
                <h3 className="text-lg font-semibold text-green-900 mb-4 flex items-center">
                    ...
                    <div className="overflow-x-auto">
                      <table className="min-w-full text-xs">
                        <thead className="bg-gray-50">
                          <tr>
                            {pendingData.length > 0 && Object.keys(pendingData[0]).slice(0, 6).map((key) => (<th key={key} className="px-3 py-2 text-left font-medium text-gray-900 border-r border-gray-200">
                                {key}
                              </th>))}
                          </tr>
                        </thead>
                        <tbody>
                          {pendingData.slice(0, 3).map((row, index) => (<tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                              {Object.values(row).slice(0, 6).map((value, cellIndex) => (<td key={cellIndex} className="px-3 py-2 text-gray-700 border-r border-gray-200">
          ...
          <DialogFooter className="gap-3 pt-6 border-t">
            <Button variant="outline" onClick={handleDialogCancel} disabled={isLoading} className="border-gray-300 hover:bg-gray-50">
              Cancel Import
            </Button>
            <Button variant="outline" disabled={pendingData.length === 0} className="border-blue-300 text-blue-600 hover:bg-blue-50">
              <Eye className="h-4 w-4 mr-2"/>
              Preview Changes
            </Button>
            <Button onClick={handleWarehouseConfirm} disabled={(dataType === 'inventory' && !selectedWarehouse) || isLoading} className="bg-blue-600 hover:bg-blue-700 text-white">
              {isLoading ? (<>
                  <RefreshCw className="h-4 w-4 mr-2 animate-spin"/>
                  Importing...
                </>) : (<>
