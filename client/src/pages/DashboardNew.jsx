...
import { Users, DollarSign, Package, TrendingUp, TrendingDown, Receipt, BarChart3, Expand, X, Calendar, RefreshCw, Radio } from 'lucide-react';
import { LineChart as RechartsLineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer, PieChart as RechartsPieChart, Cell, Pie } from 'recharts';
...
const DashboardNew = () => {
    ...
    const { data: accountingOverview, isLoading: isOverviewLoading } = useQuery({
    ...
    const { data: inventorySummary, isLoading: isInventoryLoading } = useQuery({
    ...
    const { data: warehouseBreakdown, isLoading: isWarehouseLoading } = useQuery({
    ...
    const { data: salesDistributionData = [], isLoading: isSalesDistributionLoading } = useQuery({
    ...
    const { data: categoryPerformanceData = [], isLoading: isCategoryPerformanceLoading } = useQuery({
        queryKey: ['/api/dashboard/category-performance'],
        refetchInterval: 30000,
        refetchIntervalInBackground: true,
        staleTime: 0,
    });
    return (<div className="space-y-6 px-6 pt-2 pb-6">
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex items-center space-x-4">
          ...
          <Button variant="outline" size="sm" onClick={handleManualRefresh} className="flex items-center space-x-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        ...
        <Card className="bg-white border rounded-lg shadow-sm">
          <CardHeader className="flex flex-row items-center justify-between pb-3 border-b">
            ...
            <Button variant="ghost" size="sm" onClick={() => setExpandedChart('sales-overview')} className="h-8 w-8 p-0 hover:bg-gray-100">
          ...
          <CardContent className="p-6">
            <div className="h-[260px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <RechartsLineChart data={salesData || []} margin={{ top: 15, right: 25, left: 15, bottom: 20 }}>
                  <defs>
                  ...
                  <XAxis dataKey="name" axisLine={false} tickLine={false} fontSize={11} tick={{ fill: '#6b7280', fontWeight: 500 }} height={35}/>
                  <YAxis axisLine={false} tickLine={false} fontSize={10} tick={{ fill: '#9ca3af' }} width={45}/>
            ...
            padding: '12px'
                  ...
                  <Line type="monotone" dataKey="sales" stroke="#1D3E78" strokeWidth={3} dot={{ fill: '#ffffff', stroke: '#1D3E78', strokeWidth: 3, r: 4 }} activeDot={{ r: 6, fill: '#1D3E78', stroke: '#ffffff', strokeWidth: 2 }} fill="url(#salesGradient)"/>
                </RechartsLineChart>
              </ResponsiveContainer>
            </div>
          </CardContent>
        ...
        <Card className="bg-white border rounded-lg shadow-sm">
          <CardHeader className="flex flex-row items-center justify-between pb-3 border-b">
            ...
            <Button variant="ghost" size="sm" onClick={() => setExpandedChart('sales-distribution')} className="h-8 w-8 p-0 hover:bg-gray-100">
              <Expand className="h-4 w-4"/>
            </Button>
          </CardHeader>
          <CardContent className="p-4">
            <div className="h-[280px] w-full flex items-center justify-center">
              <ResponsiveContainer width="100%" height="100%">
                <RechartsPieChart>
                  <defs>
                    {salesDistributionData.map((entry, index) => (<filter key={`shadow-${index}`} id={`shadow-${index}`}>
                        <feDropShadow dx="0" dy="2" stdDeviation="3" floodOpacity="0.2"/>
                      </filter>))}
                  </defs>
                  <Pie data={salesDistributionData} cx="50%" cy="45%" innerRadius={40} outerRadius={85} fill="#8884d8" dataKey="value" stroke="#ffffff" strokeWidth={2}>
                    {salesDistributionData.map((entry, index) => (<Cell key={`cell-${index}`} fill={entry.color} filter={`url(#shadow-${index})`}/>))}
                  </Pie>
                  <Tooltip contentStyle={{
            backgroundColor: '#ffffff',
            border: '1px solid #e2e8f0',
            ...
            padding: '10px'
                  ...
                  <Legend verticalAlign="bottom" height={50} iconType="circle" wrapperStyle={{ fontSize: '11px', fontWeight: 500, paddingTop: '10px' }}/>
                </RechartsPieChart>
              </ResponsiveContainer>
            </div>
          </CardContent>
        ...
        <Card className="bg-white border rounded-lg shadow-sm">
          <CardHeader className="flex flex-row items-center justify-between pb-3 border-b">
            ...
            <Button variant="ghost" size="sm" onClick={() => setExpandedChart('category-performance')} className="h-8 w-8 p-0 hover:bg-gray-100">
              <Expand className="h-4 w-4"/>
            </Button>
          </CardHeader>
          <CardContent className="p-4">
            <div className="h-[280px] w-full flex items-center justify-center">
              <ResponsiveContainer width="100%" height="100%">
                <RechartsPieChart>
                  <defs>
                    {categoryPerformanceData.map((entry, index) => (<filter key={`perf-shadow-${index}`} id={`perf-shadow-${index}`}>
                        <feDropShadow dx="0" dy="2" stdDeviation="3" floodOpacity="0.2"/>
                      </filter>))}
                  </defs>
                  <Pie data={categoryPerformanceData} cx="50%" cy="45%" innerRadius={35} outerRadius={80} fill="#8884d8" dataKey="value" stroke="#ffffff" strokeWidth={2}>
                    {categoryPerformanceData.map((entry, index) => (<Cell key={`cell-${index}`} fill={entry.color} filter={`url(#perf-shadow-${index})`}/>))}
                  </Pie>
                  <Tooltip contentStyle={{
            backgroundColor: '#ffffff',
            border: '1px solid #e2e8f0',
      ...
      <Dialog open={showInventoryBreakdown} onOpenChange={setShowInventoryBreakdown}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden p-0">
          <div className="bg-gradient-to-r from-green-600 to-blue-700 text-white p-6">
            <DialogHeader>
              <DialogTitle className="flex items-center justify-between">
                ...
                <Button variant="ghost" size="sm" onClick={() => setShowInventoryBreakdown(false)} className="h-10 w-10 p-0 text-white hover:bg-white/20">
                  <X className="h-5 w-5"/>
                </Button>
              </DialogTitle>
            </DialogHeader>
          ...
          <div className="overflow-y-auto max-h-[calc(90vh-140px)] p-6">
            {isWarehouseLoading ? (<div className="flex flex-col items-center justify-center py-12">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {warehouseBreakdown?.map((warehouse, index) => (<div key={warehouse.location || index} className="bg-white border rounded-lg shadow-sm p-4 hover:shadow-md transition-shadow cursor-pointer" onClick={() => window.location.href = `/inventory?warehouse=${warehouse.warehouse_id}`}>
      ...
      <Dialog open={!!selectedProductId} onOpenChange={() => setSelectedProductId(null)}>
        <DialogContent className="max-w-6xl max-h-[95vh] overflow-hidden p-0">
          <div className="bg-gradient-to-r from-blue-600 to-purple-700 text-white p-6">
            <DialogHeader>
              <DialogTitle className="flex items-center justify-between">
                ...
                <Button variant="ghost" size="sm" onClick={() => setSelectedProductId(null)} className="h-10 w-10 p-0 text-white hover:bg-white/20">
                  <X className="h-5 w-5"/>
                </Button>
              </DialogTitle>
          ...
          <div className="overflow-y-auto max-h-[calc(95vh-140px)] p-6">
            {isLoadingDetails ? (<div className="flex flex-col items-center justify-center py-12">
                <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600 mb-4"></div>
              ...
              </div>) : productDetails ? (<div className="space-y-8">
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="lg:text-right">
                      <div className="space-y-2">
                        ...
                        <span className={`inline-flex px-4 py-2 text-sm font-semibold rounded-full ${productDetails.status === 'active' ? 'bg-green-100 text-green-800 border border-green-200' :
                ...
                <div className="flex space-x-4 justify-center pt-6">
                  <Button onClick={() => window.location.href = `/inventory`} className="flex items-center space-x-2">
                    <Package className="h-4 w-4"/>
                    <span>View All Inventory</span>
                  </Button>
                  <Button variant="outline" onClick={() => window.location.href = `/inventory?search=${productDetails.name}`} className="flex items-center space-x-2">
                    <span>Search Similar Products</span>
                  </Button>
                </div>
              </div>) : (<div className="text-center py-8">
                <p className="text-gray-500">Product details not available</p>
                <Button onClick={() => window.location.href = `/inventory`} className="mt-4">
                  Browse All Products
                </Button>
              </div>)}
          </div>
        ...
        <DialogContent className="max-w-4xl h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center justify-between">
              ...
              <Button variant="ghost" size="sm" onClick={() => setExpandedChart(null)} className="h-8 w-8 p-0">
                <X className="h-4 w-4"/>
              </Button>
            </DialogTitle>
          </DialogHeader>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-full">
            <div className="lg:col-span-2 h-full">
              <ResponsiveContainer width="100%" height="90%">
                <RechartsLineChart data={salesData || []} margin={{ top: 20, right: 30, left: 20, bottom: 50 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0"/>
                  <XAxis dataKey="name" axisLine={true} tickLine={true} fontSize={12} tick={{ fill: '#6b7280' }}/>
                  <YAxis axisLine={true} tickLine={true} fontSize={12} tick={{ fill: '#6b7280' }}/>
                  <Tooltip contentStyle={{
            backgroundColor: '#ffffff',
            border: '1px solid #e2e8f0',
            borderRadius: '8px',
            ...
            boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1)'
                  ...
                  <Legend />
                  <Line type="monotone" dataKey="sales" stroke="#1D3E78" strokeWidth={3} dot={{ fill: '#1D3E78', strokeWidth: 2, r: 5 }} activeDot={{ r: 7, fill: '#1D3E78' }} name="Sales (EGP)"/>
                </RechartsLineChart>
              </ResponsiveContainer>
            </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className={`text-xl font-bold ${analyticsData?.growthMoMPct >= 0 ? 'text-green-700' : 'text-red-700'}`}>
                        {isAnalyticsLoading ? '...' : (analyticsData?.growthMoMPct !== null && analyticsData?.growthMoMPct !== undefined
                ...
                <div className="p-4 bg-gradient-to-r from-orange-50 to-orange-100 rounded-lg border">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-lg font-semibold">
                        {isAnalyticsLoading ? '...' : (analyticsData?.targetAttainmentPct
                ...
                <div className="p-4 bg-gradient-to-r from-purple-50 to-purple-100 rounded-lg border">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-lg font-semibold">
                        {isAnalyticsLoading ? '...' : (analyticsData?.avgOrderValue
        ...
        <DialogContent className="max-w-4xl h-[80vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 h-full">
            <div className="h-full">
              <ResponsiveContainer width="100%" height="90%">
                <RechartsPieChart>
                  <Pie data={salesDistributionData} cx="50%" cy="50%" outerRadius={120} fill="#8884d8" dataKey="value" label={({ name, percent }) => `${name}: ${(percent * 100).toFixed(1)}%`} labelLine={true} fontSize={12}>
                    {salesDistributionData.map((entry, index) => (<Cell key={`cell-${index}`} fill={entry.color}/>))}
                  </Pie>
                  <Tooltip contentStyle={{
            backgroundColor: '#ffffff',
            border: '1px solid #e2e8f0',
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                {salesDistributionData.map((item, index) => (<div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                    <div className="flex items-center space-x-3">
        ...
        <DialogContent className="max-w-4xl h-[80vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 h-full">
            <div className="h-full">
              <ResponsiveContainer width="100%" height="90%">
                <RechartsPieChart>
                  <Pie data={categoryPerformanceData} cx="50%" cy="50%" outerRadius={120} fill="#8884d8" dataKey="value" label={({ name, percent }) => `${name}: ${(percent * 100).toFixed(1)}%`} labelLine={true} fontSize={12}>
                    {categoryPerformanceData.map((entry, index) => (<Cell key={`cell-${index}`} fill={entry.color}/>))}
                  </Pie>
                  <Tooltip contentStyle={{
            backgroundColor: '#ffffff',
            border: '1px solid #e2e8f0',
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                {categoryPerformanceData.map((item, index) => (<div key={index} className="flex items-center justify-between p-4 bg-gradient-to-r from-gray-50 to-gray-100 rounded-lg border">
                    <div className="flex items-center space-x-3">
                      <div className="w-5 h-5 rounded-full border-2 border-white shadow-sm" style={{ backgroundColor: item.color }}></div>
                      <div>
