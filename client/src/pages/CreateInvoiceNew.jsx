...
import { Plus, Trash, Loader2, FileText, User, Calendar, Package, } from "lucide-react";
// Form schema
...
export default function CreateInvoice() {
    ...
    const handleQuotationSelection = (quotation) => {
        ...
        if (quotation.items && quotation.items.length > 0) {
            form.setValue("items", quotation.items.map((item) => ({
    ...
    const handleOrderSelection = (order) => {
        ...
        if (order.items && order.items.length > 0) {
            form.setValue("items", order.items.map((item) => ({
                productId: item.productId || 0,
                productName: item.productName || item.name,
                quantity: item.quantity || 1,
                unitPrice: item.unitPrice || item.price || 0,
    ...
    return (<div className="container mx-auto p-6 space-y-6">
      <div className="flex justify-between items-center">
        ...
        <div className="flex space-x-2">
          <Button variant="outline" onClick={() => setShowQuotationSelector(true)}>
            <FileText className="mr-2 h-4 w-4"/>
            Select from Quotations
          </Button>
          <Button variant="outline" onClick={() => setShowOrderSelector(true)}>
          ...
          <CardContent className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <Input id="customerName" {...form.register("customer.name")} placeholder="Enter customer name"/>
              ...
              <div>
                ...
                <Input id="customerEmail" type="email" {...form.register("customer.email")} placeholder="Enter customer email"/>
              ...
              <div>
                ...
                <Input id="customerCompany" {...form.register("customer.company")} placeholder="Enter company name"/>
              </div>
              <div>
                <Label htmlFor="customerPhone">Phone</Label>
                <Input id="customerPhone" {...form.register("customer.phone")} placeholder="Enter phone number"/>
              </div>
            </div>

            <div>
              <Label htmlFor="customerAddress">Address</Label>
              <Textarea id="customerAddress" {...form.register("customer.address")} placeholder="Enter customer address" rows={3}/>
            </div>
              ...
              <Table>
                ...
                <TableBody>
                  {fields.map((field, index) => (<TableRow key={field.id}>
                      <TableCell>
                        <Select value={form.watch(`items.${index}.productId`)?.toString() || ""} onValueChange={(value) => handleProductSelect(index, value)}>
                          <SelectTrigger>
                      ...
                      <TableCell>
                        <Input type="number" min="1" value={form.watch(`items.${index}.quantity`) || 1} onChange={(e) => handleItemChange(index, "quantity", parseInt(e.target.value) || 1)} className="w-20"/>
                      </TableCell>
                      <TableCell>
                        <Input type="number" min="0" step="0.01" value={form.watch(`items.${index}.unitPrice`) || 0} onChange={(e) => handleItemChange(index, "unitPrice", parseFloat(e.target.value) || 0)} className="w-24"/>
                      </TableCell>
                      <TableCell>
                ...
                <div className="space-y-2 bg-muted/50 rounded-lg p-4">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <div className="flex space-x-2">
                      ...
                      {form.watch("discountType") !== "none" && (<Input type="number" min="0" className="w-20" {...form.register("discountValue", { valueAsNumber: true })}/>)}
                    </div>
                  </div>

                  <div className="flex justify-between items-center">
                    <span>Tax Rate (%)</span>
                    <Input type="number" min="0" max="100" step="0.01" className="w-20" {...form.register("taxRate", { valueAsNumber: true })}/>
                  </div>
            ...
            <div>
              ...
              <Textarea id="notes" {...form.register("notes")} placeholder="Add any additional notes..." rows={3}/>
            </div>
          </CardContent>
