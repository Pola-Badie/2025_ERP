...
const Approvals = () => {
    ...
    const filteredExpenses = pendingExpenses?.filter(expense => expense.description.toLowerCase().includes(searchTerm.toLowerCase()) ||
      ...
      <Card>
        <CardContent className="p-0">
          {isLoading ? (<div className="p-8 flex justify-center">
            ...
            </div>) : !filteredExpenses?.length ? (<div className="p-8 text-center">
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {filteredExpenses?.map((expense) => (<tr key={expense.id} className="border-b border-slate-100 hover:bg-slate-50">
                      <td className="px-4 py-3">{formatDate(expense.date)}</td>
                      <td className="px-4 py-3 max-w-xs truncate">
                        <div className="font-medium">{expense.description}</div>
                        {expense.notes && (<div className="text-xs text-slate-500 mt-1 truncate">{expense.notes}</div>)}
                      </td>
                      ...
                      <td className="px-4 py-3">
                        {expense.receiptPath ? (<Button variant="ghost" size="sm" onClick={() => viewReceipt(expense)}>
                            <Eye className="h-4 w-4 mr-1"/>
                            View
                          </Button>) : (<span className="text-xs text-slate-500">No receipt</span>)}
                      </td>
                      <td className="px-4 py-3 text-right">
                        <div className="flex justify-end space-x-2">
                          <Button variant="outline" size="sm" className="text-red-600 border-red-200 hover:bg-red-50 hover:text-red-700" onClick={() => handleRejectClick(expense)}>
                            <XCircle className="h-4 w-4 mr-1"/>
                            Reject
                          </Button>
                          <Button size="sm" className="text-white" onClick={() => handleApprove(expense)}>
      ...
      <Dialog open={showRejectDialog} onOpenChange={setShowRejectDialog}>
        <DialogContent>
          ...
          <div className="py-4">
            ...
            {selectedExpense && (<div className="bg-slate-50 p-3 rounded-md mb-4">
            ...
            <Textarea placeholder="Enter rejection reason..." value={rejectionReason} onChange={(e) => setRejectionReason(e.target.value)} className="w-full" rows={4}/>
          </div>
          <DialogFooter>
            ...
            <Button variant="default" className="bg-red-600 hover:bg-red-700" onClick={handleConfirmReject} disabled={!rejectionReason.trim()}>
              Confirm Rejection
            </Button>
          </DialogFooter>
        </DialogContent>
