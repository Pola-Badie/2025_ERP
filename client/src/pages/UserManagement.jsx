...
import { Trash2, UserPlus, Shield, ShieldCheck, ShieldX, Users, Settings } from 'lucide-react';
...
const UserManagement = () => {
    ...
    const updateUserStatusMutation = useMutation({
        mutationFn: ({ userId, status }) => apiRequest('PATCH', `/api/users/${userId}/status`, { status }),
        onSuccess: () => {
            toast({
                title: t('success'),
    ...
    const updateUserRoleMutation = useMutation({
        mutationFn: ({ userId, role }) => apiRequest('PATCH', `/api/users/${userId}/role`, { role }),
        onSuccess: () => {
            toast({
                title: t('success'),
    ...
    const updatePermissionMutation = useMutation({
        mutationFn: ({ userId, moduleName, accessGranted }) => apiRequest('POST', `/api/permissions/users/${userId}/modules/${moduleName}`, { accessGranted }),
        onSuccess: (data) => {
            if (data.success) {
                toast({
                    title: t('success'),
    ...
    const deletePermissionMutation = useMutation({
        mutationFn: (permissionId) => apiRequest('DELETE', `/api/permissions/${permissionId}`),
        onSuccess: () => {
            toast({
                title: t('success'),
    ...
    const handlePermissionToggle = (moduleName, accessGranted) => {
        if (!selectedUser)
            return;
    ...
    const hasPermission = (moduleName) => {
        return userPermissions.some((p) => p.moduleName === moduleName && p.accessGranted);
    };
    const getPermissionCount = (userId) => {
        const userPerms = userPermissions.filter((p) => p.userId === userId && p.accessGranted);
    ...
    return (<div className="space-y-6">
          ...
          <DialogContent className="max-w-md">
            ...
            <form onSubmit={form.handleSubmit(handleCreateUser)} className="space-y-4">
              <div>
                ...
                <Input id="username" {...form.register('username')} placeholder={t('enterUsername')}/>
                {form.formState.errors.username && (<p className="text-red-600 text-sm mt-1">{form.formState.errors.username.message}</p>)}
              </div>

              <div>
                <Label htmlFor="name">{t('fullName')}</Label>
                <Input id="name" {...form.register('name')} placeholder={t('enterFullName')}/>
                {form.formState.errors.name && (<p className="text-red-600 text-sm mt-1">{form.formState.errors.name.message}</p>)}
              </div>

              <div>
                <Label htmlFor="email">{t('email')}</Label>
                <Input id="email" type="email" {...form.register('email')} placeholder={t('enterEmail')}/>
                {form.formState.errors.email && (<p className="text-red-600 text-sm mt-1">{form.formState.errors.email.message}</p>)}
              </div>

              <div>
                <Label htmlFor="password">{t('password')}</Label>
                <Input id="password" type="password" {...form.register('password')} placeholder={t('enterPassword')}/>
              ...
              <div>
                ...
                {form.formState.errors.role && (<p className="text-red-600 text-sm mt-1">{form.formState.errors.role.message}</p>)}
              </div>
      ...
      <Card>
        ...
        <CardContent>
          {usersLoading ? (<div className="flex justify-center p-8">
              ...
              <TableBody>
                {users.map((user) => (<TableRow key={user.id}>
                    ...
                    setShowPermissionsDialog(true);
                        ...
                        {user.id !== currentUser?.id && (<Button variant="outline" size="sm" onClick={() => handleDeleteUser(user.id)}>
        ...
        <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
          ...
          {selectedUser && (<div className="space-y-6">
              ...
              <div className="flex space-x-4 p-4 bg-gray-50 rounded-lg">
                <Button variant="outline" onClick={handleGrantAllPermissions} disabled={updatePermissionMutation.isPending}>
                  <ShieldCheck className="w-4 h-4 mr-2"/>
                  {t('grantAllPermissions')}
                </Button>
                <Button variant="outline" onClick={handleRevokeAllPermissions} disabled={updatePermissionMutation.isPending}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {AVAILABLE_MODULES.map(module => (<Card key={module.key} className="p-4">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center space-x-2">
                        <Switch checked={selectedUser.role === 'admin' || hasPermission(module.key)} disabled={selectedUser.role === 'admin' || updatePermissionMutation.isPending} onCheckedChange={(checked) => handlePermissionToggle(module.key, checked)}/>
                        {selectedUser.role === 'admin' ? (<Badge className="bg-green-100 text-green-800">
                            {t('adminAccess')}
                          </Badge>) : hasPermission(module.key) ? (<Badge className="bg-green-100 text-green-800">
                            {t('granted')}
                          </Badge>) : (<Badge className="bg-red-100 text-red-800">
                            {t('denied')}
                          </Badge>)}
                      </div>
                    </div>
