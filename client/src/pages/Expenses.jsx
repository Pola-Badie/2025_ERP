...
import { Plus, Download, Search, MoreHorizontal, AlertCircle, Trash, ChevronLeft, ChevronRight, FileText, Receipt, DollarSign, BarChart4, Paperclip, CheckCircle, Landmark, Image } from 'lucide-react';
const Expenses = () => {
    ...
    const getStatusBadge = (status) => {
        ...
        return (<Badge variant={statusVariants[status]}>
    ...
    return (<div>
      ...
      <Card>
        <CardContent className="p-0">
          {isLoadingExpenses ? (<div className="p-8 flex justify-center">
              <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-primary"></div>
            </div>) : sortedExpenses.length === 0 ? (<div className="p-8 text-center">
              ...
              {!(searchTerm || statusFilter !== 'all' || categoryFilter !== 'all') && (<Button onClick={() => setIsExpenseFormOpen(true)}>
                  <Plus className="h-4 w-4 mr-2"/>
                  Add Expense
                </Button>)}
              ...
              <table className="w-full">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {paginatedExpenses.map((expense) => (<tr key={expense.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-sm text-gray-900">
                        <div>
                          ...
                          {expense.notes && (<div className="text-xs text-gray-500 mt-1">
                              {expense.notes}
                            </div>)}
                        </div>
                      </td>
          ...
          {/* Pagination Controls */}
          {totalPages > 1 && (<div className="flex justify-center items-center gap-4 mt-6 pt-4 border-t">
              <Button variant="outline" size="sm" onClick={() => handlePageChange(currentPage - 1)} disabled={currentPage === 1} className="flex items-center gap-2">
              ...
              <Button variant="outline" size="sm" onClick={() => handlePageChange(currentPage + 1)} disabled={currentPage === totalPages} className="flex items-center gap-2">
                Next
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="flex items-center space-x-2">
              ...
              <Button size="sm" onClick={handleAddCategory} disabled={createCategoryMutation.isPending}>
                {createCategoryMutation.isPending ? (<div className="animate-spin h-4 w-4 border-2 border-current border-t-transparent rounded-full mr-1"/>) : (<Plus className="h-4 w-4 mr-1"/>)}
                Add
              </Button>
            </div>
            ...
            <div className="border rounded-md divide-y">
              {categories?.map((category) => (<div key={category.id} className="p-3 flex items-center justify-between">
                  <div className="flex items-center flex-1">
                    {editingCategory?.id === category.id ? (<Input value={editingCategory.name} onChange={(e) => setEditingCategory({
                    id: editingCategory.id,
                    name: e.target.value
                })} className="w-full" autoFocus onKeyDown={(e) => e.key === 'Enter' && handleUpdateCategory()}/>) : (<span>{category.name}</span>)}
                  </div>
                  <div className="flex items-center space-x-2">
                    {editingCategory?.id === category.id ? (<>
                        <Button variant="ghost" size="sm" onClick={handleUpdateCategory} disabled={updateCategoryMutation.isPending}>
                          {updateCategoryMutation.isPending ? (<div className="animate-spin h-4 w-4 border-2 border-current border-t-transparent rounded-full mr-1"/>) : ('Save')}
                        </Button>
                        <Button variant="ghost" size="sm" onClick={() => setEditingCategory(null)}>
                          Cancel
                        </Button>
                      </>) : (<>
                        <Button variant="ghost" size="sm" onClick={() => setEditingCategory({ id: category.id, name: category.name })}>
                          Edit
                        </Button>
                        <Button variant="ghost" size="sm" className="text-red-500 hover:text-red-700" onClick={() => handleDeleteCategory(category.id)} disabled={deleteCategoryMutation.isPending}>
                          {deleteCategoryMutation.isPending && deleteCategoryMutation.variables === category.id ? (<div className="animate-spin h-4 w-4 border-2 border-current border-t-transparent rounded-full"/>) : (<Trash className="h-4 w-4"/>)}
                        </Button>
                      </>)}
                  </div>
                </div>))}
              {categories?.length === 0 && (<div className="p-8 text-center text-sm text-muted-foreground">
