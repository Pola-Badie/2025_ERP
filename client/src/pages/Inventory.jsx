...
import { Plus, Trash, Trash2, Search, Filter, MoreHorizontal, AlertCircle, Calendar, Pencil, Tag, ChevronLeft, ChevronRight, Database, Package, Archive, DollarSign, Shield, Clock, FileText, ArrowRightLeft, Settings, X, FolderOpen } from 'lucide-react';
...
const Inventory = () => {
    ...
    // CSV Integration
    const { setCSVData, setCSVOptions, clearCSV } = useCSV();
    // Handle CSV import
    const handleImportProducts = async (data, warehouse) => {
    ...
    const filteredProducts = useMemo(() => {
        ...
        const filtered = products.filter(product => {
            ...
            const matchesGrade = gradeFilter.length === 0 || (product.grade &&
    ...
    const getStatusBadge = (status) => {
        ...
        if (status === 'near' || status === 'near-expiry') {
            return (<div className="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold bg-orange-200 text-orange-800">
          NEAR
        </div>);
        }
        ...
        return (<Badge variant={statusVariants[status] || 'default'}>
    ...
    const handleCreateLabel = (product) => {
    ...
    return (<div className={isRTL ? 'rtl' : 'ltr'} dir={isRTL ? 'rtl' : 'ltr'}>
        ...
        <div className="flex items-center space-x-2 mt-4 sm:mt-0">
          {activeTab === 'inventory' && (<>
              ...
              <div className="flex gap-2 mr-2">
                <CSVImport onImport={handleImportProducts} buttonText={t('importCsv')} variant="outline" size="sm" showWarehouseDialog={true} warehouseLocations={warehouses?.map(w => w.name) || []}/>
                <CSVExport data={filteredProducts} filename="inventory-products.csv" buttonText={t('exportCsv')} variant="outline" size="sm"/>
                {selectedProducts.length > 0 && (<Button variant="outline" size="sm" onClick={() => {
          ...
          <div className={`flex items-center ${isRTL ? 'space-x-reverse space-x-2' : 'space-x-2'}`}>
            <Button variant="outline" size="sm" onClick={() => setIsCategoriesDialogOpen(true)} className="border-purple-300 text-purple-600 hover:bg-purple-50">
              <FolderOpen className={`h-4 w-4 ${isRTL ? 'ml-2' : 'mr-2'}`}/>
              {t('categories')}
            </Button>
            <Button variant="outline" size="icon" onClick={() => setIsInventorySettingsOpen(true)} title={t('inventorySettings')}>
      ...
      {/* Warehouse Selector */}
      {activeTab === 'inventory' && (<Card className="mb-6">
          <CardContent className="p-4">
            <div className="flex flex-col space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-lg font-medium">{t('warehouseSelector')}</h3>
                <div className="flex items-center space-x-2">
                  <Button variant="outline" size="sm" onClick={() => setIsWarehouseDialogOpen(true)}>
              ...
              <div className="flex items-center space-x-2 overflow-x-auto pb-2">
                <Button variant={selectedWarehouse === 0 ? "default" : "outline"} className={`whitespace-nowrap flex items-center gap-1.5 ${selectedWarehouse === 0
                ? "bg-green-600 text-white hover:bg-green-700"
                : "bg-green-50 text-green-700 hover:bg-green-100 border-green-300"}`} onClick={() => setSelectedWarehouse(0)}>
                  <Database className="h-4 w-4"/>
                  {t('allStock')}
                </Button>
                {warehouses.map((warehouse) => (<Button key={warehouse.id} variant={selectedWarehouse === warehouse.id ? "default" : "outline"} className="whitespace-nowrap" onClick={() => setSelectedWarehouse(warehouse.id)}>
                    {warehouse.name}
                  </Button>))}
                <Button variant="ghost" size="icon" onClick={() => setIsWarehouseDialogOpen(true)}>
                  <Plus className="h-5 w-5"/>
                </Button>
              </div>
              
              <div className="text-sm text-muted-foreground">
                {selectedWarehouse === 0 ? (<span>{t('viewingInventory')} <span className="font-medium">{t('allWarehouses')}</span></span>) : (<>
      ...
      <div className="w-full">
          ...
          <Card className="mb-6">
            <CardContent className="p-4">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="relative">
                  ...
                  <Input placeholder={t('searchInventory')} className="pl-9" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}/>
                </div>
                ...
                <Select value={categoryFilter} onValueChange={setCategoryFilter} modal={false}>
                  <SelectTrigger>
                    <Filter className="h-4 w-4 mr-2"/>
                ...
                <DropdownMenu modal={false}>
                  <DropdownMenuTrigger asChild>
                    <Button variant="outline" className={`justify-between h-10 ${gradeFilter.length > 0 ? 'text-blue-600 border-blue-200' : ''}`}>
            ...
            : gradeFilter.length === 1
                ...
                : `${gradeFilter.length} Grades Selected`}
                      ...
                      {gradeFilter.length > 0 && (<Badge variant="secondary" className="ml-2 bg-blue-100 text-blue-600">
                          {gradeFilter.length}
                        </Badge>)}
                    </Button>
                  </DropdownMenuTrigger>
                  <DropdownMenuContent align="end" className="w-56 max-h-80 overflow-auto z-50" side="bottom" sideOffset={5} avoidCollisions={true}>
                    <div className="p-2">
                      <div className="flex items-center justify-between mb-2">
                        <span className="font-medium text-sm">Grade Filter</span>
                        {gradeFilter.length > 0 && (<Button variant="ghost" size="sm" onClick={() => setGradeFilter([])} className="h-6 px-2 text-xs">
                            Clear
                          </Button>)}
                      </div>
        ...
        ].map((grade) => (<div key={grade.value} className="flex items-center space-x-2">
                            <Checkbox id={`grade-${grade.value}`} checked={gradeFilter.includes(grade.value)} onCheckedChange={(checked) => {
                if (checked) {
                    ...
                    setGradeFilter(prev => prev.filter(g => g !== grade.value));
                            ...
                            <label htmlFor={`grade-${grade.value}`} className="flex items-center cursor-pointer text-sm w-full">
          ...
          <Card className={isRTL ? 'rtl' : 'ltr'}>
            <CardContent className="p-0">
              {isLoadingProducts ? (<div className="p-8 flex justify-center">
                ...
                : t('addFirstProduct')}
                  ...
                  {!(searchTerm || statusFilter !== 'all' || categoryFilter !== 'all' || gradeFilter.length > 0) && (<Button onClick={() => {
                    setProductToEdit(null);
                    setIsProductFormOpen(true);
                }}>
                      <Plus className={`h-4 w-4 ${isRTL ? 'ml-2' : 'mr-2'}`}/>
                      ...
                      <tr className="border-b border-slate-200 bg-slate-50">
                        <th className={`px-4 py-3 ${isRTL ? 'text-right' : 'text-left'} font-medium text-slate-500`}>
                          <div className={`flex items-center ${isRTL ? 'justify-end' : 'justify-start'}`}>
                            <Checkbox id="select-all" onCheckedChange={(checked) => {
                if (checked) {
                ...
                return (<tr key={product.id} id={`product-${product.id}`} className={getRowClassName()}>
                        ...
                        if (checked) {
                            ...
                            <td className={`px-4 py-3 ${isRTL ? 'text-right' : 'text-left'}`}>
                              <div>
                                <div className="font-medium flex items-center gap-2">
                                  {product.name}
                                  {highlightedProductId === product.id && (<span className="text-red-600 animate-bounce">📍</span>)}
                                </div>
                            ...
                            <td className={`px-4 py-3 ${isRTL ? 'text-right' : 'text-left'}`}>
                              {product.productType ? (<span className={`px-2 py-1 rounded-full text-xs font-medium whitespace-nowrap ${product.productType === 'raw'
                            ...
                            <td className={`px-4 py-3 ${isRTL ? 'text-right' : 'text-left'}`}>
                              {product.grade ? (<div className="flex flex-wrap gap-1">
                                  {product.grade.split(',').map((grade) => (<span key={grade} className={`px-1.5 py-0.5 rounded text-xs font-medium border ${grade.trim() === 'P'
                            ...
                            <td className={`px-4 py-3 ${isRTL ? 'text-right' : 'text-left'}`}>
                              <div className="flex items-center gap-2">
                                ...
                                {highlightedProductId === product.id && (product.quantity === 0 || product.quantity <= (product.lowStockThreshold || 10)) && (<span className="text-red-600 text-xs animate-pulse">⚠️</span>)}
                              </div>
                            </td>
                            <td className={`px-4 py-3 text-slate-600 ${isRTL ? 'text-right' : 'text-left'}`}>
                              <div className="flex flex-col">
                                ...
                                {selectedWarehouse !== 0 && (<span className="text-xs text-slate-500">
                                    {warehouses.find(w => w.id === selectedWarehouse)?.location}
                                  </span>)}
                              </div>
                            </td>
                            ...
                            <td className={`px-4 py-3 font-medium ${isRTL ? 'text-right' : 'text-left'}`}>
                              <FieldProtectedContent module="inventory" entityType="product" fieldName="sellingPrice" fallback={<span className="text-muted-foreground">•••</span>}>
                                {formatCurrency(product.sellingPrice)}
                              </FieldProtectedContent>
                            </td>
                            <td className={`px-4 py-3 ${isRTL ? 'text-right' : 'text-left'}`}>
                              {product.expiryDate ? (<div className={`flex items-center ${isRTL ? 'justify-end' : 'justify-start'}`}>
                                  <span className={expiryStatus?.status === 'expired' ? 'text-red-500' :
                            expiryStatus?.status === 'near-expiry' ? 'text-orange-700' : ''}>
                                    {formatDate(product.expiryDate)}
                                  </span>
                                  {expiryStatus && (<span className={`${isRTL ? 'mr-2' : 'ml-2'} text-xs ${expiryStatus.status === 'expired' ? 'text-red-500' :
                                expiryStatus.status === 'near-expiry' ? 'text-orange-700' : ''}`}>
                                      {expiryStatus.status === 'expired' ?
                                `(Expired ${expiryStatus.days} days ago)` :
                                `(Expires in ${expiryStatus.days} days)`}
                                    </span>)}
                                </div>) : (<span className="text-slate-400">N/A</span>)}
                            </td>
              ...
              {/* Pagination Controls */}
              {totalPages > 1 && (<div className={`flex items-center justify-center mt-6 p-4 bg-white rounded-lg border shadow-sm ${isRTL ? 'rtl' : 'ltr'}`} dir={isRTL ? 'rtl' : 'ltr'}>
                  <div className="flex items-center gap-2">
                    <Button variant="outline" size="sm" onClick={() => setCurrentPage(Math.max(1, currentPage - 1))} disabled={currentPage === 1} className="flex items-center gap-1">
                ...
                if (pageNum > totalPages)
                    return null;
                return (<Button key={pageNum} variant={pageNum === currentPage ? "default" : "outline"} size="sm" onClick={() => setCurrentPage(pageNum)} className={`w-8 h-8 p-0 ${pageNum === currentPage
                      ...
                      {totalPages > 5 && currentPage < totalPages - 2 && (<>
                          ...
                          <Button variant="outline" size="sm" onClick={() => setCurrentPage(totalPages)} className="w-8 h-8 p-0 hover:bg-blue-50">
                            {totalPages}
                          </Button>
                        </>)}
                    </div>
                    
                    <Button variant="outline" size="sm" onClick={() => setCurrentPage(Math.min(totalPages, currentPage + 1))} disabled={currentPage === totalPages} className="flex items-center gap-1">
                      {t('next')}
        ...
        <DialogContent className={`max-w-4xl max-h-[90vh] overflow-y-auto ${isRTL ? 'rtl' : 'ltr'}`} dir={isRTL ? 'rtl' : 'ltr'}>
          ...
          <div className="space-y-6">
            ...
            <div className="border rounded-lg">
              {isLoadingCategories ? (<div className="p-8 flex justify-center">
                  <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-primary"></div>
                  ...
                  <table className={`w-full text-sm ${isRTL ? 'text-right' : 'text-left'}`} dir={isRTL ? 'rtl' : 'ltr'}>
                    ...
                    <tbody>
                      {categories?.map((category) => (<tr key={category.id} className="border-b border-slate-100 hover:bg-slate-50">
                          ...
                          <td className={`px-4 py-3 ${isRTL ? 'text-left' : 'text-right'}`}>
                            <div className={`flex ${isRTL ? 'justify-start space-x-reverse' : 'justify-end'} space-x-2`}>
                              <Button variant="ghost" size="sm" className="text-blue-500 hover:text-blue-700 hover:bg-blue-50" onClick={() => openEditDialog(category)}>
                                <Pencil className="h-4 w-4"/>
                              </Button>
                              <Button variant="ghost" size="sm" className="text-red-500 hover:text-red-700 hover:bg-red-50" onClick={() => openDeleteDialog(category)}>
                ...
                <div className="grid gap-4">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="md:col-span-1">
                      <Input placeholder={t('Category Name')} value={categoryName} onChange={(e) => setCategoryName(e.target.value)} required className={isRTL ? 'text-right' : 'text-left'}/>
                    </div>
                    <div className="md:col-span-2">
                      <Textarea placeholder={t('Description (optional)')} value={categoryDescription} onChange={(e) => setCategoryDescription(e.target.value)} className={`h-10 py-2 ${isRTL ? 'text-right' : 'text-left'}`}/>
                    </div>
                  </div>
                  <div className={`flex ${isRTL ? 'justify-start' : 'justify-end'}`}>
                    <Button type="submit" disabled={addCategoryMutation.isPending || categoryName.trim() === ''}>
                      <Plus className={`h-4 w-4 ${isRTL ? 'ml-2' : 'mr-2'}`}/>
      ...
      <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <DialogContent className={`max-h-[90vh] overflow-y-auto ${isRTL ? 'rtl' : 'ltr'}`} dir={isRTL ? 'rtl' : 'ltr'}>
          ...
          <DialogFooter className={`${isRTL ? 'space-x-reverse' : ''}`}>
            <Button variant="outline" onClick={() => setDeleteDialogOpen(false)} disabled={deleteCategoryMutation.isPending}>
              {t('Cancel')}
            </Button>
          ...
          <div className="space-y-4 py-4">
            <div className="grid gap-4">
              <div>
                ...
                <Input id="edit-category-name" value={categoryToEdit?.name || ''} onChange={(e) => setCategoryToEdit(prev => prev ? { ...prev, name: e.target.value } : null)} className={`mt-1 ${isRTL ? 'text-right' : 'text-left'}`}/>
              </div>
              <div>
                <label htmlFor="edit-category-description" className="text-sm font-medium">
                  Description (optional)
                </label>
                <Textarea id="edit-category-description" value={categoryToEdit?.description || ''} onChange={(e) => setCategoryToEdit(prev => prev ? { ...prev, description: e.target.value } : null)} className={`mt-1 ${isRTL ? 'text-right' : 'text-left'}`}/>
              </div>
            </div>
          </div>
          <DialogFooter className={`${isRTL ? 'space-x-reverse' : ''}`}>
            <Button variant="outline" onClick={() => setEditDialogOpen(false)} disabled={updateCategoryMutation.isPending}>
              Cancel
            </Button>
            <Button onClick={handleUpdateCategory} disabled={updateCategoryMutation.isPending || !categoryToEdit?.name}>
      ...
      <Dialog open={isProductFormOpen} onOpenChange={(open) => {
        ...
        <DialogContent className={`max-w-4xl max-h-[90vh] overflow-y-auto scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-gray-100 ${isRTL ? 'rtl' : 'ltr'}`} dir={isRTL ? 'rtl' : 'ltr'}>
          ...
          <ProductForm initialData={productToEdit} onSuccess={() => {
      ...
      <Dialog open={isHistoryDialogOpen} onOpenChange={(open) => {
        ...
        <DialogContent className={`max-w-4xl max-h-[90vh] overflow-y-auto scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-gray-100 ${isRTL ? 'rtl' : 'ltr'}`} dir={isRTL ? 'rtl' : 'ltr'}>
          ...
          {selectedProductHistory && (<div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className={`bg-white p-3 rounded border border-blue-200 ${isRTL ? 'text-right' : 'text-left'}`}>
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${selectedProductHistory.status === 'Active' ? 'bg-green-100 text-green-800' :
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className={`text-sm text-green-800 bg-white p-3 rounded border border-green-200 font-bold ${isRTL ? 'text-right' : 'text-left'}`}>
                      <FieldProtectedContent module="inventory" entityType="product" fieldName="costPrice" fallback={<span className="text-muted-foreground">•••</span>}>
              ...
              <div className="bg-orange-50 p-4 rounded-lg border border-orange-200">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="space-y-2">
                    ...
                    <div className={`text-sm font-bold bg-white p-3 rounded border border-orange-200 ${isRTL ? 'text-right' : 'text-left'} ${new Date(selectedProductHistory.expiryDate) < new Date(Date.now() + 90 * 24 * 60 * 60 * 1000)
              ...
              <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
                ...
                {isLoadingActivity ? (<div className="space-y-4">
                    ...
                    const getBorderColor = (type, icon) => {
                        switch (icon || type) {
                            case 'edit':
                            case 'update':
                    ...
                    return (<div key={index} className={`flex items-start border-l-4 ${getBorderColor(activity.type, activity.icon)} pl-4 pb-4 ${isRTL ? 'border-r-4 border-l-0 pr-4 pl-0' : ''}`}>
                          ...
                          <div className="flex-1">
                            ...
                            {activity.reference && (<p className={`text-xs text-blue-600 mt-1 font-mono ${isRTL ? 'text-right' : 'text-left'}`}>
                                Ref: {activity.reference}
                              </p>)}
                            {activity.notes && (<p className={`text-xs text-gray-500 mt-1 italic ${isRTL ? 'text-right' : 'text-left'}`}>
                                Note: {activity.notes}
                              </p>)}
          ...
          <DialogFooter className={`gap-3 pt-6 border-t ${isRTL ? 'space-x-reverse flex-row-reverse' : ''}`}>
            <Button variant="outline" onClick={() => setIsHistoryDialogOpen(false)} className="border-gray-300 hover:bg-gray-50">
              {t('close')}
            </Button>
            ...
            <Button variant="outline" onClick={() => setIsTransferDialogOpen(true)} className="border-green-300 text-green-600 hover:bg-green-50">
        ...
        <DialogContent className={`sm:max-w-[500px] ${isRTL ? 'rtl' : 'ltr'}`} dir={isRTL ? 'rtl' : 'ltr'}>
          ...
          <div className="grid gap-4 py-4">
            <div className={`grid grid-cols-4 items-center gap-4 ${isRTL ? 'text-right' : 'text-left'}`}>
              ...
              <Input id="warehouse-name" value={warehouseToEdit ? warehouseToEdit.name : ''} placeholder={t('warehouseNamePlaceholder')} className={`col-span-3 ${isRTL ? 'text-right' : 'text-left'}`} onChange={(e) => {
            if (warehouseToEdit) {
            ...
            <div className={`grid grid-cols-4 items-center gap-4 ${isRTL ? 'text-right' : 'text-left'}`}>
              ...
              <Input id="warehouse-location" value={warehouseToEdit ? warehouseToEdit.location : ''} placeholder={t('warehouseLocationPlaceholder')} className={`col-span-3 ${isRTL ? 'text-right' : 'text-left'}`} onChange={(e) => {
            if (warehouseToEdit) {
      ...
      <Dialog open={isTransferDialogOpen} onOpenChange={setIsTransferDialogOpen}>
        <DialogContent className={`sm:max-w-[500px] ${isRTL ? 'rtl' : 'ltr'}`} dir={isRTL ? 'rtl' : 'ltr'}>
          ...
          <div className="grid gap-6 py-4">
            {selectedProductHistory && (<div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
            ...
            .map((warehouse) => (<SelectItem key={warehouse.id} value={warehouse.id.toString()}>
              ...
              <div className="space-y-2">
                ...
                <Input id="transfer-quantity" type="number" placeholder={t('enterQuantityToTransfer')} value={transferQuantity} onChange={(e) => setTransferQuantity(e.target.value)} max={selectedProductHistory?.currentStock || 0} min="1" className={isRTL ? 'text-right' : 'text-left'}/>
              ...
              <div className="space-y-2">
                ...
                <Textarea id="transfer-notes" placeholder={t('addTransferNotes')} rows={3} className={isRTL ? 'text-right' : 'text-left'}/>
              </div>
            </div>
          </div>
        ...
        }} disabled={!targetWarehouse || !transferQuantity || Number(transferQuantity) <= 0 || Number(transferQuantity) > (selectedProductHistory?.currentStock || 0)} className="bg-green-600 hover:bg-green-700 text-white">
              <ArrowRightLeft className={`h-4 w-4 ${isRTL ? 'ml-2' : 'mr-2'}`}/>
              {t('confirmTransfer')}
            </Button>
          </DialogFooter>
          ...
          <div className="grid gap-6 py-4">
            ...
            <div>
              ...
              <div className={`flex flex-wrap gap-2 mb-3 ${isRTL ? 'justify-end' : 'justify-start'}`}>
                {inventorySettings.unitsOfMeasure.map((unit, index) => (<Badge key={index} variant="secondary" className="flex items-center gap-1">
                    {unit}
                    <X className="h-3 w-3 cursor-pointer hover:text-destructive" onClick={() => removeOption('unitOfMeasure', unit)}/>
                  </Badge>))}
              </div>
              <div className={`flex gap-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
                <Input placeholder={t('newUnitPlaceholder')} value={newOption.type === 'unitOfMeasure' ? newOption.value : ''} onChange={(e) => setNewOption({ type: 'unitOfMeasure', value: e.target.value })} className={isRTL ? 'text-right' : 'text-left'}/>
            ...
            <div>
              ...
              <div className={`flex flex-wrap gap-2 mb-3 ${isRTL ? 'justify-end' : 'justify-start'}`}>
                {inventorySettings.productTypes.map((type, index) => (<Badge key={index} variant="secondary" className="flex items-center gap-1">
                    {type}
                    <X className="h-3 w-3 cursor-pointer hover:text-destructive" onClick={() => removeOption('productType', type)}/>
                  </Badge>))}
              </div>
              <div className={`flex gap-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
                <Input placeholder={t('newItemTypePlaceholder')} value={newOption.type === 'productType' ? newOption.value : ''} onChange={(e) => setNewOption({ type: 'productType', value: e.target.value })} className={isRTL ? 'text-right' : 'text-left'}/>
            ...
            <div>
              ...
              <div className={`flex flex-wrap gap-2 mb-3 ${isRTL ? 'justify-end' : 'justify-start'}`}>
                {inventorySettings.statusOptions.map((status, index) => (<Badge key={index} variant="secondary" className="flex items-center gap-1">
                    {status}
                    <X className="h-3 w-3 cursor-pointer hover:text-destructive" onClick={() => removeOption('statusOption', status)}/>
                  </Badge>))}
              </div>
              <div className={`flex gap-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
                <Input placeholder={t('newStatusPlaceholder')} value={newOption.type === 'statusOption' ? newOption.value : ''} onChange={(e) => setNewOption({ type: 'statusOption', value: e.target.value })} className={isRTL ? 'text-right' : 'text-left'}/>
            ...
            <div>
              ...
              <div className={`flex flex-wrap gap-2 mb-3 ${isRTL ? 'justify-end' : 'justify-start'}`}>
                {inventorySettings.locationTypes.map((location, index) => (<Badge key={index} variant="secondary" className="flex items-center gap-1">
                    {location}
                    <X className="h-3 w-3 cursor-pointer hover:text-destructive" onClick={() => removeOption('locationType', location)}/>
                  </Badge>))}
              </div>
              <div className={`flex gap-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
                <Input placeholder={t('newLocationPlaceholder')} value={newOption.type === 'locationType' ? newOption.value : ''} onChange={(e) => setNewOption({ type: 'locationType', value: e.target.value })} className={isRTL ? 'text-right' : 'text-left'}/>
            ...
            <div>
              <div className={`flex items-center justify-between mb-3 ${isRTL ? 'flex-row-reverse' : ''}`}>
                ...
                <Button variant="outline" size="sm" onClick={() => setIsCategoriesDialogOpen(true)} className="text-purple-600 border-purple-300 hover:bg-purple-50">
                  <FolderOpen className={`h-4 w-4 ${isRTL ? 'ml-2' : 'mr-2'}`}/>
                  {t('manageCategories')}
                </Button>
              </div>
              <div className={`flex flex-wrap gap-2 mb-3 ${isRTL ? 'justify-end' : 'justify-start'}`}>
                {categories?.slice(0, 5).map((category) => (<Badge key={category.id} variant="secondary" className="flex items-center gap-1">
                    {category.name}
                  </Badge>))}
                {categories && categories.length > 5 && (<Badge variant="outline" className="text-gray-500">
