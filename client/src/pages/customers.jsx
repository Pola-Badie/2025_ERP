...
// Icons
import { Search, Plus, Users, Building, DollarSign, TrendingUp, Phone, Mail, Edit, Trash2, Eye, BarChart3, Loader2, FileText, Database, RefreshCw, Receipt, Package } from 'lucide-react';
// Form handling
...
const CustomerProfileReports = ({ customerId, selectedCustomer }) => {
    ...
    const totalInvoiceValue = customerInvoices.reduce((sum, invoice) => {
        return sum + parseFloat(invoice.grandTotal || invoice.totalAmount || 0);
    }, 0);
    const totalQuotationValue = customerQuotations.reduce((sum, quotation) => {
        return sum + parseFloat(quotation.totalAmount || quotation.grandTotal || 0);
    }, 0);
    const totalItemsInvoiced = customerInvoices.reduce((sum, invoice) => {
    ...
    return (<div className="space-y-6">
      ...
      <Tabs defaultValue="profile" className="w-full">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Card>
              ...
              <CardContent className="space-y-2 text-sm">
                ...
                {selectedCustomer.address && (<div><strong>Address:</strong> {[selectedCustomer.address, selectedCustomer.city, selectedCustomer.state, selectedCustomer.zipCode].filter(Boolean).join(', ')}</div>)}
              </CardContent>
            </Card>
        ...
        <TabsContent value="invoices" className="space-y-4">
          {loadingInvoices ? (<div className="flex items-center justify-center py-8">
              <Loader2 className="h-6 w-6 animate-spin mr-2"/>
              Loading customer invoices from database...
            </div>) : customerInvoices.length > 0 ? (<Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
              ...
              <CardContent>
                <div className="max-h-96 overflow-y-auto rounded-md border">
                  <Table>
                    ...
                    <TableBody>
                      {customerInvoices.map((invoice) => (<TableRow key={invoice.id}>
                          <TableCell className="font-medium">{invoice.invoiceNumber}</TableCell>
                          <TableCell>{new Date(invoice.date || invoice.createdAt).toLocaleDateString()}</TableCell>
                          <TableCell>
                            <div className="space-y-1">
                              {invoice.items && invoice.items.length > 0 ? (invoice.items.map((item, idx) => (<div key={idx} className="text-xs">
                                    {item.productName || 'Product'} × {item.quantity}
                                  </div>))) : (<span className="text-muted-foreground text-xs">No items</span>)}
                            </div>
                          </TableCell>
                          <TableCell>EGP {parseFloat(invoice.grandTotal || invoice.totalAmount || 0).toLocaleString()}</TableCell>
                          <TableCell>
                            <Badge variant={invoice.paymentStatus === 'paid' ? 'default' :
        ...
        <TabsContent value="quotations" className="space-y-4">
          {loadingQuotations ? (<div className="flex items-center justify-center py-8">
              <Loader2 className="h-6 w-6 animate-spin mr-2"/>
              Loading customer quotations from database...
            </div>) : customerQuotations.length > 0 ? (<Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
              ...
              <CardContent>
                <div className="max-h-96 overflow-y-auto rounded-md border">
                  <Table>
                    ...
                    <TableBody>
                      {customerQuotations.map((quotation) => (<TableRow key={quotation.id}>
                          <TableCell className="font-medium">{quotation.quotationNumber}</TableCell>
                          <TableCell>{new Date(quotation.date || quotation.createdAt).toLocaleDateString()}</TableCell>
                          <TableCell>
                            <div className="space-y-1">
                              {quotation.items && quotation.items.length > 0 ? (quotation.items.map((item, idx) => (<div key={idx} className="text-xs">
                          ...
                          <TableCell>
                            <Badge variant={quotation.status === 'approved' ? 'default' :
...
const CustomersPage = () => {
    ...
    const createCustomerMutation = useMutation({
        mutationFn: (data) => apiRequest('POST', '/api/v1/customers', data),
        onSuccess: () => {
    ...
    const updateCustomerMutation = useMutation({
        mutationFn: ({ id, data }) => apiRequest('PATCH', `/api/v1/customers/${id}`, data),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['/api/v1/customers'] });
            queryClient.invalidateQueries({ queryKey: ['/api/v1/reports/customers'] });
    ...
    const deleteCustomerMutation = useMutation({
        mutationFn: (id) => apiRequest('DELETE', `/api/v1/customers/${id}`),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['/api/v1/customers'] });
            queryClient.invalidateQueries({ queryKey: ['/api/v1/reports/customers'] });
    ...
    return (<div className="p-6 space-y-6" data-testid="customers-page">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <CSVImport onImport={handleCSVImport} buttonText="Import CSV" size="sm" variant="outline" dataType="customers" data-testid="import-csv-button"/>
          <CSVExport data={customers} filename="customers_export.csv" buttonText="Export" size="sm" variant="outline" data-testid="export-csv-button"/>
      ...
      {reports && (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <Card data-testid="total-customers-card">
            ...
            <CardContent>
              <div className="text-2xl font-bold" data-testid="total-customers-count">
                {reportsLoading ? (<Loader2 className="h-6 w-6 animate-spin"/>) : (reports.summary.totalCustomers.toLocaleString())}
              </div>
          ...
          <Card data-testid="total-revenue-card">
            ...
            <CardContent>
              <div className="text-2xl font-bold" data-testid="total-revenue-amount">
                {reportsLoading ? (<Loader2 className="h-6 w-6 animate-spin"/>) : (`EGP ${parseFloat(reports.summary.totalRevenue).toLocaleString()}`)}
              </div>
              <p className="text-xs text-muted-foreground">
          ...
          <Card data-testid="repeat-customers-card">
            ...
            <CardContent>
              <div className="text-2xl font-bold" data-testid="repeat-customers-rate">
                {reportsLoading ? (<Loader2 className="h-6 w-6 animate-spin"/>) : (reports.summary.repeatCustomers)}
              </div>
              <p className="text-xs text-muted-foreground">
          ...
          <Card data-testid="average-lifetime-value-card">
            ...
            <CardContent>
              <div className="text-2xl font-bold" data-testid="average-lifetime-value">
                {reportsLoading ? (<Loader2 className="h-6 w-6 animate-spin"/>) : (`EGP ${parseFloat(reports.summary.averageLifetimeValue).toLocaleString()}`)}
              </div>
              <p className="text-xs text-muted-foreground">
                Per customer value
              </p>
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            <div className="flex-1">
              <div className="relative">
                ...
                <Input placeholder="Search customers by name, email, company..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-10" data-testid="search-input"/>
              </div>
            </div>
            <div className="sm:w-48">
              <Select value={sectorFilter} onValueChange={setSectorFilter}>
                ...
                <SelectContent>
                  ...
                  {sectors.map((sector) => (<SelectItem key={sector} value={sector || "unknown"}>
                      {sector || "Unknown"}
                    </SelectItem>))}
                </SelectContent>
              </Select>
      ...
      <Card>
        ...
        <CardContent>
          {customersLoading ? (<div className="text-center py-12">
              <Loader2 className="mx-auto h-8 w-8 animate-spin mb-4"/>
              <p>Loading customer data from database...</p>
            </div>) : customers.length === 0 ? (<div className="text-center py-12">
              ...
              {!searchQuery && !sectorFilter && (<Button onClick={() => setShowAddDialog(true)}>
                  <Plus className="mr-2 h-4 w-4"/>
                  Add First Customer
                </Button>)}
              ...
              <Table>
                ...
                <TableBody>
                  {customers.map((customer) => (<TableRow key={customer.id} data-testid={`customer-row-${customer.id}`}>
                      <TableCell>
                        <div>
                          <div className="font-medium" data-testid={`customer-name-${customer.id}`}>
                            {customer.name}
                          </div>
                          {customer.position && (<div className="text-sm text-muted-foreground">
                              {customer.position}
                            </div>)}
                        </div>
                      </TableCell>
                      ...
                      <TableCell>
                        <div>
                          {customer.company && (<div className="font-medium">{customer.company}</div>)}
                          {customer.sector && (<Badge variant="outline" className="text-xs">
                              {customer.sector}
                            </Badge>)}
                        </div>
                      </TableCell>
                      ...
                      <TableCell>
                        <div className="flex items-center gap-2">
                          <Button variant="ghost" size="sm" onClick={() => handleViewDetails(customer)} data-testid={`view-customer-${customer.id}`}>
                            <Eye className="h-4 w-4"/>
                          </Button>
                          <Button variant="ghost" size="sm" onClick={() => handleEdit(customer)} data-testid={`edit-customer-${customer.id}`}>
                            <Edit className="h-4 w-4"/>
                          </Button>
                          <Button variant="ghost" size="sm" onClick={() => handleDelete(customer)} data-testid={`delete-customer-${customer.id}`}>
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onAddSubmit)} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <FormField control={form.control} name="position" render={({ field }) => (<FormItem>
                ...
                <FormField control={form.control} name="sector" render={({ field }) => (<FormItem>
                ...
                <FormField control={form.control} name="taxNumber" render={({ field }) => (<FormItem>
              ...
              <div className="flex justify-end gap-2">
                <Button type="button" variant="outline" onClick={() => setShowAddDialog(false)} data-testid="cancel-add-customer">
                  Cancel
                </Button>
                <Button type="submit" disabled={createCustomerMutation.isPending} data-testid="save-add-customer">
        ...
        <DialogContent className="max-w-2xl">
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onEditSubmit)} className="space-y-4">
              ...
              <div className="flex justify-end gap-2">
                <Button type="button" variant="outline" onClick={() => setShowEditDialog(false)} data-testid="cancel-edit-customer">
                  Cancel
                </Button>
                <Button type="submit" disabled={updateCustomerMutation.isPending} data-testid="save-edit-customer">
      ...
      <Dialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <DialogContent>
          ...
          <div className="flex justify-end gap-2">
            <Button variant="outline" onClick={() => setShowDeleteDialog(false)} data-testid="cancel-delete-customer">
              Cancel
            </Button>
            <Button variant="destructive" onClick={() => selectedCustomer && deleteCustomerMutation.mutate(selectedCustomer.id)} disabled={deleteCustomerMutation.isPending} data-testid="confirm-delete-customer">
      ...
      <Dialog open={showDetailsDialog} onOpenChange={setShowDetailsDialog}>
        <DialogContent className="max-w-3xl">
          ...
          {selectedCustomer && (<CustomerProfileReports customerId={selectedCustomer.id} selectedCustomer={selectedCustomer}/>)}
        </DialogContent>
      </Dialog>
    </div>);
};
