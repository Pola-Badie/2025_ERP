...
import { Printer, Download, Tag, Loader2, RefreshCw, Plus, FileDown, Trash2, Package, CalendarIcon, } from 'lucide-react';
...
const LabelGenerator = () => {
    ...
    const getHazardImagePath = (hazardType) => {
        switch (hazardType) {
            case 'explosive':
    ...
    const handleSelectProductAdvanced = (product) => {
        setSelectedProduct(product);
      ...
      <Tabs defaultValue="standard">
          ...
          <div className="grid gap-6 md:grid-cols-2">
              ...
              <CardContent className="space-y-4">
                <div>
                  ...
                  <div className="relative mt-1">
                    <Input id="product-search" placeholder="Search by name, SKU, or category" value={productQuery} onChange={(e) => setProductQuery(e.target.value)} className="w-full"/>
                  </div>
                </div>

                <div className="mt-4 border rounded-md h-64 overflow-y-auto">
                  {isLoadingProducts ? (<div className="flex items-center justify-center h-full">
                      <Loader2 className="h-6 w-6 animate-spin text-muted-foreground"/>
                    </div>) : products.length > 0 ? (<ul className="divide-y">
                      {products.map((product) => (<li key={product.id} className={`p-3 cursor-pointer hover:bg-muted transition-colors ${selectedProduct?.id === product.id ? "bg-muted" : ""}`} onClick={() => handleSelectProduct(product)}>
                          <div className="font-medium">{product.name}</div>
                          <div className="text-sm text-muted-foreground">
                            SKU: {product.sku || "N/A"} | Category: {product.category?.name || "N/A"}
                          </div>
                        </li>))}
                    </ul>) : (<div className="flex flex-col items-center justify-center h-full text-center p-4">
            ...
            if (format)
                setSelectedFormat(format);
                        ...
                        <SelectContent>
                          {labelFormats.map((format) => (<SelectItem key={format.id} value={format.id}>
                              {format.name}
                            </SelectItem>))}
                        </SelectContent>
                      </Select>
                    ...
                    <div className="flex items-center space-x-2">
                      <Switch id="multiple-labels" checked={showMultiple} onCheckedChange={setShowMultiple}/>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="border rounded-lg p-4 flex justify-center">
                    {selectedProduct ? (<div ref={labelRef} style={labelStyle} className="flex flex-col justify-between font-sans">
                ...
                currency: 'USD'
                        ...
                        <div className="flex justify-center mt-2">
                          {barcodeURL && (<img src={barcodeURL} alt="Product Barcode" style={{ maxWidth: '100%', height: 'auto' }}/>)}
                        </div>
                ...
                <CardFooter className="flex justify-between">
                  <Button variant="outline" onClick={() => setSelectedProduct(null)} disabled={!selectedProduct}>
                    <RefreshCw className="mr-2 h-4 w-4"/>
                    Reset
                  </Button>
                  <div className="flex space-x-2">
                    <Button variant="outline" onClick={handleDownloadPDF} disabled={!selectedProduct || isGenerating}>
                      <Download className="mr-2 h-4 w-4"/>
                      Download
                    </Button>
                    <Button onClick={() => handlePrint()} disabled={!selectedProduct || isGenerating}>
                      {isGenerating ? (<>
        ...
        <TabsContent value="enhanced">
          <div className="grid gap-6 md:grid-cols-2">
            ...
            if (product)
                handleSelectProductAdvanced(product);
        }}>
                      <SelectTrigger>
                ...
                <div>
                  <Label>Formula</Label>
                  <Input placeholder="e.g. C6H12O6" value={formula} onChange={(e) => setFormula(e.target.value)}/>
                </div>
                
                <div>
                  <Label>M.W.</Label>
                  <Input placeholder="Molecular Weight" value={molecularWeight} onChange={(e) => setMolecularWeight(e.target.value)}/>
                </div>
                  ...
                  <div className="relative w-full">
                    <Popover>
                      ...
                      <PopoverContent className="w-auto p-0" align="start">
                        <Calendar mode="single" selected={manufacturingDate ? new Date(manufacturingDate) : undefined} onSelect={(date) => date && setManufacturingDate(format(date, 'dd/MM/yyyy'))} initialFocus/>
                      </PopoverContent>
                    </Popover>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="relative w-full">
                    <Popover>
                      ...
                      <PopoverContent className="w-auto p-0" align="start">
                        <Calendar mode="single" selected={expiryDate ? new Date(expiryDate) : undefined} onSelect={(date) => date && setExpiryDate(format(date, 'dd/MM/yyyy'))} initialFocus/>
                      </PopoverContent>
                    </Popover>
                  </div>
                </div>
                ...
                <div className="space-y-4 pt-4">
                  ...
                  {labTests.map((test, index) => (<div key={index} className="grid grid-cols-5 gap-2 items-center">
                      <div className="col-span-2">
                        <Select value={test.type} onValueChange={(value) => updateLabTest(index, 'type', value)}>
                          <SelectTrigger>
                      ...
                      <div className="col-span-2">
                        <Input value={test.value} onChange={(e) => updateLabTest(index, 'value', e.target.value)} placeholder="Value"/>
                      </div>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="border rounded-lg p-4 mb-6 min-h-[300px] flex justify-center">
                    {productName ? (<div ref={labelRef} className="bg-white border border-gray-300 shadow-sm" style={{
                width: "100%",
                maxWidth: "210mm",
                minHeight: `${100 + Math.max(0, (labTests.length - 3) * 10)}mm`
                        ...
                        <div className="bg-blue-600 text-white p-1.5 relative">
                          <div className="absolute top-0 left-0" style={{ zIndex: 5 }}>
                            <img src="/logo.png" alt="Morgan Chemicals Logo" className="w-16 h-16 object-cover" style={{ objectPosition: 'left top' }}/>
                          </div>
                          ...
                          <div className="w-1/2 p-3">
                            <table className="w-full text-xs">
                              <tbody>
                                ...
                                {labTests.map((test, index) => (<tr key={index}>
                            ...
                            {selectedSpecification && (<div className="mt-1 text-[10px] font-semibold">
                ...
                justifyContent: 'center'
                                    ...
                                    {Array(25).fill(0).map((_, i) => (<div key={i} style={{
                    width: `${(i % 3 === 0) ? 4 : 3}px`,
                    height: '100%',
                    ...
                    marginRight: '2px'
                          ...
                          {selectedHazard && (<div className="absolute left-[58%] transform -translate-x-1/2 top-1/3 -translate-y-1/2">
                              <img src={getHazardImagePath(selectedHazard)} alt="Hazard symbol" className="w-20 object-contain" style={{ imageRendering: 'crisp-edges' }}/>
                            </div>)}
                        </div>
                        ...
                        <div className="bg-blue-600 text-white text-[8px] leading-tight p-1.5 relative -mt-0.5">
                          ...
                          <div className="absolute top-0 left-0 h-full">
                            <img src="/qrcode.png" alt="QR Code" className="h-full object-contain"/>
                          </div>
                ...
                <CardFooter className="flex justify-between border-t pt-4">
                  <Button type="button" variant="outline" onClick={resetEnhancedForm} disabled={isGenerating}>
                    <RefreshCw className="mr-2 h-4 w-4"/>
                    Reset
                  </Button>
                  <div className="flex gap-2">
                    <Button type="button" variant="outline" onClick={downloadEnhancedPDF} disabled={isGenerating || !productName}>
                      <FileDown className="mr-2 h-4 w-4"/>
                      Download PDF
                    </Button>
                    <Button type="button" onClick={() => handlePrint()} disabled={isGenerating || !productName}>
                      <Printer className="mr-2 h-4 w-4"/>
                      Print
                    </Button>
                  </div>
