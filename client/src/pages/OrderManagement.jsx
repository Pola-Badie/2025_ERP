...
import { Loader2, Save, Eye, FileText, Trash2, Plus, X, Search, FileDown } from 'lucide-react';
...
const OrderManagement = () => {
    ...
    const [refiningSelectedCustomer, setRefiningSelectedCustomer] = useState(null);
    ...
    const [packagingUnitPrice, setPackagingUnitPrice] = useState('');
    ...
    const [equipmentCost, setEquipmentCost] = useState('0.00');
    const [qualityControlCost, setQualityControlCost] = useState('0.00');
    const [storageCost, setStorageCost] = useState('0.00');
    ...
    const [refiningLaborCost, setRefiningLaborCost] = useState('0.00');
    const [refiningEquipmentCost, setRefiningEquipmentCost] = useState('0.00');
    const [refiningQualityControlCost, setRefiningQualityControlCost] = useState('0.00');
    const [refiningStorageCost, setRefiningStorageCost] = useState('0.00');
    const [refiningProcessingCost, setRefiningProcessingCost] = useState('0.00');
    // Refining raw materials states
    const [refiningRawMaterials, setRefiningRawMaterials] = useState([]);
    const [refiningMaterialToAdd, setRefiningMaterialToAdd] = useState(null);
    const [refiningMaterialQuantity, setRefiningMaterialQuantity] = useState('');
    const [refiningMaterialUnitPrice, setRefiningMaterialUnitPrice] = useState('');
    ...
    const filteredCustomers = React.useMemo(() => {
        ...
        return customers.filter((customer) => {
            ...
            const customerOrders = allCustomerOrders.filter((order) => order.customerName === customer.name ||
                order.customerId === customer.id);
            // Get the most recent order
            const recentOrder = customerOrders.sort((a, b) => new Date(b.createdAt || b.orderDate).getTime() - new Date(a.createdAt || a.orderDate).getTime())[0];
            return {
                ...customer,
    ...
    const handleRemoveRefiningMaterial = (index) => {
    ...
    const handleCreateRefiningOrder = async () => {
        try {
            ...
            const rawMaterialsCost = refiningRawMaterials.reduce((sum, material) => sum + (material.quantity * parseFloat(material.unitPrice || '0')), 0);
            const packagingCost = packagingItems.reduce((sum, item) => sum + ((item.quantity || 0) * parseFloat(item.unitPrice || '0')), 0);
    ...
    const exportProductionOrdersByWarehouse = async (warehouseType) => {
        try {
    ...
    const exportRefiningOrdersByWarehouse = async (warehouseType) => {
        try {
            // Filter orders based on warehouse type
    ...
    const formatMaterialsForCSV = (materials) => {
        if (!materials)
            return 'None';
        try {
            ...
            if (!Array.isArray(materialsList))
                return 'None';
            return materialsList.map(m => `${m.name || 'Unknown'} (${m.quantity || 0} ${m.unitOfMeasure || ''})`).join('; ');
        }
        catch (error) {
      ...
      <Tabs value={activeTab} onValueChange={handleTabChange}>
        ...
        <TabsContent value="create" className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2">
              <Card>
                    ...
                    <div>
                      ...
                      <Popover open={customerPopoverOpen} onOpenChange={setCustomerPopoverOpen}>
                        ...
                        <PopoverContent className="w-full p-0">
                          <Command>
                            <CommandInput placeholder={t('searchCustomers')} value={customerSearchTerm} onValueChange={setCustomerSearchTerm}/>
                            <CommandList>
                              <CommandEmpty>{t('noCustomerFound')}</CommandEmpty>
                              <CommandGroup>
                                {filteredCustomers.map((customer) => (<CommandItem key={customer.id} value={customer.name} onSelect={() => {
                ...
                setCustomerPopoverOpen(false);
                      ...
                      {selectedCustomer && (<div className="mt-2 p-3 bg-muted rounded-md">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <Input value={batchNumber} onChange={(e) => setBatchNumber(e.target.value)} placeholder={t('autoGenerated')}/>
                      </div>
                      <div>
                        <Label>{t('finalProduct')}</Label>
                        <Input value={finalProductDescription} onChange={(e) => setFinalProductDescription(e.target.value)} placeholder={t('describeFinalProduct')}/>
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-4 gap-2 mb-3">
                        <Popover open={materialPopoverOpen} onOpenChange={setMaterialPopoverOpen}>
                          <PopoverTrigger asChild>
                            <Button variant="outline" role="combobox" aria-expanded={materialPopoverOpen} className="justify-between">
                          ...
                          <PopoverContent className="w-80 p-0">
                            <Command>
                              <CommandInput placeholder={`${t('searchMaterials')}...`} value={materialSearchTerm} onValueChange={setMaterialSearchTerm}/>
                              <CommandList>
                                <CommandEmpty>{t('noMaterialsFound')}</CommandEmpty>
                                <CommandGroup>
                                  {rawMaterialsData
            ?.filter((material) => (material.name.toLowerCase().includes(materialSearchTerm.toLowerCase()) ||
            ...
            ?.map((material) => (<CommandItem key={material.id} value={material.name} onSelect={() => {
                ...
                setMaterialSearchTerm('');
                        ...
                        <div className="relative">
                          <Input type="number" placeholder={t('quantity')} min="1" value={materialQuantity === '0' ? '' : materialQuantity} onChange={(e) => setMaterialQuantity(e.target.value === '' || e.target.value === '0' ? '' : Math.max(1, parseInt(e.target.value) || 1).toString())}/>
                          {materialToAdd && (<span className="absolute right-3 top-1/2 transform -translate-y-1/2 text-xs text-muted-foreground pointer-events-none">
                              {materialToAdd.unitOfMeasure || 'units'}
                            </span>)}
                        </div>

                        <Input type="number" step="0.01" placeholder={t('unitPrice')} min="0.01" value={materialUnitPrice === '0' || materialUnitPrice === '0.00' ? '' : materialUnitPrice} onChange={(e) => setMaterialUnitPrice(e.target.value === '' || e.target.value === '0' || e.target.value === '0.00' ? '' : e.target.value)}/>
                      ...
                      {/* Materials List */}
                      {rawMaterials.length > 0 && (<div className="space-y-2">
                          {rawMaterials.map((material, index) => (<div key={index} className="flex items-center justify-between p-2 bg-muted rounded">
                              <span className="font-medium">{material.name}</span>
                              <span className="text-sm text-muted-foreground">
                                {material.quantity} {material.unitOfMeasure} × {formatCurrency(material.unitPrice)}
                              </span>
                              <Button variant="ghost" size="sm" onClick={() => handleRemoveMaterial(index)}>
                                <X className="h-4 w-4"/>
                              </Button>
                            </div>))}
                        </div>)}
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-4 gap-2 mb-3">
                        <Popover open={packagingPopoverOpen} onOpenChange={setPackagingPopoverOpen}>
                          <PopoverTrigger asChild>
                            <Button variant="outline" role="combobox" aria-expanded={packagingPopoverOpen} className="justify-between">
                          ...
                          <PopoverContent className="w-80 p-0">
                            <Command>
                              <CommandInput placeholder="Search packaging..." value={packagingSearchTerm} onValueChange={setPackagingSearchTerm}/>
                              <CommandList>
                                <CommandEmpty>No packaging found.</CommandEmpty>
                                <CommandGroup>
                                  {packagingMaterials
            ?.filter((item) => item.name.toLowerCase().includes(packagingSearchTerm.toLowerCase()) ||
            ...
            ?.map((item) => (<CommandItem key={item.id} value={item.name} onSelect={() => {
                ...
                setPackagingSearchTerm('');
                        ...
                        <div className="relative">
                          <Input type="number" placeholder="Qty" min="1" value={packagingQuantity === '0' ? '' : packagingQuantity} onChange={(e) => setPackagingQuantity(e.target.value === '' || e.target.value === '0' ? '' : Math.max(1, parseInt(e.target.value) || 1).toString())}/>
                          {packagingToAdd && (<span className="absolute right-3 top-1/2 transform -translate-y-1/2 text-xs text-muted-foreground pointer-events-none">
                              {packagingToAdd.unitOfMeasure || 'units'}
                            </span>)}
                        </div>

                        <Input type="number" step="0.01" placeholder="Unit Price" min="0.01" value={packagingUnitPrice === '0' || packagingUnitPrice === '0.00' ? '' : packagingUnitPrice} onChange={(e) => setPackagingUnitPrice(e.target.value === '' || e.target.value === '0' || e.target.value === '0.00' ? '' : e.target.value)}/>
                      ...
                      {/* Packaging List */}
                      {packagingItems.length > 0 && (<div className="space-y-2">
                          {packagingItems.map((item, index) => (<div key={index} className="flex items-center justify-between p-2 bg-muted rounded">
                              <span className="font-medium">{item.name}</span>
                              <span className="text-sm text-muted-foreground">
                                {item.quantity} {item.unitOfMeasure} × ${item.unitPrice}
                              </span>
                              <Button variant="ghost" size="sm" onClick={() => handleRemovePackaging(index)}>
                                <X className="h-4 w-4"/>
                              </Button>
                            </div>))}
                        </div>)}
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-2 gap-4 mt-2">
                        <div>
                          ...
                          <Input type="number" step="0.01" value={transportationCost} onChange={(e) => setTransportationCost(e.target.value)} placeholder="0.00"/>
                        </div>
                        <div>
                        ...
                        <div>
                          ...
                          <Input type="number" step="0.01" value={equipmentCost} onChange={(e) => setEquipmentCost(e.target.value)} placeholder="0.00"/>
                        </div>
                        <div>
                          <Label>{t('overheadCost')}</Label>
                          <Input type="number" step="0.01" value={qualityControlCost} onChange={(e) => setQualityControlCost(e.target.value)} placeholder="0.00"/>
                        </div>
                        <div>
                          <Label>{t('overheadCost')}</Label>
                          <Input type="number" step="0.01" value={storageCost} onChange={(e) => setStorageCost(e.target.value)} placeholder="0.00"/>
                        </div>
                        <div>
                          <Label>Tax Percentage</Label>
                          <Input type="number" value={taxPercentage} onChange={(e) => setTaxPercentage(parseInt(e.target.value) || 14)} placeholder="14"/>
                        </div>
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <Textarea value={transportationNotes} onChange={(e) => setTransportationNotes(e.target.value)} placeholder="Any special transportation requirements..."/>
                    </div>

                    
    <Button onClick={handleCreateOrder} className="w-full" type="button" disabled={!selectedCustomer || isLoading}>
            ...
            <CardContent className="p-6">
              ...
              <div className="rounded-md border">
                <Table>
                  ...
                  <TableBody>
                    {isLoadingOrders ? (<TableRow>
                        <TableCell colSpan={7} className="text-center py-4">
                          <Loader2 className="h-4 w-4 animate-spin mx-auto"/>
                        </TableCell>
                      </TableRow>) : productionOrders.length === 0 ? (<TableRow>
                        <TableCell colSpan={7} className="text-center py-4 text-muted-foreground">
                          No production orders found
                        </TableCell>
                      </TableRow>) : (productionOrders.map((order) => (<TableRow key={order.id}>
                          <TableCell className="font-medium">{order.batchNumber}</TableCell>
                          <TableCell>{order.customerName}</TableCell>
                          <TableCell>{order.finalProduct || 'N/A'}</TableCell>
                          <TableCell>${order.totalCost}</TableCell>
                          <TableCell>
                            <Badge variant="outline" className={`inline-flex items-center whitespace-nowrap font-medium text-xs px-2 py-1 rounded-full border ${order.status === 'completed' ? 'bg-green-100 text-green-800 border-green-300' :
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <Popover open={refiningCustomerPopoverOpen} onOpenChange={setRefiningCustomerPopoverOpen}>
                        <PopoverTrigger asChild>
                          <Button variant="outline" role="combobox" aria-expanded={refiningCustomerPopoverOpen} className="w-full justify-between">
                        ...
                        <PopoverContent className="w-full p-0">
                          <Command>
                            <CommandInput placeholder="Search customers..." value={refiningCustomerSearchTerm} onValueChange={setRefiningCustomerSearchTerm}/>
                            <CommandList>
            ...
            ?.map((customer) => (<CommandItem key={customer.id} value={customer.name} onSelect={() => {
                ...
                setRefiningCustomerSearchTerm('');
                      ...
                      {refiningSelectedCustomer && (<div className="mt-2 p-3 bg-muted rounded-md">
                    ...
                    <div>
                      ...
                      <Input value={refiningBatchNumber} onChange={(e) => setRefiningBatchNumber(e.target.value)} placeholder="Auto-generated"/>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="mt-3">
                      {sourceType === 'production' ? (<Popover open={productionOrderPopoverOpen} onOpenChange={setProductionOrderPopoverOpen}>
                          <PopoverTrigger asChild>
                            <Button variant="outline" role="combobox" aria-expanded={productionOrderPopoverOpen} className="w-full justify-between">
                          ...
                          <PopoverContent className="w-full p-0">
                            <Command>
                              <CommandInput placeholder="Search production orders..." value={productionOrderSearchTerm} onValueChange={setProductionOrderSearchTerm}/>
                              <CommandList>
                                <CommandEmpty>No production order found.</CommandEmpty>
                                <CommandGroup>
                                  {productionOrders
                ?.filter((order) => order.batchNumber.toLowerCase().includes(productionOrderSearchTerm.toLowerCase()) ||
                ...
                ?.map((order) => (<CommandItem key={order.id} value={order.batchNumber} onSelect={() => {
                    ...
                    setProductionOrderSearchTerm('');
                        ...
                        </Popover>) : (<Popover open={stockItemPopoverOpen} onOpenChange={setStockItemPopoverOpen}>
                          <PopoverTrigger asChild>
                            <Button variant="outline" role="combobox" aria-expanded={stockItemPopoverOpen} className="w-full justify-between">
                          ...
                          <PopoverContent className="w-full p-0">
                            <Command>
                              <CommandInput placeholder="Search inventory..." value={stockItemSearchTerm} onValueChange={setStockItemSearchTerm}/>
                              <CommandList>
                                <CommandEmpty>No inventory item found.</CommandEmpty>
                                <CommandGroup>
                                  {inventoryItems
                ?.filter((item) => item.name.toLowerCase().includes(stockItemSearchTerm.toLowerCase()) ||
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-4 gap-2 mb-3">
                        <Popover open={refiningMaterialPopoverOpen} onOpenChange={setRefiningMaterialPopoverOpen}>
                          <PopoverTrigger asChild>
                            <Button variant="outline" role="combobox" aria-expanded={refiningMaterialPopoverOpen} className="justify-between">
                          ...
                          <PopoverContent className="w-80 p-0">
                            <Command>
                              <CommandInput placeholder="Search materials..." value={refiningMaterialSearchTerm} onValueChange={setRefiningMaterialSearchTerm}/>
                              <CommandList>
                                <CommandEmpty>No material found.</CommandEmpty>
                                <CommandGroup>
                                  {rawMaterialsData
            ?.filter((material) => (material.name.toLowerCase().includes(refiningMaterialSearchTerm.toLowerCase()) ||
            ...
            ?.map((material) => (<CommandItem key={material.id} value={material.name} onSelect={() => {
                ...
                setRefiningMaterialSearchTerm('');
                        ...
                        <Input type="number" placeholder="Qty" min="1" value={refiningMaterialQuantity === '0' ? '' : refiningMaterialQuantity} onChange={(e) => setRefiningMaterialQuantity(e.target.value === '' || e.target.value === '0' ? '' : Math.max(1, parseInt(e.target.value) || 1).toString())}/>

                        <Input type="number" step="0.01" placeholder="Unit Price" min="0.01" value={refiningMaterialUnitPrice === '0' || refiningMaterialUnitPrice === '0.00' ? '' : refiningMaterialUnitPrice} onChange={(e) => setRefiningMaterialUnitPrice(e.target.value === '' || e.target.value === '0' || e.target.value === '0.00' ? '' : e.target.value)}/>
                      ...
                      {/* Materials List */}
                      {refiningRawMaterials.filter(material => material.quantity > 0 && parseFloat(material.unitPrice || '0') > 0).length > 0 && (<div className="grid grid-cols-2 gap-2">
                          {refiningRawMaterials
                .filter(material => material.quantity > 0 && parseFloat(material.unitPrice || '0') > 0)
                .map((material, index) => (<div key={index} className="flex items-center justify-between p-2 bg-muted rounded">
                              ...
                              <Button variant="ghost" size="sm" onClick={() => handleRemoveRefiningMaterial(index)}>
                                <X className="h-3 w-3"/>
                              </Button>
                            </div>))}
                        </div>)}
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-4 gap-2 mb-3">
                        <Popover open={refiningPackagingPopoverOpen} onOpenChange={setRefiningPackagingPopoverOpen}>
                          <PopoverTrigger asChild>
                            <Button variant="outline" role="combobox" aria-expanded={refiningPackagingPopoverOpen} className="justify-between">
                              {packagingToAdd ? packagingToAdd.name : 'Select packaging'}
                              <Search className="ml-2 h-4 w-4 shrink-0 opacity-50"/>
                            </Button>
                          </PopoverTrigger>
                          <PopoverContent className="w-80 p-0">
                            <Command>
                              <CommandInput placeholder="Search packaging..." value={refiningPackagingSearchTerm} onValueChange={setRefiningPackagingSearchTerm}/>
                              <CommandList>
                                <CommandEmpty>No packaging found.</CommandEmpty>
                                <CommandGroup>
                                  {inventoryItems
            ?.filter((item) => (item.name.toLowerCase().includes(refiningPackagingSearchTerm.toLowerCase()) ||
            ...
            ?.map((item) => (<CommandItem key={item.id} value={item.name} onSelect={() => {
                ...
                setRefiningPackagingSearchTerm('');
                      ...
                      {/* Packaging List */}
                      {packagingItems.filter(item => (item.quantity || 0) > 0 && parseFloat(item.unitPrice || '0') > 0).length > 0 && (<div className="space-y-2">
                          {packagingItems
                .filter(item => (item.quantity || 0) > 0 && parseFloat(item.unitPrice || '0') > 0)
                .map((item, index) => (<div key={index} className="flex items-center justify-between p-2 bg-muted rounded">
                              <span className="font-medium">{item.name}</span>
                              <span className="text-sm text-muted-foreground">
                                {item.quantity} {item.unitOfMeasure} × ${item.unitPrice}
                              </span>
                    ...
                    <div>
                      ...
                      <div className="flex gap-2 mb-3">
                        <Input value={newRefiningStep} onChange={(e) => setNewRefiningStep(e.target.value)} placeholder="Add refining step..." onKeyPress={(e) => e.key === 'Enter' && handleAddRefiningStep()}/>
                        <Button onClick={handleAddRefiningStep} size="sm">
                          <Plus className="h-4 w-4"/>
                        </Button>
                      </div>

                      {refiningSteps.length > 0 && (<div className="space-y-2">
                          {refiningSteps.map((step, index) => (<div key={index} className="flex items-center justify-between p-2 bg-muted rounded">
                              <span className="font-medium">{step}</span>
                              <Button variant="ghost" size="sm" onClick={() => handleRemoveRefiningStep(index)}>
                                <X className="h-4 w-4"/>
                              </Button>
                            </div>))}
                        </div>)}
                    ...
                    <div>
                      ...
                      <Textarea value={expectedOutput} onChange={(e) => setExpectedOutput(e.target.value)} placeholder="Describe the expected output..."/>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-2 gap-4 mt-2">
                        <div>
                          ...
                          <Input type="number" step="0.01" value={refiningSubtotal} onChange={(e) => setRefiningSubtotal(e.target.value)} placeholder="0.00"/>
                        </div>
                        <div>
                          <Label>Transportation Cost</Label>
                          <Input type="number" step="0.01" value={refiningTransportationCost} onChange={(e) => setRefiningTransportationCost(e.target.value)} placeholder="0.00"/>
                        </div>
                        <div>
                          <Label>Labor Cost</Label>
                          <Input type="number" step="0.01" value={refiningLaborCost} onChange={(e) => setRefiningLaborCost(e.target.value)} placeholder="0.00"/>
                        </div>
                        <div>
                          <Label>Equipment Cost</Label>
                          <Input type="number" step="0.01" value={refiningEquipmentCost} onChange={(e) => setRefiningEquipmentCost(e.target.value)} placeholder="0.00"/>
                        </div>
                        <div>
                          <Label>Quality Control Cost</Label>
                          <Input type="number" step="0.01" value={refiningQualityControlCost} onChange={(e) => setRefiningQualityControlCost(e.target.value)} placeholder="0.00"/>
                        </div>
                        <div>
                          <Label>Storage Cost</Label>
                          <Input type="number" step="0.01" value={refiningStorageCost} onChange={(e) => setRefiningStorageCost(e.target.value)} placeholder="0.00"/>
                        </div>
                        <div>
                          <Label>Processing Cost</Label>
                          <Input type="number" step="0.01" value={refiningProcessingCost} onChange={(e) => setRefiningProcessingCost(e.target.value)} placeholder="0.00"/>
                        </div>
                        <div>
                          <Label>Tax Percentage</Label>
                          <Input type="number" value={refiningTaxPercentage} onChange={(e) => setRefiningTaxPercentage(parseInt(e.target.value) || 14)} placeholder="14"/>
                        </div>
                      </div>
                    </div>
                    ...
                    <div>
                      <Label>Transportation Notes</Label>
                      <Textarea value={refiningTransportationNotes} onChange={(e) => setRefiningTransportationNotes(e.target.value)} placeholder="Any special transportation requirements..."/>
                    </div>
              ...
              <Card>
                <CardContent className="p-6">
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between">
                      <span>Subtotal:</span>
                      <span>{formatCurrency((refiningRawMaterials.reduce((sum, material) => sum + (material.quantity * parseFloat(material.unitPrice || '0')), 0) +
            ...
            (parseFloat(refiningQualityControlCost) || 0) +
            (parseFloat(refiningStorageCost) || 0) +
            (parseFloat(refiningProcessingCost) || 0)).toFixed(2))}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Transportation Cost:</span>
            ...
            (parseFloat(refiningQualityControlCost) || 0) +
            (parseFloat(refiningStorageCost) || 0) +
            (parseFloat(refiningProcessingCost) || 0)) * (refiningTaxPercentage / 100)).toFixed(2))}</span>
                    </div>
                    <div className="flex justify-between font-bold text-lg">
                      <span>Total:</span>
                ...
                <Table>
                  ...
                  <TableBody>
                    {isLoadingOrders ? (<TableRow>
                      ...
                      </TableRow>) : refiningOrders.length === 0 ? (<TableRow>
                        <TableCell colSpan={7} className="text-center py-4 text-muted-foreground">
                          No refining orders found
                        </TableCell>
                      </TableRow>) : (refiningOrders.map((order) => (<TableRow key={order.id}>
                          <TableCell className="font-medium">{order.batchNumber}</TableCell>
                          <TableCell>{order.customerName}</TableCell>
          ...
          <div className="grid gap-6 max-h-[60vh] overflow-y-auto pr-2">
            <div className="grid md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Badge variant="outline" className={`inline-flex items-center whitespace-nowrap font-medium text-xs px-2 py-1 rounded-full border ${selectedOrder?.status === 'completed' ? 'bg-green-100 text-green-800 border-green-300' :
            ...
            {selectedOrder?.orderType === 'production' ? (<>
                ...
                <div>
                  ...
                  {selectedOrder?.rawMaterials && selectedOrder.rawMaterials.length > 0 ? (<div className="rounded-md border overflow-hidden">
                      <Table>
                        <TableHeader>
                          <TableRow>
                    ...
                    : selectedOrder.rawMaterials).map((material, index) => (<TableRow key={index}>
                    ...
                    </div>) : (<p className="text-sm text-muted-foreground">No raw materials specified</p>)}
                </div>

                {selectedOrder?.packagingMaterials && selectedOrder.packagingMaterials.length > 0 && (<div>
                    ...
                    : selectedOrder.packagingMaterials).map((item, index) => (<TableRow key={index}>
                ...
                {selectedOrder?.refiningSteps && (<div>
                    ...
                    : selectedOrder.refiningSteps).map((step, index) => (<div key={index} className="p-2 bg-muted rounded text-sm">
                          {index + 1}. {step}
                        </div>))}
                    </div>
                  </div>)}
            ...
            {selectedOrder?.transportationCost && parseFloat(selectedOrder.transportationCost) > 0 && (<div>
                <h3 className="text-sm font-medium text-muted-foreground mb-1">Transportation</h3>
                <p>Cost: ${selectedOrder.transportationCost}</p>
                {selectedOrder.transportationNotes && (<p className="text-sm text-muted-foreground mt-1">{selectedOrder.transportationNotes}</p>)}
              </div>)}
