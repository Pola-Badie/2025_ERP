...
const BackupRestore = () => {
    ...
    const { data: backupSettings, isLoading: isLoadingSettings } = useQuery({
        queryKey: ['/api/backup-settings'],
    });
    // Backup and restore operations
    const { performBackup, isBackingUp, restoreFromBackup, isRestoring, updateBackupSettings, isUpdatingSettings } = useBackup();
    ...
    const handleToggleBackupSetting = (setting) => {
        if (!backupSettings)
            return;
    ...
    return (<div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        <Card className="md:col-span-2">
          <CardContent className="p-6">
            ...
            {isLoadingBackups ? (<div className="flex justify-center p-8">
                <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-primary"></div>
              </div>) : !backups?.length ? (<div className="text-center py-6">
                ...
                {backups.map((backup) => (<div key={backup.id} className="border rounded-lg p-4 hover:bg-slate-50 transition-colors">
                    <div className="flex justify-between items-start">
                      <div>
                        <div className="flex items-center mb-1">
                          ...
                          <span className={`ml-2 text-xs px-2 py-0.5 rounded-full ${getStatusColor(backup.status)}`}>
                            {backup.status}
                          </span>
                        </div>
                      ...
                      <Button variant="outline" size="sm" onClick={() => openRestoreDialog(backup)} disabled={isRestoring || backup.status !== 'completed'}>
        ...
        <Card>
          <CardContent className="p-6">
            ...
            {isLoadingSettings ? (<div className="space-y-4">
                ...
                <div className="flex items-center justify-between p-3 border rounded-lg">
                  ...
                  <Switch checked={backupSettings?.dailyBackup || false} onCheckedChange={() => handleToggleBackupSetting('dailyBackup')} disabled={isUpdatingSettings}/>
                </div>
                ...
                <div className="flex items-center justify-between p-3 border rounded-lg">
                  ...
                  <Switch checked={backupSettings?.weeklyBackup || false} onCheckedChange={() => handleToggleBackupSetting('weeklyBackup')} disabled={isUpdatingSettings}/>
                </div>
                ...
                <div className="flex items-center justify-between p-3 border rounded-lg">
                  ...
                  <Switch checked={backupSettings?.monthlyBackup || false} onCheckedChange={() => handleToggleBackupSetting('monthlyBackup')} disabled={isUpdatingSettings}/>
                </div>
      ...
      <Card>
        <CardContent className="p-6">
          ...
          <div className="space-y-2">
            <Button variant="outline" className="w-full" onClick={handleManualBackup} disabled={isBackingUp}>
              <Upload className="h-4 w-4 mr-2"/>
              Create New Backup
            </Button>
          </div>
      ...
      <Dialog open={restoreDialogOpen} onOpenChange={setRestoreDialogOpen}>
        <DialogContent>
          ...
          {selectedBackup && (<div className="py-4">
          ...
          <DialogFooter>
            ...
            <Button variant="default" className="bg-amber-600 hover:bg-amber-700" onClick={handleRestore} disabled={isRestoring}>
