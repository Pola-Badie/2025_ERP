...
import { Search, Filter, Calendar, Eye, Download, ClipboardList, Package, Factory, TestTube, FileText, Plus, Clock, CheckCircle2, DollarSign } from 'lucide-react';
...
const getQuotationTypeBadge = (type) => {
    switch (type) {
        case 'manufacturing':
            return (<Badge className="bg-purple-50 text-purple-700 border-purple-200 hover:bg-purple-100">
          <Factory className="mr-1 h-3 w-3"/>
          Manufacturing
        </Badge>);
        case 'refining':
            return (<Badge className="bg-orange-50 text-orange-700 border-orange-200 hover:bg-orange-100">
        ...
        default:
            return (<Badge className="bg-gray-50 text-gray-700 border-gray-200">
...
const QuotationHistory = () => {
    ...
    const { data: quotations = [], isLoading } = useQuery({
        ...
        queryFn: async () => {
            const res = await apiRequest('GET', `/api/quotations?query=${encodeURIComponent(searchTerm)}&status=${statusFilter}&type=${typeFilter}&date=${dateFilter}`);
    ...
    return (<div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-emerald-50">
      <div className="container mx-auto p-6 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          ...
          <Button onClick={() => window.location.href = '/create-quotation'} className="bg-blue-600 hover:bg-blue-700 text-white shadow-md">
            ...
            <div className="flex flex-col md:flex-row gap-4">
              <div className="flex-1">
                <div className="relative">
                  ...
                  <Input placeholder="Search quotations by number, customer, or product..." className="pl-10 border-gray-200 focus:border-blue-500 focus:ring-blue-500" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}/>
                </div>
              </div>
            ...
            {isLoading ? (<div className="flex justify-center py-12">
                ...
                <div className="bg-gradient-to-r from-slate-50 to-gray-50 rounded-lg border border-gray-200 p-4">
                  <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      <Button variant="outline" size="sm" onClick={() => window.location.href = '/create-quotation'} className="text-purple-600 border-purple-200 hover:bg-purple-50 transition-colors">
                        <Plus className="mr-2 h-4 w-4"/>
                        New Quotation
                      </Button>
                      <Button variant="outline" size="sm" onClick={() => alert('Bulk actions: Select quotations and perform batch operations like status updates or exports')} className="text-blue-600 border-blue-200 hover:bg-blue-50 transition-colors">
                        <FileText className="mr-2 h-4 w-4"/>
                        Bulk Actions
                      </Button>
                      <Button variant="outline" size="sm" onClick={() => {
                const csvContent = filteredQuotations.map(q => `"${q.quotationNumber}","${q.customerName}","${q.date}","${q.status}","${q.total || q.amount}"`).join('\n');
                ...
                <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
                  <div className="overflow-x-auto">
                    ...
                    <TableBody>
                      {filteredQuotations.map((quotation, index) => (<TableRow key={quotation.id} className="hover:bg-gray-50/70 border-b border-gray-100 transition-colors">
                    ...
                    setShowPreview(true);
                          ...
                          <TableCell className="font-medium text-gray-900 px-4">
                            <div>
                              ...
                              {quotation.type === 'manufacturing' && (<div className="text-xs text-gray-500">Manufacturing Services</div>)}
                              {quotation.type === 'refining' && (<div className="text-xs text-gray-500">API Purification</div>)}
                              {quotation.type === 'finished' && (<div className="text-xs text-gray-500">Finished Products</div>)}
                            </div>
                          </TableCell>
                          ...
                          <TableCell className="px-4">
                            {quotation.status === 'accepted' && (<span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">
                                Accepted
                              </span>)}
                            {quotation.status === 'pending' && (<span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-orange-100 text-orange-800">
                                Pending
                              </span>)}
                            {quotation.status === 'rejected' && (<span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800">
                                Rejected
                              </span>)}
                            {quotation.status === 'expired' && (<span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800">
                                Expired
                              </span>)}
                            {quotation.status === 'draft' && (<span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-800">
                                Draft
                              </span>)}
                            {quotation.status === 'sent' && (<span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                                Sent
                              </span>)}
                          </TableCell>
                          <TableCell className="px-4 text-center">
                            <div className="flex items-center gap-2">
                              <Button variant="outline" size="sm" className="h-8 px-3 text-xs text-blue-600 border-blue-200 hover:bg-blue-50" onClick={() => {
                    setSelectedQuotation(quotation);
                    setShowPreview(true);
                }}>
                              ...
                              <Button variant="ghost" size="sm" className="h-7 w-7 p-0 text-green-600 hover:bg-green-50 rounded-full" title="Convert to Invoice" onClick={() => handleConvertToInvoice(quotation)}>
        ...
        {/* Preview Dialog */}
        {selectedQuotation && (<Dialog open={showPreview} onOpenChange={setShowPreview}>
            <DialogContent className="max-w-6xl max-h-[95vh] w-[95vw] overflow-hidden flex flex-col">
              ...
              <div className="flex-1 overflow-y-auto">
                <PrintableQuotation quotationNumber={selectedQuotation.quotationNumber} date={new Date(selectedQuotation.date)} validUntil={selectedQuotation.validUntil} customer={{
            ...
            }))} packagingItems={selectedQuotation.packagingItems || []} subtotal={selectedQuotation.subtotal} transportationFees={selectedQuotation.transportationFees} packagingFees={selectedQuotation.packagingFees} vatPercentage={14} vatAmount={selectedQuotation.tax} grandTotal={selectedQuotation.total} notes={selectedQuotation.notes} transportationType={selectedQuotation.transportationType} transportationNotes={selectedQuotation.transportationNotes} packagingType={selectedQuotation.packagingType} packagingNotes={selectedQuotation.packagingNotes} quotationType={selectedQuotation.type} termsAndConditions={selectedQuotation.termsAndConditions}/>
              </div>
