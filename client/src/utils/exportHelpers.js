 ...
 */
export const exportOrdersToCSV = (orders, orderType, warehouseFilter = null) => {
    ...
    try {
        ...
        const filteredOrders = warehouseFilter
            ? orders.filter(order => (order.location === warehouseFilter ||
        ...
        link.setAttribute('download', `${orderType}-orders${warehouseLabel}-${format(new Date(), 'yyyy-MM-dd')}.csv`);
