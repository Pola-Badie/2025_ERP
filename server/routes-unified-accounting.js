import { db } from "./db";
import { sales, saleItems, expenses, purchaseOrders, purchaseOrderItems, customers, suppliers, products, productCategories, journalEntries, accounts } from "@shared/schema";
import { sql, eq, and, or, gte, lte, desc, count, sum } from "drizzle-orm";
export function registerUnifiedAccountingRoutes(app) {
    // =============== UNIFIED INVOICES ENDPOINT ===============
    // This endpoint provides the EXACT same data to both invoice history and accounting modules
    app.get("/api/unified/invoices", async (req, res) => {
        try {
            ...
            if (status) {
                query = query.where(eq(sales.paymentStatus, status));
            }
            let salesData = await query.orderBy(desc(sales.date));
            if (limit) {
                salesData = salesData.slice(0, parseInt(limit));
            }
    ...
    app.get("/api/unified/purchase-orders", async (req, res) => {
        try {
    ...
    app.patch("/api/unified/purchase-orders/:id/status", async (req, res) => {
        try {
            const { id } = req.params;
    ...
    app.get("/api/accounting/unified-dashboard", async (_req, res) => {
        try {
            const now = new Date();
            const firstDayOfMonth = new Date(now.getFullYear(), now.getMonth(), 1);
            const lastDayOfMonth = new Date(now.getFullYear(), now.getMonth() + 1, 0);
            // Run all queries in parallel for better performance
            const [monthlyRevenue, monthlyExpenses, journalCount, accountsCount, pendingPurchasesCount, outstandingInvoices] = await Promise.all([
                // 1. Get Revenue from Invoices (This Month)
                db.select({ total: sql `COALESCE(SUM(CAST(grand_total AS DECIMAL)), 0)` })
                    .from(sales)
                    .where(and(gte(sales.date, firstDayOfMonth), lte(sales.date, lastDayOfMonth))),
                // 2. Get Expenses (This Month)  
                db.select({ total: sql `COALESCE(SUM(CAST(amount AS DECIMAL)), 0)` })
                    .from(expenses)
                    .where(and(gte(expenses.date, firstDayOfMonth.toISOString().split('T')[0]), lte(expenses.date, lastDayOfMonth.toISOString().split('T')[0]))),
    ...
    app.get("/api/accounting/pending-purchases", async (req, res) => {
        try {
    ...
    app.get("/api/accounting/purchases", async (_req, res) => {
        try {
    ...
    app.get("/api/accounting/invoices-due", async (_req, res) => {
        try {
            ...
            const outstandingInvoices = await db
                ...
                .from(sales)
                .leftJoin(customers, eq(sales.customerId, customers.id))
                .where(or(eq(sales.paymentStatus, 'pending'), eq(sales.paymentStatus, 'partial')))
    ...
    app.get("/api/accounting/revenue", async (req, res) => {
        try {
            ...
            if (startDate && endDate) {
                query = query.where(and(gte(sales.date, new Date(startDate)), lte(sales.date, new Date(endDate))));
            }
    ...
    app.patch("/api/accounting/approve-purchase/:id", async (req, res) => {
        try {
    ...
    app.get("/api/accounting/sync-status", async (_req, res) => {
        try {
