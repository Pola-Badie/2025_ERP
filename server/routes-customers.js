...
import { db } from "./db";
import { customers, sales, orders, insertCustomerSchema } from "@shared/schema";
const router = Router();
...
router.get("/", async (req, res) => {
    try {
        ...
        let whereConditions = [];
        if (search) {
            whereConditions.push(sql `${customers.name} ILIKE ${'%' + search + '%'} OR ${customers.email} ILIKE ${'%' + search + '%'} OR ${customers.company} ILIKE ${'%' + search + '%'}`);
        }
        if (sector) {
            whereConditions.push(eq(customers.sector, sector));
        }
