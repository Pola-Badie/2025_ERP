...
import { db } from './db';
import { sales, purchaseOrders, expenses } from '../shared/schema';
...
router.get('/profit-loss', async (req, res) => {
    try {
        ...
        const invoiceData = await db.select().from(sales)
            .where(and(sql `DATE(${sales.date}) >= ${start.toISOString().split('T')[0]}`, sql `DATE(${sales.date}) <= ${end.toISOString().split('T')[0]}`));
        const expenseData = await db.select().from(expenses)
            .where(and(gte(expenses.date, start.toISOString().split('T')[0]), lte(expenses.date, end.toISOString().split('T')[0])));
...
router.get('/sales-analysis', async (req, res) => {
    ...
    `);
        res.json({
            summary: {
                totalSales: salesData.rows.reduce((acc, row) => acc + parseFloat(row.revenue || 0), 0),
                totalTransactions: salesData.rows.reduce((acc, row) => acc + parseInt(row.transactions || 0), 0),
                avgOrderValue: salesData.rows.length > 0 ?
                    salesData.rows.reduce((acc, row) => acc + parseFloat(row.avg_order_value || 0), 0) / salesData.rows.length : 0,
                topCategory: categoryData.rows.length > 0 ?
                    categoryData.rows.reduce((prev, current) => (parseFloat(prev.total) > parseFloat(current.total)) ? prev : current).grade : 'N/A'
            },
            monthlyData: salesData.rows.map((row) => ({
                month: new Date(row.month).toLocaleDateString('en-US', { month: 'short', year: 'numeric' }),
                revenue: parseFloat(row.revenue || 0),
                transactions: parseInt(row.transactions || 0),
                avgOrderValue: parseFloat(row.avg_order_value || 0)
            })),
            categoryBreakdown: categoryData.rows.map((row) => ({
...
router.get('/inventory-analysis', async (req, res) => {
    ...
    `);
      ...
      ORDER BY month DESC
    `);
        const totalValue = inventoryLevels.rows.reduce((acc, row) => acc + parseFloat(row.total_value || 0), 0);
        const lowStockCount = inventoryLevels.rows.filter((row) => row.status === 'Low Stock').length;
        const outOfStockCount = inventoryLevels.rows.filter((row) => row.status === 'Out of Stock').length;
        res.json({
            summary: {
                ...
                avgTurnoverRatio: turnoverData.rows.length > 0 ?
                    turnoverData.rows.reduce((acc, row) => acc + parseFloat(row.turnover_ratio || 0), 0) / turnoverData.rows.length : 0
            },
            stockLevels: inventoryLevels.rows.slice(0, 20).map((row) => ({
                id: row.id,
                name: row.name,
                sku: row.sku,
            ...
            topMovers: turnoverData.rows.slice(0, 10).map((row) => ({
            ...
            stockTrends: stockMovement.rows.map((row) => ({
                month: new Date(row.month).toLocaleDateString('en-US', { month: 'short', year: 'numeric' }),
...
router.get('/production-analysis', async (req, res) => {
    ...
    `);
        ...
        const totalCosts = costAnalysis.rows.reduce((acc, row) => acc + parseFloat(row.total_amount || 0), 0);
        res.json({
            summary: {
            ...
            gradeBreakdown: gradeBreakdown.rows.map((row) => ({
                grade: row.grade === 'P' ? 'Pharmaceutical' :
                    row.grade === 'F' ? 'Food Grade' :
                        row.grade === 'T' ? 'Technical' : 'Other',
            ...
            costBreakdown: costAnalysis.rows.map((row) => ({
                type: row.cost_type,
                count: parseInt(row.count || 0),
...
router.get('/top-customers', async (req, res) => {
    ...
    `);
      ...
      ORDER BY month DESC
    `);
        const totalRevenue = topCustomers.rows.reduce((acc, row) => acc + parseFloat(row.total_revenue || 0), 0);
        res.json({
            summary: {
                ...
                topSegment: segmentation.rows.length > 0 ?
                    segmentation.rows.reduce((prev, current) => (parseFloat(prev.segment_revenue) > parseFloat(current.segment_revenue)) ? prev : current).segment : 'N/A'
            },
...
router.get('/finance-breakdown', async (req, res) => {
    ...
    `);
        ...
        const totalRevenue = revenueBreakdown.rows.reduce((acc, row) => acc + parseFloat(row.total_revenue || 0), 0);
        const totalExpenses = expenseBreakdown.rows.reduce((acc, row) => acc + parseFloat(row.total_amount || 0), 0);
        ...
        res.json({
            ...
            revenueBreakdown: revenueBreakdown.rows.map((row) => ({
            ...
            cashFlow: cashFlow.rows.map((row) => ({
                month: new Date(row.month).toLocaleDateString('en-US', { month: 'short', year: 'numeric' }),
                revenue: parseFloat(row.total_revenue || 0),
...
router.get('/dashboard-analytics', async (req, res) => {
    ...
    `);
        ...
        res.json({
            salesAnalytics: {
                monthlySales: salesData.rows.map((row) => ({
                    month: row.month,
            ...
            customerGrades: customerGrades.rows.map((row) => ({
                grade: row.grade,
            ...
            productionData: productionData.rows.map((row) => ({
                id: row.id,
                name: row.name,
