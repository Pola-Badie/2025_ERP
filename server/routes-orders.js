import { db } from './db';
import { orders, customers, systemPreferences, warehouseInventory, warehouses } from '../shared/schema';
import { eq, desc, and, sql } from 'drizzle-orm';
import { calculateMaterialsCost, parseMaterials } from './utils/materials-parser';
// ============= INVENTORY MANAGEMENT HELPER FUNCTIONS =============
/**
 * Check if sufficient stock is available in warehouses for a product
 */
async function checkStockAvailability(productId, requiredQuantity, warehouseId) {
    try {
        let warehouseStock;
        if (warehouseId) {
            ...
            warehouseStock = await db
                ...
                .where(and(eq(warehouseInventory.productId, productId), eq(warehouseInventory.warehouseId, warehouseId)));
        }
        else {
 ...
 */
async function deductInventoryStock(productId, quantity, warehouseId) {
    try {
            ...
            for (const warehouse of stockCheck.warehouseDetails) {
                ...
                if (deductFromWarehouse > 0) {
                    await db
                        ...
                        .where(and(eq(warehouseInventory.productId, productId), eq(warehouseInventory.warehouseId, warehouse.warehouseId)));
                    deductions.push({
 ...
 */
async function confirmInventoryDeduction(productId, quantity, warehouseId) {
    try {
        if (warehouseId) {
            await db
                .update(warehouseInventory)
...
export function registerOrderRoutes(app) {
        ...
        try {
            ...
            if (materials && materials.length > 0) {
                for (const material of materials) {
                    if (material.productId && material.quantity) {
                        const stockCheck = await checkStockAvailability(parseInt(material.productId), parseFloat(material.quantity));
                        if (!stockCheck.available) {
                ...
                for (const validation of stockValidation) {
                    const deduction = await deductInventoryStock(validation.productId, validation.quantity);
            ...
            const orderRecord = {
                ...
                materialCostBreakdown: {
                    rawMaterialsCost: materials?.reduce((sum, mat) => sum + (parseFloat(mat.unitPrice || '0') * parseFloat(mat.quantity || '0')), 0) || 0,
                    packagingCost: packaging?.reduce((sum, pack) => sum + (parseFloat(pack.unitPrice || '0') * parseFloat(pack.quantity || '0')), 0) || 0
                }
            };
    ...
    app.patch("/api/refining-orders/:id", async (req, res) => {
        try {
            const orderId = parseInt(req.params.id);
            const { status } = req.body;
    ...
    app.get("/api/orders/production-history", async (req, res) => {
        try {
            ...
            const memoryOrders = global.createdOrders || [];
            ...
            const dbOrdersFormatted = await Promise.all(dbOrders.map(async (order) => {
                let customer = [];
                try {
                    customer = await db
                ...
                return {
                    ...
                    materialCostBreakdown: {
                        rawMaterialsCost: realRawMaterials?.reduce((sum, mat) => sum + (parseFloat(mat.unitPrice || '0') * parseFloat(mat.quantity || '0')), 0) || 0,
                        packagingCost: realPackagingMaterials?.reduce((sum, pack) => sum + (parseFloat(pack.unitPrice || '0') * parseFloat(pack.quantity || '0')), 0) || 0
                    }
                };
            }));
    ...
    app.get("/api/orders/detailed-history", async (req, res) => {
        try {
    ...
    app.get("/api/orders/refining-history", async (req, res) => {
        try {
            ...
            const memoryOrders = global.createdOrders ? global.createdOrders.filter((order) => order.orderType === 'refining') : [];
            console.log(`🔥 MEMORY ORDERS FOUND: ${memoryOrders.length}`);
    ...
    app.patch("/api/orders/:id/profit-margin", async (req, res) => {
        try {
