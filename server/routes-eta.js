...
let etaCredentials = null;
let etaAccessToken = null;
let tokenExpirationTime = 0;
export function registerETARoutes(app) {
    // Authenticate with ETA API
    app.post("/api/eta/authenticate", async (req, res) => {
        try {
    ...
    app.post("/api/eta/submit/:invoiceId", async (req, res) => {
        try {
    ...
    app.post("/api/eta/submit-invoice/:invoiceId", async (req, res) => {
        try {
            const invoiceId = parseInt(req.params.invoiceId);
            if (!invoiceId) {
                return res.status(400).json({
    ...
    app.get("/api/eta/status/:invoiceId", async (req, res) => {
        try {
            const invoiceId = parseInt(req.params.invoiceId);
    ...
    app.get("/api/eta/invoices", async (req, res) => {
        try {
