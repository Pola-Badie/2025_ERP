...
import { db } from "./db";
import { z } from "zod";
import { insertProductSchema, updateProductSchema, insertCustomerSchema, insertSaleSchema, insertSaleItemSchema, updateBackupSettingsSchema, insertPurchaseOrderSchema, insertSupplierSchema, insertProductCategorySchema, updateSystemPreferenceSchema, insertRolePermissionSchema, insertQuotationPackagingItemSchema, quotationPackagingItems, insertOrderSchema, insertOrderItemSchema, insertOrderFeeSchema, insertExpenseSchema, insertWarehouseSchema, insertWarehouseInventorySchema, backupSettings } from "@shared/schema";
...
export async function registerRoutes(app) {
    ...
    app.get("/api/products/raw-materials", (req, res) => {
        try {
    ...
    app.get("/api/products/semi-finished", (req, res) => {
        try {
...
6. Liability: Our liability is limited to the value of services provided. We maintain comprehensive insurance coverage for pharmaceutical operations.`;
    ...
    app.get("/api/quotations", async (req, res) => {
        try {
            ...
            const transformedQuotations = await Promise.all(quotations.map(async (quotation) => {
                ...
                try {
                    ...
                    items = await Promise.all(quotationItems.map(async (item) => {
    ...
    app.get("/api/sales", async (req, res) => {
        try {
            ...
            const sales = await storage.getSales(query || '', date || 'all');
            res.json(sales);
        }
        catch (error) {
    ...
    app.get("/api/purchase-orders", async (req, res) => {
        try {
            ...
            const orders = await storage.getPurchaseOrders(query || '', status || 'all');
            res.json(orders);
        }
        catch (error) {
    ...
    app.get("/api/suppliers", async (req, res) => {
        try {
            const { query } = req.query;
            const suppliers = await storage.getSuppliers(query || '');
            res.json(suppliers);
        }
        catch (error) {
    ...
    app.patch("/api/system-preferences/:id", async (req, res) => {
        try {
            const id = Number(req.params.id);
    ...
    app.get("/api/expenses", async (req, res) => {
        try {
            ...
            const expenses = await storage.getExpenses(query || '', date || 'all', categoryId ? Number(categoryId) : undefined);
            res.json(expenses);
        }
        catch (error) {
    ...
    app.get("/api/orders", async (req, res) => {
        try {
            const { query, status, date } = req.query;
            const orders = await storage.getOrders(query || '', status || 'all', date || 'all');
            res.json(orders);
        }
        catch (error) {
