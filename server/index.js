...
import { requestTimeout } from "./middleware/requestLogger.js";
...
async function ensurePortAvailable(preferredPort) {
    const maxRetries = 3;
...
async function startServer() {
    try {
        ...
        const server = app.listen(PORT, "0.0.0.0", async () => {
            ...
            app.post("/api/sync-data", async (_req, res) => {
                try {
        ...
        process.on('uncaughtException', (error) => {
            ...
            if (error.code === 'EADDRINUSE') {
                logger.error(`❌ Port ${PORT} is already in use! This should not happen with our port management system.`);
