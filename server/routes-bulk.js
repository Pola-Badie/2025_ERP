...
import { z } from 'zod';
import { db } from './db';
import { products, customers, suppliers, orders, expenses, warehouses } from '../shared/schema';
...
router.get('/test', requirePermission('bulk_operations'), (req, res) => {
...
router.post('/bulk-import', requirePermission('bulk_operations'), upload.single('file'), async (req, res) => {
...
router.get('/bulk-export', requirePermission('bulk_operations'), async (req, res) => {
    try {
...
router.get('/bulk-template', requirePermission('bulk_operations'), (req, res) => {
    try {
...
async function processImportRow(type, row, warehouse) {
    switch (type) {
        case 'products':
        ...
        case 'customers':
            ...
            if (existingCustomer.length === 0) {
                ...
                const hasChanges = existing.name !== customerData.name ||
