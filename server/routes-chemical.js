...
function getChemicalFormula(productName) {
    const name = productName.toLowerCase();
    if (name.includes('aspirin') || name.includes('acetylsalicylic'))
        return 'C9H8O4';
...
export function registerChemicalRoutes(app) {
    ...
    app.get("/api/products/semi-finished", async (req, res) => {
        try {
            const productStorage = new ProductStorage();
            ...
            const semiFinishedProducts = allProducts
                .filter(product => product.productType === 'finished' && (product.name.toLowerCase().includes('aspirin') ||
