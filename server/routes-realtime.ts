...
router.get("/reports/advanced", async (req: Request, res: Response) => {
  try {
    ...
    const reportData = {
      ...
      customerMetrics: [
        {
          customer: "Advanced Pharmaceuticals",
          totalOrders: 45,
          totalValue: 125000,
          lastOrder: "2024-01-05",
        },
        {
          customer: "BioTech Innovations",
          totalOrders: 38,
          totalValue: 98000,
          lastOrder: "2024-01-04",
        },
        {
          customer: "PureChem Industries",
          totalOrders: 52,
          totalValue: 145000,
          lastOrder: "2024-01-05",
        },
...
router.get("/notifications/templates", async (req: Request, res: Response) => {
  const templates = [
    ...
    },
    {
      ...
      content:
        "Your order {{orderNumber}} has been confirmed for {{customerName}}.",
      type: "notification",
          ...
          const fetchRealDashboardData = async () => {
            try {
              ...
              const expiring = await db
                ...
                .from(inventory)
                .where(
                  sql`expiry_date <= ${expiringDate.toISOString()}::date AND expiry_date >= CURRENT_DATE`,
                );
