...
import { accounts, journalEntries, journalEntryLines, journalLines, accountingPeriods, customerPayments, paymentAllocations, sales, customers, expenses, insertAccountSchema, insertJournalEntrySchema, insertAccountingPeriodSchema, } from "@shared/schema";
import { eq, and, gte, lte, desc, sql } from "drizzle-orm";
import { generateFinancialSummary, ACCOUNT_CODES, generateJournalEntryNumber, getAccountIdByCode } from "./accounting-integration";
export function registerAccountingRoutes(app) {
    // Accounting API Routes
    // Process refund and update sales revenue
    app.post("/api/accounting/process-refund", async (req, res) => {
        try {
            const { invoiceId, invoiceNumber, customerId, customerName, totalRefundAmount, refundReason, refundDate, items } = req.body;
    ...
    app.get("/api/accounting/trial-balance", async (req, res) => {
        try {
            ...
            const { accountFilter = 'all', fromDate, toDate, includeZeroBalance = 'true', showTransactionDetails = 'false', groupByAccountType = 'false' } = req.query;
            ...
            if (includeZeroBalance === 'false') {
                filteredTrialBalance = trialBalance.filter(account => account.debit > 0 || account.credit > 0);
            }
    ...
    app.get("/api/accounting/profit-loss", async (req, res) => {
        try {
            ...
            const salesData = await db
                .select()
                .from(sales)
                .where(and(sql `DATE(${sales.date}) >= ${start}`, sql `DATE(${sales.date}) <= ${end}`));
            // Get actual expense data
            const expenseData = await db
                .select()
                .from(expenses)
                .where(and(sql `DATE(${expenses.date}) >= ${start}`, sql `DATE(${expenses.date}) <= ${end}`));
    ...
    // Get balance sheet
    app.get("/api/accounting/balance-sheet", async (_req, res) => {
        try {
    ...
    app.get("/api/accounting/customer-balances", async (_req, res) => {
        try {
            ...
            const customerBalances = customersResult.map(customer => {
                ...
                const customerInvoices = financialData.dueInvoices.filter(invoice => invoice.client.toLowerCase().includes(customer.name.toLowerCase()) ||
    ...
    app.get("/api/accounting/cash-flow-disabled", async (req, res) => {
        try {
            const { startDate, endDate } = req.query;
            ...
            const equipmentPurchases = financialData.expenses
                .filter(expense => new Date(expense.date) >= start &&
    ...
    app.post("/api/accounting/generate-sample-data", async (_req, res) => {
        try {
            ...
            // Entry 1: Sales
            journalLinesData.push({ journalId: insertedEntries[0].id, accountId: insertedAccounts[1].id, debit: "15000.00", credit: "0.00", description: "AR - Cairo Medical", position: 1 }, { journalId: insertedEntries[0].id, accountId: insertedAccounts[7].id, debit: "0.00", credit: "15000.00", description: "Sales revenue", position: 2 });
            // Entry 2: Purchase
            journalLinesData.push({ journalId: insertedEntries[1].id, accountId: insertedAccounts[2].id, debit: "8500.00", credit: "0.00", description: "Raw materials", position: 1 }, { journalId: insertedEntries[1].id, accountId: insertedAccounts[4].id, debit: "0.00", credit: "8500.00", description: "AP - ChemCorp", position: 2 });
            // Entry 3: Utilities
            journalLinesData.push({ journalId: insertedEntries[2].id, accountId: insertedAccounts[9].id, debit: "2500.00", credit: "0.00", description: "Utility expense", position: 1 }, { journalId: insertedEntries[2].id, accountId: insertedAccounts[0].id, debit: "0.00", credit: "2500.00", description: "Cash payment", position: 2 });
    ...
    app.post("/api/journal-entries", async (req, res) => {
        try {
            ...
            const journalLinesData = lines.map((line, index) => ({
                journalId: journalEntry.id,
    ...
    app.get("/api/reports/pnl", async (req, res) => {
        try {
            ...
            const salesData = financialData.dueInvoices.filter(invoice => new Date(invoice.invoiceDate) >= start && new Date(invoice.invoiceDate) <= end);
            // COGS from purchases and direct material costs
            const directMaterialPurchases = financialData.purchases.filter(purchase => new Date(purchase.date) >= start && new Date(purchase.date) <= end &&
            ...
            const operatingExpenses = [
                ...financialData.expenses.filter(expense => new Date(expense.date) >= start && new Date(expense.date) <= end),
                ...financialData.purchases.filter(purchase => new Date(purchase.date) >= start && new Date(purchase.date) <= end &&
                    !(purchase.item.toLowerCase().includes('pharmaceutical') ||
                        purchase.item.toLowerCase().includes('active') ||
                        purchase.item.toLowerCase().includes('ingredient') ||
                        purchase.item.toLowerCase().includes('material')))
            ...
            operatingExpenses.forEach(item => {
                const costCenter = 'costCenter' in item ? item.costCenter : 'Operations';
            ...
            const ytdOperatingExpenses = [
                ...financialData.expenses.filter(expense => new Date(expense.date) >= yearStart && new Date(expense.date) <= end),
                ...financialData.purchases.filter(purchase => new Date(purchase.date) >= yearStart && new Date(purchase.date) <= end &&
                    !(purchase.item.toLowerCase().includes('pharmaceutical') ||
                        purchase.item.toLowerCase().includes('active') ||
                        purchase.item.toLowerCase().includes('ingredient') ||
                        purchase.item.toLowerCase().includes('material')))
            ...
            res.json({
                ...
                operatingExpenses: {
                    ...
                    items: Object.entries(expensesByCategory).flatMap(([category, expenses]) => expenses.map((expense, idx) => ({
    ...
    app.get("/api/reports/balance-sheet", async (req, res) => {
        try {
            ...
            const cashTransactions = [
                ...financialData.expenses.filter(expense => new Date(expense.date) <= reportDate &&
                    (expense.paymentMethod === 'Cash' || expense.paymentMethod === 'Bank Transfer')).map(expense => ({ ...expense, type: 'expense', amount: -expense.amount })),
                ...financialData.purchases.filter(purchase => new Date(purchase.date) <= reportDate &&
                    purchase.paidStatus === 'Paid' &&
                    (purchase.paymentMethod === 'Cash' || purchase.paymentMethod === 'Bank Transfer')).map(purchase => ({ ...purchase, type: 'purchase', amount: -purchase.total })),
                ...financialData.dueInvoices.filter(invoice => new Date(invoice.invoiceDate) <= reportDate &&
                    invoice.status === 'Paid').map(invoice => ({ ...invoice, type: 'sale', amount: invoice.amountPaid }))
            ];
            const cashBalance = Math.max(0, cashTransactions.reduce((sum, transaction) => sum + ('amount' in transaction ? transaction.amount : 0), 50000)); // Starting cash balance
            // Accounts Receivable - unpaid customer invoices
            const accountsReceivableData = financialData.dueInvoices.filter(invoice => new Date(invoice.invoiceDate) <= reportDate &&
                invoice.status !== 'Paid');
            const accountsReceivableBalance = accountsReceivableData.reduce((sum, invoice) => sum + invoice.balance, 0);
            // Inventory - from material purchases not yet used
            const inventoryData = financialData.purchases.filter(purchase => new Date(purchase.date) <= reportDate &&
                (purchase.item.toLowerCase().includes('pharmaceutical') ||
                    purchase.item.toLowerCase().includes('active') ||
                    purchase.item.toLowerCase().includes('ingredient') ||
                    purchase.item.toLowerCase().includes('material')));
            const inventoryBalance = inventoryData.reduce((sum, purchase) => sum + (purchase.total * 0.3), 0); // Assume 30% of materials remain in inventory
            // Accounts Payable - unpaid supplier invoices
            const accountsPayableData = financialData.purchases.filter(purchase => new Date(purchase.date) <= reportDate &&
                purchase.paidStatus !== 'Paid');
            const accountsPayableBalance = accountsPayableData.reduce((sum, purchase) => sum + purchase.total, 0);
            // Accrued Expenses - from expense records
            const accruedExpensesData = financialData.expenses.filter(expense => new Date(expense.date) <= reportDate &&
                expense.paymentMethod === 'Credit');
            const accruedExpensesBalance = accruedExpensesData.reduce((sum, expense) => sum + expense.amount, 0);
            // Calculate totals
    ...
    app.get("/api/accounting-periods", async (_req, res) => {
        try {
    ...
    app.post("/api/accounting-periods", async (req, res) => {
        try {
            ...
            const overlappingPeriods = await db
                .select()
                .from(accountingPeriods)
                .where(and(lte(accountingPeriods.startDate, new Date(validatedData.endDate)), gte(accountingPeriods.endDate, new Date(validatedData.startDate))));
    ...
    app.patch("/api/accounting-periods/:id/status", async (req, res) => {
        try {
            const { id } = req.params;
    ...
    app.get("/api/customer-payments", async (req, res) => {
        try {
            ...
            if (customerId) {
                query = query.where(eq(customerPayments.customerId, parseInt(customerId)));
            }
    ...
    app.get("/api/customer-invoices", async (req, res) => {
        try {
            ...
            const invoices = await db
                ...
                amountPaid: sql `'0'`, // We'll calculate this below
                status: sales.paymentStatus
            })
                .from(sales)
                .where(and(eq(sales.customerId, parseInt(customerId)), eq(sales.paymentStatus, 'pending')))
                .orderBy(sales.date);
            // For each invoice, calculate amount paid from payment allocations
            const invoicesWithPayments = await Promise.all(invoices.map(async (invoice) => {
                const allocations = await db
                    .select({ amount: paymentAllocations.amount })
                    .from(paymentAllocations)
                    .where(eq(paymentAllocations.invoiceId, invoice.id));
                const amountPaid = allocations.reduce((total, allocation) => total + parseFloat(allocation.amount.toString()), 0);
    ...
    app.post("/api/accounting/seed-accounts", async (_req, res) => {
        try {
