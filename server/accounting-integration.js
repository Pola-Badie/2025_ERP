import { db } from "./db";
import { accounts, journalEntries, journalEntryLines } from "@shared/schema";
...
export async function getAccountIdByCode(code) {
    try {
...
export async function createInvoiceJournalEntry(invoiceData, userId) {
    try {
...
export async function createPaymentJournalEntry(paymentData, userId) {
    try {
        const entryNumber = await generateJournalEntryNumber();
...
export async function createExpenseJournalEntry(expenseData, userId) {
    try {
        const entryNumber = await generateJournalEntryNumber();
        const cashAccountId = await getAccountIdByCode(ACCOUNT_CODES.CASH);
...
async function getExpenseAccountId(category) {
...
export async function generateFinancialSummary() {
    try {
        ...
        const [revenueResult, expensesResult, outstandingResult, accountsResult, salesCount, expensesCount] = await Promise.all([
            // Total revenue using SQL aggregation
            db.select({ total: sql `COALESCE(SUM(CAST(grand_total AS DECIMAL)), 0)` }).from(sales),
            // Total expenses using SQL aggregation  
            db.select({ total: sql `COALESCE(SUM(CAST(amount AS DECIMAL)), 0)` }).from(expenses),
...
export async function updateAccountBalances(journalEntryId) {
    try {
