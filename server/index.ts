...
async function initializeDatabase() {
  try {
    // Create client with better connection settings
    dbClient = new Client({
...
// Add request logging with better error handling
app.use((req, res, next) => {
  const start = Date.now();
  ...
  res.on('finish', () => {
    ...
    // Log API requests that return null or errors
...
logger.info('🚀 Premier ERP System running on port 5000');
