...
export class PermissionService {
    ...
    async checkPermission(check) {
        ...
        try {
            ...
            const [userPermission] = await db
                ...
                .where(and(eq(userPermissions.userId, check.userId), eq(userPermissions.moduleName, check.resource)));
            if (userPermission) {
                const result = {
            ...
            const [rolePermission] = await db
                ...
                .where(and(eq(rolePermissions.role, user.role), eq(rolePermissions.resource, check.resource), eq(rolePermissions.action, check.action)));
            if (rolePermission) {
                const result = {
                    granted: true,
    ...
    async getUserPermissions(userId) {
        ...
        roleBased.forEach(perm => {
            ...
            const explicitDenied = explicit.find(ep => ep.moduleName === resource && !ep.accessGranted);
     ...
     */
    async setUserPermission(userId, moduleName, accessGranted, adminUserId) {
     ...
     */
    async checkFieldPermission(userId, module, entityType, fieldName, permission) {
        try {
            // Get user details
            ...
            const fieldPerms = await db
                ...
                .where(and(eq(fieldPermissions.module, module), eq(fieldPermissions.entityType, entityType), eq(fieldPermissions.fieldName, fieldName), or(
            // User-specific permission
            and(eq(fieldPermissions.scope, 'user'), eq(fieldPermissions.targetId, userId)), 
            // Role-based permission
            and(eq(fieldPermissions.scope, 'role'), eq(fieldPermissions.targetRole, user.role)), 
            // Global permission
