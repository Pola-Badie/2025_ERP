...
import { users, products, productCategories, customers, suppliers, sales, saleItems, purchaseOrders, purchaseOrderItems, backups, backupSettings, systemPreferences, rolePermissions, loginLogs, userPermissions, journalEntries, journalEntryLines, expenses, warehouses, warehouseInventory, quotations, quotationItems, quotationPackagingItems, insertUserSchema, insertProductSchema, updateProductSchema, insertProductCategorySchema, insertCustomerSchema, insertSaleSchema, insertSaleItemSchema, insertSupplierSchema, insertSystemPreferenceSchema, updateSystemPreferenceSchema, insertRolePermissionSchema, insertLoginLogSchema, insertQuotationPackagingItemSchema } from "@shared/schema";
...
export async function registerRoutes(app) {
    ...
    app.post('/api/bulk/import-json', async (req, res) => {
        ...
        try {
            ...
            if (warehouse) {
                ...
                async function performAutomaticBackup(type) {
                    try {
                        ...
                        exec(`chmod +x ${backupScript} && ${backupScript}`, async (error, stdout, stderr) => {
                            if (error) {
    ...
    app.post('/api/bulk/import', csvUpload.single('file'), async (req, res) => {
    ...
    app.post("/api/users/:userId/permissions", async (req, res) => {
        try {
            ...
            const existingPermission = await db
                .select()
                .from(userPermissions)
                .where(and(eq(userPermissions.userId, userId), eq(userPermissions.moduleName, moduleName)));
    ...
    app.delete("/api/users/:userId/permissions/:moduleName", async (req, res) => {
        try {
            const userId = parseInt(req.params.userId);
    ...
    app.get("/api/customers/:id/profile", async (req, res) => {
        try {
            ...
            const salesWithItemCount = await Promise.all(customerSales.map(async (sale) => {
    ...
    app.get("/api/sample-invoices", async (_req, res) => {
        try {
    ...
    app.patch("/api/users/:id/permissions/:moduleName", async (req, res) => {
        try {
            const userId = Number(req.params.id);
    ...
    // Delete user permission
    app.delete("/api/users/:id/permissions/:moduleName", async (req, res) => {
        try {
            const userId = Number(req.params.id);
            const moduleName = req.params.moduleName;
            // Check if user exists
        ...
        try {
            ...
            if (warehouseId && warehouseId !== '0' && warehouseId !== '') {
                ...
                const productsWithInventory = await db
                    ...
                    .where(and(eq(warehouses.id, warehouseNumber), ...(whereConditions.length > 0 ? whereConditions : [])))
                    .orderBy(products.name);
                // Get categories
                const productsWithCategories = await Promise.all(productsWithInventory.map(async (product) => {
                ...
                const allProductsQuery = await db
                    ...
                    .orderBy(products.name);
                // Get categories
                const productsWithCategories = await Promise.all(allProductsQuery.map(async (product) => {
                    const [category] = await db.select().from(productCategories).where(eq(productCategories.id, product.categoryId));
                    return {
                        ...product,
                        category: category?.name || 'Uncategorized'
    ...
    app.get("/api/sales", async (req, res) => {
        try {
            ...
            if (startDate && endDate) {
                salesQuery = salesQuery.where(and(gte(sales.date, new Date(startDate)), lte(sales.date, new Date(endDate))));
            }
                ...
                for (const warehouse of validation.warehouseDetails) {
                    ...
                    if (deductFromWarehouse > 0) {
                        await db
                            ...
                            .where(and(eq(warehouseInventory.productId, validation.productId), eq(warehouseInventory.warehouseId, warehouse.warehouseId)));
                        itemDeductions.push({
    ...
    app.get("/api/quotations", async (req, res) => {
        try {
            ...
            const transformedQuotations = await Promise.all(dbQuotations.map(async (quotation) => {
                // Get customer name
                ...
                try {
                    ...
                    items = await Promise.all(quotationItemsData.map(async (item) => {
                        // Get product details
            ...
            if (query && query !== '') {
                ...
                filteredQuotations = filteredQuotations.filter(quotation => quotation.quotationNumber.toLowerCase().includes(searchTerm) ||
                    quotation.customerName.toLowerCase().includes(searchTerm) ||
                    quotation.items.some(item => item.productName.toLowerCase().includes(searchTerm)));
            }
            // Filter by status
    ...
    app.post("/api/quotations", async (req, res) => {
        try {
            ...
            const { quotationNumber, type, customerId, customerName, validUntil, notes, items, subtotal, transportationFees, transportationType, transportationNotes, tax, total, status, date } = req.body;
    ...
    app.get("/api/system-preferences", isAdmin, async (req, res) => {
        try {
    ...
    app.get("/api/system-preferences/category/:category", isAdmin, async (req, res) => {
        try {
            ...
            const filteredPreferences = preferences.filter(pref => pref.key.toLowerCase().includes(category.toLowerCase()));
    ...
    app.get("/api/system-preferences/:key", isAdmin, async (req, res) => {
        try {
    ...
    app.post("/api/system-preferences", isAdmin, async (req, res) => {
        try {
    ...
    app.patch("/api/system-preferences/:key", isAdmin, async (req, res) => {
        try {
            const key = req.params.key;
    ...
    app.get("/api/dashboard/analytics", async (req, res) => {
        try {
            ...
            const allSales = await db.select().from(sales);
            const completedSales = allSales.filter(sale => sale.paymentStatus === 'completed' || sale.paymentStatus === 'partial');
            ...
            const peakMonth = last12Months.reduce((peak, current) => current.revenue > peak.revenue ? current : peak);
            // Current month data
            const currentMonthData = monthlyData[monthlyData.length - 1];
            const previousMonthData = monthlyData[monthlyData.length - 2];
            const sameMonthLastYearData = monthlyData.find(m => m.month === currentMonthData.month && m.year === currentMonthData.year - 1);
    ...
    app.get("/api/role-permissions/:role", isAdmin, async (req, res) => {
        try {
    ...
    app.post("/api/role-permissions", isAdmin, async (req, res) => {
        try {
    ...
    app.delete("/api/role-permissions/:id", isAdmin, async (req, res) => {
        try {
            const id = Number(req.params.id);
    ...
    // Get login logs
    app.get("/api/login-logs", isAdmin, async (req, res) => {
        try {
    ...
    app.get('/api/inventory/low-stock', async (req, res) => {
        ...
        `
                ...
                .where(and(eq(products.status, 'active'), lte(products.quantity, products.lowStockThreshold)))
    ...
    app.get('/api/inventory/expiring', async (req, res) => {
        ...
        `
                ...
                .from(products)
                .leftJoin(productCategories, eq(products.categoryId, productCategories.id))
                .where(and(eq(products.status, 'active'), lte(products.expiryDate, thirtyDaysFromNow.toISOString().split('T')[0])))
    ...
    app.get('/api/inventory/warehouse-breakdown', async (req, res) => {
        try {
            ...
            const warehouseBreakdown = await db.select({
                ...
                .from(products)
                .where(and(eq(products.status, 'active'), sql `${products.location} IS NOT NULL AND ${products.location} != ''`))
    ...
    app.get('/api/products/:id/activity', async (req, res) => {
        try {
            const productId = Number(req.params.id);
    ...
    app.get('/api/accounting/reports/:reportType', async (req, res) => {
        try {
            ...
            switch (reportType) {
                ...
                case 'profit-loss':
                    ...
                    reportData = {
                        ...
                        rows: [
                            ...profitLoss.revenue.accounts.map((acc) => [acc.name, `$${parseFloat(acc.balance).toLocaleString()}.00`]),
                            ["", ""],
                            ...profitLoss.expenses.accounts.map((acc) => [acc.name, `($${parseFloat(acc.balance).toLocaleString()}.00)`]),
                        ],
                ...
                case 'balance-sheet':
                    ...
                    reportData = {
                        ...
                        rows: [
                            ["ASSETS", ""],
                            ...balanceSheetData.assets.accounts.map((acc) => [acc.name, `$${parseFloat(acc.balance).toLocaleString()}.00`]),
                            ["", ""],
                            ["LIABILITIES", ""],
                            ...balanceSheetData.liabilities.accounts.map((acc) => [acc.name, `$${parseFloat(acc.balance).toLocaleString()}.00`]),
                            ["", ""],
                            ["EQUITY", ""],
                            ...balanceSheetData.equity.accounts.map((acc) => [acc.name, `$${parseFloat(acc.balance).toLocaleString()}.00`]),
                        ],
                ...
                case 'chart-of-accounts':
                    ...
                    reportData = {
                        ...
                        rows: chartOfAccounts.map((account) => [
                            account.accountCode,
                            account.accountName,
                            account.accountType,
    ...
    app.get("/api/procurement/purchase-orders", async (req, res) => {
        try {
            ...
            const ordersWithItems = await Promise.all(purchaseOrdersData.map(async (order) => {
    ...
    app.post("/api/procurement/purchase-orders", async (req, res) => {
        try {
            const { supplier, items, notes, expectedDeliveryDate, totalAmount, transportationType, transportationCost, transportationNotes } = req.body;
            ...
            const finalTotalAmount = totalAmount || items.reduce((sum, item) => sum + (item.quantity * item.unitPrice), 0);
            // Generate PO number
