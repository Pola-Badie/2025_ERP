...
router.post('/permissions/users/:userId/modules/:moduleName', async (req, res) => {
    try {
        ...
        const adminUserId = req.user?.id || 1; // Fallback for development
        const permission = await permissionService.setUserPermission(userId, moduleName, accessGranted, adminUserId);
        res.json({
            success: true,
            data: permission,
...
router.post('/permissions/users/:userId/bulk', async (req, res) => {
    try {
        ...
        for (const perm of permissions) {
            try {
                const result = await permissionService.setUserPermission(userId, perm.moduleName, perm.accessGranted, adminUserId);
                results.push(result);
            }
            catch (error) {
                errors.push({
...
router.get('/permissions/history', async (req, res) => {
    try {
        const targetUserId = req.query.userId ? parseInt(req.query.userId) : undefined;
