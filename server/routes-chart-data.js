...
export function registerChartDataRoutes(app) {
    ...
    app.get("/api/dashboard/sales-distribution", async (req, res) => {
        try {
      ...
      `);
            ...
            if (distributionResult.rows && distributionResult.rows.length > 0) {
                const totalSales = distributionResult.rows.reduce((sum, row) => sum + parseFloat(row.category_total), 0);
                distributionData = distributionResult.rows.map((row, index) => ({
    ...
    app.get("/api/dashboard/category-performance", async (req, res) => {
      ...
      `);
            ...
            if (performanceResult.rows && performanceResult.rows.length > 0) {
                const maxRevenue = Math.max(...performanceResult.rows.map((row) => parseFloat(row.total_revenue)));
                performanceData = performanceResult.rows.map((row, index) => ({
                    name: row.category_name || 'Other',
