...
export function generatePendingInvoices(count = 10) {
    ...
    for (let i = 0; i < count; i++) {
        ...
        const amountPaid = faker.helpers.maybe(() => faker.number.int({ min: 0, max: total * 0.8 * 100 }) / 100, { probability: 0.6 }) || 0;
...
export function registerCustomerPaymentRoutes(app) {
    ...
    app.get("/api/accounting/payments", async (req, res) => {
        try {
            ...
            if (customerId) {
                payments = payments.filter(payment => payment.customerId === parseInt(customerId));
            }
    ...
    app.get("/api/accounting/invoices/pending", async (req, res) => {
        try {
            const { customerId } = req.query;
            let pendingInvoices = generatePendingInvoices();
            // Filter by customer ID if provided
            if (customerId) {
                pendingInvoices = pendingInvoices.filter(invoice => invoice.customerId === parseInt(customerId));
            }
