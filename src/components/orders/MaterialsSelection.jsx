...
const MaterialsSelection = ({ value, onChange }) => {
    ...
    const handleAddMaterial = () => {
        ...
        const selectedMaterialObj = materials?.find((m) => m.id === Number(selectedMaterial));
    ...
    return (<div className="space-y-4">
          ...
          <DialogContent>
            ...
            <div className="grid gap-4 py-4">
              <div className="space-y-2">
                ...
                <Select value={selectedMaterial.toString()} onValueChange={setSelectedMaterial}>
                  <SelectTrigger id="material">
                    <SelectValue placeholder="Select material"/>
                  </SelectTrigger>
                  <SelectContent>
                    {isLoadingMaterials ? (<SelectItem value="loading" disabled>
                        Loading materials...
                      </SelectItem>) : (materials?.map((material) => (<SelectItem key={material.id} value={material.id.toString()}>
                          {material.name}
                        </SelectItem>)))}
                  </SelectContent>
                </Select>
      ...
      {value.length > 0 ? (<div className="border rounded-md">
          <Table>
            <TableHeader>
              <TableRow>
            ...
            <TableBody>
              {value.map((material, index) => (<TableRow key={`${material.id}-${index}`}>
                  <TableCell className="font-medium">{material.name}</TableCell>
                  <TableCell>
                    <div className="flex items-center space-x-2">
                      <Input type="number" min="1" value={material.quantity} onChange={(e) => handleQuantityChange(index, Number(e.target.value))} className="w-20"/>
                  ...
                  <TableCell>
                    <Button variant="ghost" size="sm" onClick={() => handleRemoveMaterial(index)} className="h-8 w-8 p-0">
        ...
        </div>) : (<div className="flex items-center justify-center border rounded-md p-4">
