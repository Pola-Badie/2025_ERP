...
const CustomerProfileDialog = ({ open, onOpenChange, customer }) => {
    ...
    const exportToPDF = async () => {
        ...
        try {
            ...
            if (profileData?.invoices && profileData.invoices.length > 0) {
                ...
                // Table rows
                doc.setTextColor(0, 0, 0);
                profileData.invoices.forEach((invoice, index) => {
                    if (index % 2 === 0) {
        ...
        try {
            ...
            if (profileData?.invoices && profileData.invoices.length > 0) {
                const invoiceData = [
                    ...
                    ['Total Amount:', `EGP ${profileData.invoices.reduce((sum, inv) => sum + inv.totalAmount, 0).toLocaleString()}`],
                    ['Paid Invoices:', profileData.invoices.filter((inv) => inv.paymentStatus === 'paid').length],
                    ['Pending Invoices:', profileData.invoices.filter((inv) => inv.paymentStatus !== 'paid').length]
                ];
    ...
    return (<Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-gray-100">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="space-y-2">
                ...
                <div className="bg-white p-3 rounded border border-yellow-200">
                  {customer.taxNumber ? (<span className="text-blue-600 font-medium font-mono">{customer.taxNumber}</span>) : (<span className="text-gray-500">Not registered with ETA</span>)}
                </div>
              </div>
              ...
              <Table>
                ...
                <TableBody>
                  {isLoading ? (<TableRow>
                    ...
                    </TableRow>) : profileData?.invoices.length === 0 ? (<TableRow>
                      <TableCell colSpan={6} className="text-center py-8 text-gray-500">
                        No invoices found for this customer
                      </TableCell>
                    </TableRow>) : (profileData?.invoices.map((invoice) => (<TableRow key={invoice.id}>
        ...
        <div className="flex justify-end gap-3 pt-6 border-t">
          <Button variant="outline" onClick={() => onOpenChange(false)} className="border-gray-300 hover:bg-gray-50">
            Close
          </Button>
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="outline" className="border-blue-300 text-blue-600 hover:bg-blue-50" disabled={isExporting}>
