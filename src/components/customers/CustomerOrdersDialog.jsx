...
const generateMockOrders = (customerId, realProducts = []) => {
...
const CustomerOrdersDialog = ({ open, onOpenChange, customer }) => {
    const [isExporting, setIsExporting] = useState(false);
    const [selectedOrder, setSelectedOrder] = useState(null);
    // Fetch real products from database
    const { data: products = [] } = useQuery({
        queryKey: ['/api/products'],
        select: (data) => data.filter(p => p.productType === 'finished').slice(0, 10)
    });
    if (!customer)
        return null;
    ...
    return (<Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto">
          ...
          <DialogTitle className="flex items-center justify-between">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="outline" size="sm" disabled={isExporting} className="border-blue-300 text-blue-600 hover:bg-blue-50">
        ...
        {/* Order Details Modal */}
        {selectedOrder && (<Dialog open={!!selectedOrder} onOpenChange={() => setSelectedOrder(null)}>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {selectedOrder.items.map((item, index) => (<TableRow key={index}>
