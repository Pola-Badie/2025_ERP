...
export const PrintableInvoice = ({ invoiceNumber, paperInvoiceNumber, approvalNumber, date, customer, items, subtotal, discountAmount = 0, taxRate, taxAmount, grandTotal, paymentTerms, notes, amountPaid = 0, paymentStatus, }) => {
    ...
    return (<div className={`printable-invoice bg-white p-8 max-w-4xl mx-auto text-black ${isRTL ? 'rtl' : 'ltr'}`} dir={isRTL ? 'rtl' : 'ltr'}>
      {/* Header */}
      <div className={`flex ${isRTL ? 'flex-row-reverse' : ''} justify-between items-start mb-8 border-b pb-6`}>
        <div className={`company-info flex ${isRTL ? 'flex-row-reverse' : ''} items-start gap-4`}>
          <img src={''} alt="Premier ERP Logo" className="w-16 h-16 object-contain" style={{ display: 'none' }}/>
        ...
        <div className={`invoice-header ${isRTL ? 'text-left' : 'text-right'}`}>
          ...
          <div className="text-sm">
            ...
            {paperInvoiceNumber && (<p><span className="font-semibold">{t('paperInvoiceNumber')}:</span> {paperInvoiceNumber}</p>)}
            {approvalNumber && (<p><span className="font-semibold">{t('approvalNo')}:</span> {approvalNumber}</p>)}
        ...
        <div className="bg-gray-50 p-4 rounded border">
          <div className={`flex ${isRTL ? 'flex-row-reverse' : ''} justify-between items-start`}>
            <div className="flex-1">
              {customer.company ? (<div className={isRTL ? 'text-right' : ''}>
                  <h3 className="font-medium text-lg">{customer.company}</h3>
                  <p className="text-sm text-gray-600">{customer.name}</p>
                </div>) : (<h3 className={`font-medium text-lg ${isRTL ? 'text-right' : ''}`}>{customer.company || customer.name}</h3>)}
          ...
          <div className={`space-y-1 pt-2 border-t mt-3 ${isRTL ? 'text-right' : ''}`}>
            ...
            {customer.taxNumber && (<p className="text-sm text-gray-600">{t('etaNumber')}: {customer.taxNumber}</p>)}
            {customer.address && (<p className="text-sm text-gray-600">{t('address')}: {customer.address}</p>)}
          </div>
        </div>
      </div>
      ...
      <div className={`flex ${isRTL ? 'justify-start' : 'justify-end'} mb-8`}>
        <div className="w-80">
          <div className="border border-gray-300 bg-gray-50">
            ...
            {discountAmount > 0 && (<div className={`flex ${isRTL ? 'flex-row-reverse' : ''} justify-between px-4 py-2 border-b border-gray-300 text-green-600`}>
      ...
      <div className="payment-status mb-6">
        <div className={`flex ${isRTL ? 'flex-row-reverse' : ''} items-center gap-4`}>
          ...
          <span className={`px-3 py-1 rounded-full text-sm font-medium ${paymentStatus === 'paid' ? 'bg-green-100 text-green-800' :
            paymentStatus === 'partial' ? 'bg-yellow-100 text-yellow-800' :
                'bg-red-100 text-red-800'}`}>
            {t(paymentStatus)}
          </span>
        </div>
        {paymentTerms && (<p className={`text-sm text-gray-600 mt-2 ${isRTL ? 'text-right' : ''}`}>
            <span className="font-medium">{t('paymentTerms')}:</span> {paymentTerms} {t('days')}
          </p>)}
      </div>

      {/* Notes */}
      {notes && (<div className="notes mb-8">
