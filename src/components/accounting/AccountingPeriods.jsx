...
const AccountingPeriods = () => {
    ...
    const handleStatusChange = (periodId, action) => {
    ...
    return (<div className="space-y-6">
          ...
          <DialogContent className="sm:max-w-[500px]">
            ...
            <Form {...form}>
              <form onSubmit={form.handleSubmit((data) => createPeriodMutation.mutate(data))} className="space-y-4">
                <FormField control={form.control} name="periodName" render={({ field }) => (<FormItem>
      ...
      {/* Warning alerts */}
      {hasNoOpenPeriods && (<Alert variant="destructive" className="mb-4">
      ...
      {hasOverlappingPeriods && (<Alert className="mb-4 border-yellow-500 text-yellow-600 bg-yellow-50">
          <AlertTriangle className="h-4 w-4"/>
          <AlertTitle>Warning</AlertTitle>
          <AlertDescription>
      ...
      {/* Current period card */}
      {currentPeriod && (<Card className="mb-4">
          <CardContent className="pt-6">
            <div className="flex justify-between items-center">
              ...
              <Badge variant={currentPeriod.status === 'open' ? 'outline' : 'secondary'}>
                {currentPeriod.status === 'open' ? (<><Unlock className="h-3 w-3 mr-1"/> Open</>) : (<><Lock className="h-3 w-3 mr-1"/> Closed</>)}
              </Badge>
            </div>
          </CardContent>
        </Card>)}
                ...
                <TableCell colSpan={5} className="text-center">No accounting periods found. Create one to get started.</TableCell>
                  ...
                  <TableCell>
                    <Badge variant={period.status === 'open' ? 'outline' : 'secondary'}>
                      {period.status === 'open' ? (<><Unlock className="h-3 w-3 mr-1"/> Open</>) : (<><Lock className="h-3 w-3 mr-1"/> Closed</>)}
                    </Badge>
                  </TableCell>
                  <TableCell className="text-right">
                    {period.status === 'open' ? (<Button variant="outline" size="sm" onClick={() => handleStatusChange(period.id, 'close')}>
                        <Lock className="h-3 w-3 mr-1"/> Close Period
                      </Button>) : (<Button variant="outline" size="sm" onClick={() => handleStatusChange(period.id, 'open')}>
      ...
      <Dialog open={isConfirmDialogOpen} onOpenChange={setIsConfirmDialogOpen}>
        <DialogContent>
          ...
          {confirmAction?.action === 'close' && (<Alert className="mt-2 border-yellow-500 text-yellow-600 bg-yellow-50">
              <AlertTriangle className="h-4 w-4"/>
          ...
          <DialogFooter>
            ...
            <Button type="button" variant={confirmAction?.action === 'close' ? 'destructive' : 'default'} onClick={confirmStatusChange} disabled={updatePeriodStatusMutation.isPending}>
