          ...
          <TableBody>
            {users.length === 0 ? (<TableRow>
                <TableCell colSpan={6} className="text-center py-4 text-muted-foreground">
                  ...
                  <TableCell>
                    {user.active !== false ? (<span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-green-100 text-green-800">
                  ...
                  <TableCell className="text-right">
                    <Button variant="ghost" size="sm" onClick={() => openEditUserDialog(user)} className="h-8 w-8 p-0 mr-2">
                      <Pencil className="h-4 w-4"/>
                      <span className="sr-only">Edit</span>
                    </Button>
                    {user.active !== false && (<Button variant="ghost" size="sm" onClick={() => handleDeactivateUser(user.id)} className="h-8 w-8 p-0 text-red-500 hover:text-red-700 hover:bg-red-50">
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(handleAddUser)} className="space-y-4">
              ...
              <DialogFooter>
                ...
                <Button type="submit" disabled={form.formState.isSubmitting || addUserMutation.isPending}>
                  {(form.formState.isSubmitting || addUserMutation.isPending) && (<Loader2 className="mr-2 h-4 w-4 animate-spin"/>)}
                  Add User
                </Button>
              </DialogFooter>
            </form>
          ...
          <Form {...editForm}>
            <form onSubmit={editForm.handleSubmit(handleEditUser)} className="space-y-4">
              ...
              <DialogFooter>
                ...
                <Button type="submit" disabled={editForm.formState.isSubmitting || editUserMutation.isPending}>
                  {(editForm.formState.isSubmitting || editUserMutation.isPending) && (<Loader2 className="mr-2 h-4 w-4 animate-spin"/>)}
                  Update User
                </Button>
              </DialogFooter>
            </form>
        ...
        <div className="border rounded-md p-4 space-y-6">
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
              {['products', 'categories', 'sales', 'purchases', 'reports', 'users', 'system_preferences', 'backups'].map((resource) => (<div key={`admin-${resource}`} className="flex items-center space-x-2">
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
              {['View Products', 'Manage Sales', 'View Customers', 'Create Customers', 'View Reports'].map((permission) => (<div key={`staff-${permission}`} className="flex items-center space-x-2">
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
              {['Manage Products', 'Manage Categories', 'Manage Purchases', 'View Reports', 'View Suppliers', 'Create Suppliers'].map((permission) => (<div key={`manager-${permission}`} className="flex items-center space-x-2">
