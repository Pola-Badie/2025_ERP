...
import { SettingsIcon, ShoppingCartIcon, UsersIcon, PackageIcon, DollarSignIcon, FileTextIcon, BarChart3Icon, TruckIcon, ShieldCheckIcon, BellIcon, CalendarIcon, TagIcon, CreditCardIcon, UserCheckIcon, DatabaseIcon, GlobeIcon, ChevronDownIcon } from 'lucide-react';
import { Collapsible, CollapsibleContent } from '@/components/ui/collapsible';
const ModuleConfigurationTab = ({ preferences, refetch }) => {
    ...
    useEffect(() => {
        if (preferences) {
            const modulePrefs = preferences.filter((pref) => pref.category === 'modules');
            if (modulePrefs.length) {
                const newModuleSettings = { ...moduleSettings };
                modulePrefs.forEach((pref) => {
    ...
    const handleModuleToggle = (moduleId, setting, value) => {
          ...
          <CardContent>
            <div className="space-y-4">
              {modules
                ...
                return (<div key={module.id} className="border rounded-lg p-4">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-4">
                          <div className="flex items-center gap-2">
                            ...
                            <Switch id={`${module.id}-visible`} checked={settings.visible} onCheckedChange={(checked) => handleModuleToggle(module.id, 'visible', checked)} disabled={!settings.enabled}/>
                          </div>
                          <div className="flex items-center gap-2">
                            <Label htmlFor={`${module.id}-enabled`} className="text-sm">Enabled</Label>
                            <Switch id={`${module.id}-enabled`} checked={settings.enabled} onCheckedChange={(checked) => handleModuleToggle(module.id, 'enabled', checked)}/>
                          </div>
                          ...
                          <div className="pl-9 space-y-3">
                            <div>
                              ...
                              <div className="grid grid-cols-2 gap-2">
                                {module.features.map(feature => (<div key={feature} className="flex items-center gap-2 text-sm">
