...
export const InventoryTable = ({ products, isLoading, onEditProduct, onDeleteProduct, onCreateLabel, calculateExpiryStatus, }) => {
    if (isLoading) {
        ...
        <tbody>
          {products.map((product) => {
            const expiryStatus = product.expiryDate
                ...
                : null;
            return (<tr key={product.id} className="border-b">
                ...
                <td className="px-4 py-3">
                  {product.expiryDate ? (<div className="flex items-center">
                      <span className={expiryStatus?.status === 'expired' ? 'text-red-500' :
                        expiryStatus?.status === 'near-expiry' ? 'text-orange-700' : ''}>
                        {formatDate(product.expiryDate)}
                      </span>
                      {expiryStatus && (<span className={`ml-2 text-xs ${expiryStatus.status === 'expired' ? 'text-red-500' :
                            expiryStatus.status === 'near-expiry' ? 'text-orange-700' : ''}`}>
                ...
                <td className="px-4 py-3 text-right">
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      ...
                      <DropdownMenuItem className="text-red-600" onClick={() => onDeleteProduct(product)}>
