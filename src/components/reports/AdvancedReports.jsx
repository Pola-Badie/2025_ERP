...
export default function AdvancedReports() {
    ...
    const MetricCard = ({ title, value, change, icon: Icon, color }) => (<Card>
      <CardContent className="p-6">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm font-medium text-muted-foreground">{title}</p>
            <p className="text-2xl font-bold">{value}</p>
            {change && (<p className={`text-sm flex items-center ${change > 0 ? 'text-green-600' : 'text-red-600'}`}>
    ...
    return (<div className="container mx-auto p-6 space-y-6">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Select value={selectedWarehouse} onValueChange={setSelectedWarehouse}>
            ...
            <SelectContent>
              ...
              {warehouses?.map((warehouse) => (<SelectItem key={warehouse.id} value={warehouse.id}>
                  {warehouse.name}
                </SelectItem>))}
            </SelectContent>
          </Select>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <MetricCard title="Total Revenue" value={`$${reportData?.financialSummary.totalRevenue.toLocaleString()}`} change={12.5} icon={DollarSign} color="text-green-600"/>
        <MetricCard title="Total Orders" value={reportData?.salesTrend.reduce((sum, item) => sum + item.orders, 0)} change={8.3} icon={FileText} color="text-blue-600"/>
        <MetricCard title="Active Customers" value={reportData?.customerMetrics.length} change={-2.1} icon={Users} color="text-purple-600"/>
        <MetricCard title="Inventory Value" value={`$${reportData?.inventoryAnalysis.reduce((sum, item) => sum + item.value, 0).toLocaleString()}`} change={5.7} icon={Package} color="text-orange-600"/>
      </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <Card>
              ...
              <CardContent>
                <ResponsiveContainer width="100%" height={300}>
                  <PieChart>
                    <Pie data={reportData?.inventoryAnalysis} cx="50%" cy="50%" labelLine={false} label={({ name, percent }) => `${name} ${(percent * 100).toFixed(0)}%`} outerRadius={80} fill="#8884d8" dataKey="value">
                      {reportData?.inventoryAnalysis.map((entry, index) => (<Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]}/>))}
                    </Pie>
                    <Tooltip />
                  </PieChart>
                </ResponsiveContainer>
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-4">
                {reportData?.customerMetrics.slice(0, 10).map((customer, index) => (<div key={index} className="flex items-center justify-between p-4 border rounded-lg">
                    <div>
