...
import { Plus, Trash2, FileText, Factory, TestTube, Package, Eye, Save, Send, ArrowLeft, Calculator, Truck, Edit, MessageCircle, Mail, ChevronDown } from 'lucide-react';
...
const CreateQuotation = () => {
    ...
    // Form state
    const [quotationType, setQuotationType] = useState('finished');
...
6. Liability: Our liability is limited to the value of services provided. We maintain comprehensive insurance coverage for pharmaceutical operations.`);
    ...
    const { data: refiningServices = [], isLoading: isLoadingRefining } = useQuery({
        ...
        select: (semiProducts) => semiProducts.map(product => ({
            name: product.name,
            uom: product.unit || 'kg',
    ...
    const { data: finishedProducts = [], isLoading: isLoadingFinished } = useQuery({
        ...
        select: (rawProducts) => rawProducts
            .filter(product => product.productType === 'finished')
            .map(product => ({
            name: product.name,
            uom: product.unitOfMeasure || 'units',
    ...
    const getQuotationTypeIcon = (type) => {
        switch (type) {
    ...
    const getQuotationTypeDescription = (type) => {
        switch (type) {
    ...
    const getTransportationTypeLabel = (type) => {
        switch (type) {
    ...
    const generateQuotationPDF = async () => {
        try {
            ...
            const previewElement = document.querySelector('.printable-quotation');
            if (!previewElement) {
                toast({
                    title: "Error",
    ...
    return (<div className="container mx-auto p-6 space-y-6" dir={isRTL ? 'rtl' : 'ltr'}>
      {/* Header */}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {['manufacturing', 'refining', 'finished'].map((type) => (<Card key={type} className={`cursor-pointer transition-all ${quotationType === type ? 'ring-2 ring-blue-500 bg-blue-50' : 'hover:bg-gray-50'}`} onClick={() => {
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div>
                ...
                <Textarea placeholder={t('addSpecialNotes')} value={notes} onChange={(e) => setNotes(e.target.value)} rows={3}/>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <ProductSearch value={selectedProduct} onChange={(product) => {
            ...
            setNewItem({
                ...
                <div>
                  ...
                  <Input placeholder={t('additionalDescription')} value={newItem.description} onChange={(e) => setNewItem({ ...newItem, description: e.target.value })}/>
                </div>
                <div>
                  <Label>{t('quantity')}</Label>
                  <Input type="number" min="1" placeholder={t('enterQuantity')} value={newItem.quantity || ''} onChange={(e) => setNewItem({ ...newItem, quantity: Number(e.target.value) || 0 })}/>
                </div>
                <div>
                  <Label>{t('unitOfMeasure')}</Label>
                  <Select value={newItem.uom} onValueChange={(value) => setNewItem({ ...newItem, uom: value })}>
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                ...
                <div>
                  ...
                  <Input type="number" step="0.01" min="0" placeholder="0.00" value={newItem.unitPrice || ''} onChange={(e) => setNewItem({ ...newItem, unitPrice: Number(e.target.value) || 0 })}/>
                </div>
                <div>
                  <Label>{t('grade')}</Label>
                ...
                <div>
                  <Label>{t('total')}</Label>
                  <Input value={`$${((newItem.quantity || 0) * (newItem.unitPrice || 0)).toFixed(2)}`} readOnly/>
                </div>
              </div>

              {/* Additional fields based on quotation type */}
              {quotationType === 'manufacturing' && (<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <Label>{t('processingTime')}</Label>
                    <Input type="number" min="1" placeholder={t('enterProcessingDays')} value={newItem.processingTime || ''} onChange={(e) => setNewItem({ ...newItem, processingTime: Number(e.target.value) || 0 })}/>
                  </div>
                  <div>
                    <Label>{t('qualityGrade')}</Label>
                    <Select value={newItem.qualityGrade} onValueChange={(value) => setNewItem({ ...newItem, qualityGrade: value })}>
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
              ...
              {quotationType === 'refining' && (<div>
                  <Label>{t('specificationsRequirements')}</Label>
                  <Textarea placeholder={t('specifyPurityRequirements')} value={newItem.specifications} onChange={(e) => setNewItem({ ...newItem, specifications: e.target.value })} rows={2}/>
                </div>)}
                    ...
                    <TableBody>
                      {items.map((item) => (<TableRow key={item.id}>
                          ...
                          <TableCell className="text-sm text-muted-foreground">
                            {item.description}
                            {item.type === 'manufacturing' && item.processingTime && (<div className="text-xs mt-1">
                                {t('processing')}: {item.processingTime} {t('days')} | {t('grade')}: {item.qualityGrade}
                              </div>)}
                            {item.type === 'refining' && item.specifications && (<div className="text-xs mt-1">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <Input placeholder="e.g., Bottles, Boxes, Bags" value={newPackagingItem.type} onChange={(e) => setNewPackagingItem({ ...newPackagingItem, type: e.target.value })} data-testid="input-packaging-type"/>
                </div>
                <div>
                  <Label>Description</Label>
                  <Input placeholder="Packaging description" value={newPackagingItem.description} onChange={(e) => setNewPackagingItem({ ...newPackagingItem, description: e.target.value })} data-testid="input-packaging-description"/>
                </div>
                <div>
                  <Label>Quantity</Label>
                  <Input type="number" min="1" value={newPackagingItem.quantity} onChange={(e) => setNewPackagingItem({ ...newPackagingItem, quantity: Number(e.target.value) || 1 })} data-testid="input-packaging-quantity"/>
                </div>
                <div>
                  <Label>Unit Price (EGP)</Label>
                  <Input type="number" step="0.01" min="0" value={newPackagingItem.unitPrice || ''} onChange={(e) => setNewPackagingItem({ ...newPackagingItem, unitPrice: Number(e.target.value) || 0 })} placeholder="0.00" data-testid="input-packaging-unit-price"/>
                </div>
              </div>
              
              <div>
                <Label>Notes</Label>
                <Textarea placeholder="Special packaging requirements or notes" value={newPackagingItem.notes} onChange={(e) => setNewPackagingItem({ ...newPackagingItem, notes: e.target.value })} rows={2} data-testid="textarea-packaging-notes"/>
              </div>
              
              <Button onClick={addPackagingItem} className="w-full" data-testid="button-add-packaging">
                <Plus className="mr-2 h-4 w-4"/>
                Add to Quotation
              </Button>
              
              {/* Packaging Items Table */}
              {packagingItems.length > 0 && (<div className="mt-6">
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {packagingItems.map((item) => (<TableRow key={item.id}>
                          <TableCell>
                            <div>
                          ...
                          <TableCell className="text-center">
                            <Button variant="ghost" size="icon" onClick={() => removePackagingItem(item.id)} data-testid={`button-remove-packaging-${item.id}`}>
                              <Trash2 className="h-4 w-4"/>
                            </Button>
                          </TableCell>
                        </TableRow>))}
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Input type="number" step="0.01" min="0" value={transportationFees || ''} onChange={(e) => setTransportationFees(Number(e.target.value) || 0)} placeholder="0.00"/>
                </div>
              </div>
              <div>
                <Label>{t('transportationNotes')}</Label>
                <Textarea placeholder={t('addSpecialTransportationRequirements')} value={transportationNotes} onChange={(e) => setTransportationNotes(e.target.value)} rows={2}/>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <Textarea value={termsAndConditions} onChange={(e) => setTermsAndConditions(e.target.value)} rows={8} placeholder={t('enterTermsAndConditions')}/>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="space-y-6">
          <Card>
            ...
            <CardContent className="space-y-4">
              {selectedCustomer && (<div className="text-sm">
              ...
              <div className="space-y-2 text-sm">
                ...
                {transportationFees > 0 && (<div className="flex justify-between text-muted-foreground">
                    <span>{t('transportation')}:</span>
                    <span>EGP {transportationFees.toFixed(2)}</span>
                  </div>)}
                {calculatePackagingTotal() > 0 && (<div className="flex justify-between text-muted-foreground">
              ...
              <div className="mt-4">
                ...
                <Input type="number" min="0" max="100" step="0.1" value={vatPercentage} onChange={(e) => setVatPercentage(Number(e.target.value) || 0)}/>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className="space-y-2">
              <Button variant="outline" className="w-full justify-start" onClick={() => setIsPreviewOpen(true)}>
                <Eye className="mr-2 h-4 w-4"/>
                {t('previewQuotation')}
              </Button>
              <Button variant="outline" className="w-full justify-start" onClick={() => handleSubmit('draft')} disabled={!selectedCustomer || items.length === 0}>
                <Save className="mr-2 h-4 w-4"/>
                {t('saveDraft')}
              </Button>
              <Button className="w-full justify-start" onClick={() => handleSubmit('send')} disabled={!selectedCustomer || items.length === 0}>
      ...
      <Dialog open={isPreviewOpen} onOpenChange={setIsPreviewOpen}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-auto">
          ...
          <div className="space-y-6">
            {selectedCustomer ? (<PrintableQuotation quotationNumber={quotationNumber} date={new Date()} validUntil={validUntil} customer={{
            ...
            }} items={items} packagingItems={packagingItems} subtotal={subtotal} transportationFees={transportationFees} packagingFees={packagingFees} vatPercentage={vatPercentage} vatAmount={vatAmount} grandTotal={grandTotal} notes={notes} transportationType={transportationType} transportationNotes={transportationNotes} packagingType={packagingType} packagingNotes={packagingNotes} quotationType={quotationType} termsAndConditions={termsAndConditions}/>) : (<div className="text-center py-8">
                <FileText className="h-12 w-12 text-muted-foreground mx-auto mb-4"/>
