...
import { Search, Plus, Edit, Trash2, Eye, Upload, FileText, Download, ShoppingBag, Paperclip, Landmark, DollarSign, ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight, Truck } from "lucide-react";
...
export default function Procurement() {
    ...
    const handleChangePurchaseOrderStatus = (order, newStatus) => {
        setPurchaseOrders(prev => prev.map(po => po.id === order.id
            ? { ...po, status: newStatus }
            : po));
        toast({
    ...
    const createPurchaseOrder = async () => {
        ...
        try {
            ...
            const expectedDeliveryDate = document.getElementById('delivery-date')?.value;
    ...
    return (<div className="space-y-6">
        ...
        <CardContent className="p-6">
          <div className="flex flex-wrap items-center gap-4">
            <div className="relative flex-1 min-w-[280px]">
              ...
              <Input placeholder="Search by PO number or supplier..." className="pl-9" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}/>
            </div>
      ...
      <Card className="mb-6">
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {samplePurchaseOrders.slice(0, 3).map((order, index) => (<div key={order.id} className="bg-gradient-to-br from-slate-50 to-blue-50 p-4 rounded-lg border border-blue-200 hover:shadow-md transition-shadow">
                ...
                {order.materials && order.materials.length > 0 && (<div className="mt-2 pt-2 border-t border-blue-200">
                    <p className="text-xs text-gray-500">
                      {order.materials.length} item(s): {order.materials[0]?.name}
                      {order.materials.length > 1 && ` +${order.materials.length - 1} more`}
                    </p>
                  </div>)}
              </div>))}
          </div>
          
          {samplePurchaseOrders.length > 3 && (<div className="mt-4 text-center">
      ...
      <Card data-procurement-table>
        <CardHeader className="pb-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <Button variant="outline" size="sm" onClick={() => setLocation('/create-invoice')} className="flex items-center gap-2">
                <FileText className="h-4 w-4"/>
                View Invoices
              </Button>
              <Button variant="outline" size="sm" onClick={() => setLocation('/suppliers')} className="flex items-center gap-2">
          ...
          {filteredPurchaseOrders && filteredPurchaseOrders.length > 0 && (<div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-4">
              ...
              <div className="bg-purple-50 p-4 rounded-lg border border-purple-200">
                ...
                .reduce((sum, order) => sum + parseFloat(order.totalAmount || '0'), 0) /
                Math.max(filteredPurchaseOrders.length, 1)).toLocaleString()} EGP
                    </p>
                  </div>
                </div>
              </div>
        ...
        <CardContent className="p-0">
          {isLoading ? (<div className="p-8 flex justify-center">
            ...
            </div>) : paginatedOrders && paginatedOrders.length > 0 ? (<div className="overflow-x-auto">
              <table className="w-full">
                    ...
                    day: '2-digit'
                      ...
                      <td className="px-6 py-4 text-sm text-gray-900">
                        <div className="max-w-xs">
                          {order.materials && order.materials.length > 0 ? (<div className="space-y-1">
                              {order.materials.slice(0, 2).map((material, index) => (<div key={index} className="text-xs">
                                  {material.name} ({material.quantity} {material.unit})
                                </div>))}
                              {order.materials.length > 2 && (<div className="text-xs text-gray-500">
                                  +{order.materials.length - 2} more items
                                </div>)}
                            </div>) : (<span className="text-xs text-gray-500 italic">No items specified</span>)}
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                ...
                }} className="h-8 w-8 p-0 text-green-600 hover:text-green-700 hover:bg-green-50" title="Send to Financial Accounting">
          ...
          {totalPages > 1 && (<div className="px-6 py-4 border-t border-gray-200 bg-gradient-to-r from-gray-50 to-blue-50">
              <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                ...
                <div className="flex items-center space-x-1">
                  {/* First Page Button */}
                  <Button variant="outline" size="sm" onClick={() => goToPage(1)} disabled={currentPage === 1} className="px-2 py-1 hidden sm:flex" title="First page">
                    <ChevronsLeft className="h-4 w-4"/>
                  </Button>

                  {/* Previous Button */}
                  <Button variant="outline" size="sm" onClick={goToPreviousPage} disabled={currentPage === 1} className="px-3 py-1 flex items-center gap-1">
                  ...
                  {/* Page Numbers */}
                  <div className="flex items-center space-x-1">
                    {getPageNumbers().map((pageNum, index) => (<div key={index}>
                        {pageNum === '...' ? (<span className="px-3 py-1 text-gray-500 font-medium">...</span>) : (<Button variant={currentPage === pageNum ? "default" : "outline"} size="sm" onClick={() => goToPage(pageNum)} className={`px-3 py-1 min-w-[40px] font-medium transition-all duration-200 ${currentPage === pageNum
                  ...
                  {/* Next Button */}
                  <Button variant="outline" size="sm" onClick={goToNextPage} disabled={currentPage === totalPages} className="px-3 py-1 flex items-center gap-1">
                    <span className="hidden sm:inline">Next</span>
                    <ChevronRight className="h-4 w-4"/>
                  </Button>

                  {/* Last Page Button */}
                  <Button variant="outline" size="sm" onClick={() => goToPage(totalPages)} disabled={currentPage === totalPages} className="px-2 py-1 hidden sm:flex" title="Last page">
      ...
      <Dialog open={isPurchaseOrderFormOpen} onOpenChange={setIsPurchaseOrderFormOpen}>
        <DialogContent className="sm:max-w-[900px] max-h-[90vh] overflow-y-auto scrollbar-thin scrollbar-thumb-blue-400 scrollbar-track-blue-100 bg-gradient-to-br from-slate-50 to-blue-50 border-0 shadow-2xl">
              ...
              <div className="border border-gray-200 rounded-lg overflow-hidden">
                  ...
                  {purchaseItems.map((item, index) => (<div key={item.id} className={`px-4 py-3 border-b border-gray-100 ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}>
                      <div className="grid grid-cols-12 gap-2 items-center">
                        ...
                        <div className="col-span-1">
                          <input type="number" value={item.quantity} onChange={(e) => updatePurchaseItem(item.id, 'quantity', parseFloat(e.target.value) || 0)} min="0" step="0.01" className="w-full px-1 py-1 text-sm border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-blue-500"/>
                        </div>
                        <div className="col-span-1">
                        ...
                        <div className="col-span-1">
                          <input type="number" value={item.unitPrice} onChange={(e) => updatePurchaseItem(item.id, 'unitPrice', parseFloat(e.target.value) || 0)} min="0" step="0.01" placeholder="Price" className="w-full px-1 py-1 text-sm border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-blue-500"/>
                        </div>
                        <div className="col-span-1">
                          <input type="date" value={item.expiryDate} onChange={(e) => updatePurchaseItem(item.id, 'expiryDate', e.target.value)} className="w-full px-1 py-1 text-xs border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-blue-500"/>
                        </div>
                        <div className="col-span-1">
                          <div className="space-y-1">
                            <div className="flex flex-col gap-1">
                              <Select value={item.discountType} onValueChange={(value) => updatePurchaseItem(item.id, 'discountType', value)}>
                              ...
                              <input type="number" value={item.discountValue} onChange={(e) => updatePurchaseItem(item.id, 'discountValue', parseFloat(e.target.value) || 0)} min="0" step="0.01" placeholder="0" className="w-full px-1 py-0.5 text-xs border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-blue-500"/>
                            </div>
                          </div>
                        </div>
                        <div className="col-span-1">
                        ...
                        <div className="col-span-1">
                          <div className="flex flex-col items-center gap-1">
                            <Button type="button" onClick={() => removePurchaseItem(item.id)} variant="ghost" size="sm" className="h-5 w-5 p-0 text-red-500 hover:text-red-700">
                              <Trash2 className="h-3 w-3"/>
                            </Button>
                            {item.expiryDate && new Date(item.expiryDate) < new Date(Date.now() + 30 * 24 * 60 * 60 * 1000) && (<div className="text-xs text-orange-500" title="Expires within 30 days">
                                ⚠️
                              </div>)}
                          </div>
                        </div>
                ...
                <div className="bg-gray-50 px-4 py-3 border-t border-gray-200">
                  <div className="space-y-3">
                    ...
                    {/* Expiry Date Summary */}
                    {purchaseItems.some(item => item.expiryDate) && (<div className="border-t border-gray-300 pt-2 mt-3">
                        <div className="text-xs text-gray-600 mb-1">Expiry Summary:</div>
                        {purchaseItems.filter(item => item.expiryDate && new Date(item.expiryDate) < new Date(Date.now() + 30 * 24 * 60 * 60 * 1000)).length > 0 && (<div className="text-xs text-orange-600 flex items-center gap-1">
                        ...
                        {purchaseItems.filter(item => item.expiryDate && new Date(item.expiryDate) < new Date()).length > 0 && (<div className="text-xs text-red-600 flex items-center gap-1">
            ...
            <div className="bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200 rounded-lg p-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Input id="transportation-cost" type="number" value={transportationCost} onChange={(e) => setTransportationCost(parseFloat(e.target.value) || 0)} placeholder="0.00" min="0" step="0.01" className="text-right"/>
                </div>
              </div>
              <div className="mt-3">
                <Label htmlFor="transportation-notes">Transportation Notes</Label>
                <Textarea id="transportation-notes" value={transportationNotes} onChange={(e) => setTransportationNotes(e.target.value)} placeholder="Special handling instructions, delivery location details, contact information..." rows={2} className="mt-1"/>
              </div>
            </div>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setIsPurchaseOrderFormOpen(false)} className="border-gray-300 hover:bg-gray-50">
              Cancel
            </Button>
            <Button onClick={createPurchaseOrder} className="bg-blue-600 hover:bg-blue-700 text-white">
              <Plus className="h-4 w-4 mr-2"/>
              Create Purchase Order
            </Button>
          </DialogFooter>
          ...
          {detailsOrder && (<div className="space-y-4">
              <div className="grid grid-cols-2 gap-4 p-4 bg-gray-50 rounded-lg">
                ...
                <div>
                  ...
                  <p className="font-semibold">{detailsOrder.paymentMethod || 'Not specified'}</p>
                </div>
                <div>
                  <p className="text-sm font-medium text-muted-foreground">Payment Terms</p>
                  <p className="font-semibold">{detailsOrder.paymentTerms || 'Not specified'}</p>
                </div>
                <div>
                  <p className="text-sm font-medium text-muted-foreground">Transportation Type</p>
                  <p className="font-semibold">{detailsOrder.transportationType || 'Standard Delivery'}</p>
                </div>
                <div>
                  <p className="text-sm font-medium text-muted-foreground">Transportation Cost</p>
                  <p className="font-semibold text-green-600">{parseFloat(detailsOrder.transportationCost || 0).toLocaleString('en-EG', { style: 'currency', currency: 'EGP' })}</p>
                </div>
                {detailsOrder.paymentDueDate && (<div className="col-span-2">
                    <p className="text-sm font-medium text-muted-foreground">Payment Due Date</p>
                    <p className="font-semibold text-red-600 text-lg">
                      {new Date(detailsOrder.paymentDueDate).toLocaleDateString()}
                    </p>
                  </div>)}
              </div>

              {/* Transportation Information Section */}
              {(detailsOrder.transportationType || detailsOrder.transportationCost || detailsOrder.transportationNotes) && (<div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
                  <div className="flex items-start gap-3">
                    <div className="p-1 bg-blue-100 rounded-full">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          ...
                          <p className="font-semibold text-blue-800">{detailsOrder.transportationType || 'Standard Delivery'}</p>
                        </div>
                        <div>
                          <p className="text-sm font-medium text-blue-700">Cost</p>
                          <p className="font-semibold text-blue-800">{parseFloat(detailsOrder.transportationCost || 0).toLocaleString('en-EG', { style: 'currency', currency: 'EGP' })}</p>
                        </div>
                      </div>
                      {detailsOrder.transportationNotes && (<div className="mt-3">
                          <p className="text-sm font-medium text-blue-700">Notes</p>
                          <p className="text-sm text-blue-600 mt-1 bg-white p-2 rounded border">{detailsOrder.transportationNotes}</p>
                        </div>)}
                    </div>
                  </div>
                </div>)}
              ...
              <div>
                ...
                return (<tr key={index} className="hover:bg-gray-50">
                    ...
                    <tfoot className="bg-gray-50">
                      {(() => {
                const items = detailsOrder.items || [];
                const itemsSubtotal = items.reduce((sum, item) => sum + parseFloat(item.total || 0), 0);
                ...
                return (<>
                            ...
                            {(detailsOrder.transportationCost && parseFloat(detailsOrder.transportationCost) > 0) && (<tr>
                                <td className="p-3 font-medium" colSpan={5}>Transportation Cost</td>
                                <td className="p-3 text-right font-medium">{parseFloat(detailsOrder.transportationCost || 0).toLocaleString('en-EG', { style: 'currency', currency: 'EGP' })}</td>
                              </tr>)}
              ...
              <div>
                <Label>Supporting Documents</Label>
                <div className="mt-2 border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-blue-400 transition-colors">
                  <input type="file" multiple accept=".pdf,.doc,.docx,.jpg,.jpeg,.png,.txt,.xls,.xlsx" onChange={handleDocumentUpload} className="hidden" id="details-document-upload"/>
                ...
                {/* Uploaded Documents List */}
                {(uploadedDocuments.length > 0 || (detailsOrder.documents && detailsOrder.documents.length > 0)) && (<div className="mt-3 space-y-2">
                    <h4 className="text-sm font-medium text-gray-700">Uploaded Documents:</h4>
                    <div className="max-h-32 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-gray-100">
                      {/* Show existing documents from order */}
                      {detailsOrder.documents && detailsOrder.documents.map((doc, index) => (<div key={`existing-${index}`} className="flex items-center justify-between bg-gray-50 p-2 rounded-md">
                          <div className="flex items-center space-x-2">
                            <Paperclip className="h-4 w-4 text-gray-500"/>
                      ...
                      {uploadedDocuments.map((file, index) => (<div key={`new-${index}`} className="flex items-center justify-between bg-gray-50 p-2 rounded-md">
                          <div className="flex items-center space-x-2">
                            <Paperclip className="h-4 w-4 text-gray-500"/>
                            <span className="text-sm text-gray-700 truncate">{file.name}</span>
                            <span className="text-xs text-gray-500">({(file.size / 1024 / 1024).toFixed(2)} MB)</span>
                ...
                {uploadedDocuments.length === 0 && !(detailsOrder.documents && detailsOrder.documents.length > 0) && (<div className="text-center py-6 text-muted-foreground">
