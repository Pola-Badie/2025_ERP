...
export const useNotificationsData = () => {
    ...
    const { data: recentInvoices } = useQuery({
        ...
        select: (data) => {
            ...
            return data?.filter(invoice => new Date(invoice.createdAt) > yesterday).slice(0, 5) || [];
        },
    ...
    const { data: recentOrders } = useQuery({
        ...
        select: (data) => {
            ...
            const yesterday = new Date();
            yesterday.setDate(yesterday.getDate() - 1);
            return data?.filter(order => new Date(order.createdAt) > yesterday).slice(0, 3) || [];
        },
    ...
    const generateNotifications = () => {
        ...
        if (expiringProducts && expiringProducts.length > 0) {
            const urgentlyExpiring = expiringProducts.filter(p => p.daysUntilExpiry <= 30 && p.daysUntilExpiry > 0);
            const expired = expiringProducts.filter(p => p.daysUntilExpiry <= 0 || p.expiryStatus === 'expired');
